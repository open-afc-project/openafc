...
build_dev_server() {
  ...
  docker_build ${wd}/Dockerfile ${SRV_DI}:${rand} "--build-arg BLD_TAG=${rand} --build-arg PRINST_TAG=${rand} --build-arg BLD_NAME=${D4B_DEV} --build-arg PRINST_NAME=${PRINST_DEV} --build-arg BUILDREV=`git rev-parse --short HEAD`"
}
