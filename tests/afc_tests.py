...
""
	...
	Description
		...
		The execution always done in specific order :
	configuration,
	database,
	testing.In case there are not available valid responses to compare with need to
			make acquisition and
		create new database as following../
			afc_tests
				.py-- addr<address> --log debug-- cmd run ""
									  "

			import argparse import certifi import csv import datetime import hashlib
				import inspect import io import json import logging import openpyxl
					as oxl import os import re import requests import shutil
						import sqlite3 import subprocess import sys import
							time

								import smtplib import ssl from email
									import encoders from email
				.mime.base import MIMEBase from
					email.mime.multipart import MIMEMultipart from email.mime
				.text import MIMEText

					from bs4 import BeautifulSoup from deepdiff import DeepDiff
						from multiprocessing
				.pool import Pool from string import Template from
					_afc_types import *from _afc_errors import *from _version
						import __version__ from _wfa_types import *

							AFC_URL_SUFFIX =
		'/fbrat/ap-afc/' AFC_REQ_NAME = 'availableSpectrumInquiry' AFC_WEBUI_URL_SUFFIX =
			'/fbrat/ratapi/v1/' AFC_WEBUI_REQ_NAME =
				'availableSpectrumInquirySec' headers = {'content-type':
'application/json'
}
...
#Channel numbers
	...
	try : numbers = self._get_item(src, path[:-1] + ["channelCfi"],
                                             default_last=[])
                    chan_dict.update(
...
def send_email(cfg):
    ...
    if isinstance(cfg['email_to'], type(None)):
        app_log.debug(f"({os.getpid()}) {inspect.stack()[0][3]}()"
                f" Not sending email because of no receiver specified")
        return
...
#add tls certificates if explicitly provided
    ...
    app_log.debug(f"Client {cli_certs}, Server {ser_cert}")
				    ...
				    try : rawresp
					    = post_func(cfg['url_path'],
							params = params_data,
							data = new_req,
							headers = headers,
							timeout = 600,
							#10 min cert = cli_certs,
							allow_redirects = False,
							verify = ser_cert if cfg
								['verif'] else False) if cfg
						      ['webui'] :
						    ...
						    tState = resp.get('taskState')
								     params_data['task_id'] =
						    tId while (tState == 'PENDING') or
						    (tState == 'PROGRESS') :
						    app_log.debug('_run_test() state %s, tid %s, status %d',
								  tState,
								  tId,
								  rawresp.status_code) time.sleep(2)
							    rawresp =
							    requests.get(cfg['url_path'],
									 params =
										 params_data) if rawresp
								    .status_code == 200 :
						    resp = rawresp.json() break

							    app_log.debug(f "({os.getpid()}) "
									    "{inspect.stack()[0][3]"
									    "}()" f
									    " Resp status: "
									    "{rawresp.status_"
...
#add tls certificates if explicitly provided
						    ...
						    os.environ['REQUESTS_CA_BUNDLE'] =
							    certifi.where() app_log
								    .debug(f "REQUESTS_CA_BUNDLE " f
									     "{os.environ.get('"
									     "REQUESTS_CA_BUNDLE')"
									     "}") if "REQUESTS_CA_"
										     "BUNDLE" in os
								    .environ :
						    ser_cert = "".join(os.environ.get(
								    'REQUESTS_CA_BUNDLE'))
								       cfg['verif'] = True else :
						    app_log.error(f "Missing CA certificate while "
								    "forced.") return

						    app_log.debug(f "Client {cli_certs}, Server "
								    "{ser_cert}")
...
#get login
					    ...
					    try : rawresp
						    = ssn.get(url_login,
							    ...
							    ssn.cookies
								    .get('csrf_token',
									 default ='') if not csrf_token :
							    app_log.error("CSRF token not found in "
									  "login response")
								    app_log.debug(
									    f
									    "({os.getpid()}) "
...
#fetch username and password from test db
								    ...
								    con.cursor() cur.execute(
									    'SELECT * FROM %s\n' %
									    TBL_USERS_NAME)
									    found_user = cur.fetchall() con
												 .close()
													 found_json =
									    json.loads(found_user
											       [0]
											       [1])
										    app_log.debug(
											    f
						    ...
						    try : rawresp
							    = ssn.post(url_login,
								    ...
								    app_log.debug('%s() The db file is exists, no need to create new one.',
									    ...
									    app_log
									    .info('Create DB tables (%s, %s) from source files',
										  TBL_REQS_NAME,
										  TBL_RESPS_NAME)
										    con = sqlite3.connect(
								    filename) cur =
								    con.cursor() cur
									    ...
									    .close() return True
										    ...
										    def compare_afc_config(
											    cfg) :
								    ""
								    "
								    Compare AFC configuration from
									    the DB with provided one
									    .""
									     "
								    app_log.debug('%s()',
										  inspect.stack()
									    ...
									    if not os.path
									    .isfile(cfg['db_filename']) :
								    app_log.error(
									    'Missing DB file %s',
									    cfg['db_filename']) return AFC_ERR
...
#get record from the input file
												    if isinstance (
													    ...
													    type(None)) :
								    app_log.debug(
									    'Missing input file to compare with.') return AFC_OK
									    ...
									    filename = cfg['infile']
											  [0] with open(
												  ...
												  as fp_test :
								    while True :
								    rec = fp_test.read() if not rec :
								    break
							    try : get_rec = json.loads(rec)
									    ...
									    = json
										    ...
										    Fetch test vectors
											    from the
											      ...
											      found_resp,
											      ids,
											      test_ids = _convert_reqs_n_resps_to_dict(
												      cfg)
...
#to reacquisition
										    ...
										    all_resps =
											    True if all_resps:
									    try : app_log.debug(f"{inspect.stack()[0][3]}() "
...
#drop existing table of requests and create new one
    ...
    cur = con.cursor()
    try : cur
	    .execute('DROP TABLE ' + TBL_REQS_NAME) except Exception as OperationalError :
		    app_log.debug(f "Fail to drop, missing table {TBL_REQS_NAME}") cur
			    .execute('CREATE TABLE ' + TBL_REQS_NAME +
				     ' (test_id varchar(50), data json)') con
			    .commit() return insert_reqs_int(filename, con, cur)
		    ...
		    app_log.debug(f "{inspect.stack()[0][3]}()")
			    ...
			    if isinstance (cfg['infile'], type(None)) :
		    app_log.error(f "Missing input file") return AFC_ERR

		    filename = cfg['infile'][0] app_log
				       .debug(f "{inspect.stack()[0][3]}() {filename}")

					       if not os.path.isfile(filename) :
		    app_log.error(f "Missing raw test data file {filename}") return AFC_ERR

		    if not os.path.isfile(cfg['db_filename']) :
		    app_log.error(f "Unable to find test db file.") return AFC_ERR

		    con = sqlite3.connect(cfg['db_filename'])
#add more rows to existing table of requests
				  app_log.info(
					  f "Extending table of requests ({TBL_REQS_NAME})") cur =
		    con.cursor() return insert_reqs_int(filename, con, cur)

			    def insert_devs(cfg) :
		    ""
		    "
		    Insert device
		    descriptors from input file to a table in test db.Drop previous table of devices
			    .""
			     "
		    app_log.debug(f "{inspect.stack()[0][3]}()")
		    ...
		    con = sqlite3.connect(cfg['db_filename'])
#drop existing table of requests and create new one
				  app_log.info(f "Drop table of devices ({TBL_AP_CFG_NAME})") cur =
			    con.cursor()
	    try : cur.execute('DROP TABLE ' + TBL_AP_CFG_NAME)
    except Exception as OperationalError:
...
#
            ...
            freq_range.set_range_limit(
                ...
                'high')
		    try : res_str
			    += freq_range.append_range() except IncompleteFreqRange as e :
				    app_log.debug(f "{e} -  row {i}") freq_range =
				    AfcFreqRange() freq_range
					    .set_range_limit(
						    sheet.cell(row = i,
					    ...
					    .set_range_limit(
						    ...
						    'high')
			    try : tmp_str
				    = freq_range.append_range() res_str +=
					    ', ' + tmp_str except IncompleteFreqRange as e :
					    app_log.debug(f "{e} -  row {i}")
						    ...
						    cell = sheet.cell(
							    row = i,
							    column =
								    MINDESIREDPOWER) if (cell.value) :
					    res_str += REQ_MIN_DESIRD_PWR + str(cell.value) +
						       ', '
#
#Location
#
						       res_str += REQ_LOC_HEADER cell = sheet.cell(
							       row = i,
							       column = INDOORDEPLOYMENT) res_str +=
					    REQ_LOC_INDOORDEPL + str(cell.value) +
					    ', '

#Location - elevation
					    res_str += REQ_LOC_ELEV_HEADER cell = sheet.cell(
						    row = i,
						    column =
							    ELE_VERTICALUNCERTAINTY) if isinstance (cell.value,
												    int) :
					    res_str +=
					    REQ_LOC_VERT_UNCERT + str(cell.value) +
					    ', ' cell = sheet.cell(row = i, column = ELE_HEIGHTTYPE)
								res_str +=
					    REQ_LOC_HEIGHT_TYPE + '"' + str(cell.value) + '"' cell =
						    sheet.cell(
							    row = i,
							    column =
								    ELE_HEIGHT) if isinstance (cell.value,
											       int) or
						    isinstance(cell.value, float) :
					    res_str += ', ' + REQ_LOC_HEIGHT +
						       str(cell.value) res_str += '}, '

#Location - uncertainty reqion
					    geo_coor = AfcGeoCoordinates(sheet, i)
				    try : res_str += geo_coor.collect_coordinates()
...
def test_report(fname, runtimedata, testnumdata, testvectordata,
    ...
    Args:
        ...
        testnumdata(str):
				    Tested case id.testvectordata(int): Tested vector id.
...
if __name__ == '__main__':
							    try : main() except KeyboardInterrupt:
        sys.exit(1)
