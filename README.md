...
|AFC_WEBUI_BIND|`0.0.0.0`|webui|the socket to bind. a string of the form: <host>|
|AFC_WEBUI_PORT|`8000`|webui|the port to use in bind. a string of the form: <port>|
|AFC_WEBUI_PID|`/run/gunicorn/openafc_app.pid`|webui|a filename to use for the PID file|
|AFC_WEBUI_WORKERS|`20`|webui|the number of worker processes for handling requests|
|AFC_WEBUI_TIMEOUT|`180`|webui|workers silent for more than this many seconds are killed and restarted|
|AFC_WEBUI_ACCESS_LOG||webui|the Access log file to write to. Default to don't. Use `/proc/self/fd/2` for console|
|AFC_WEBUI_ERROR_LOG|`/proc/self/fd/2`|webui|the Error log file to write to|
|AFC_WEBUI_LOG_LEVEL|`info`|webui|The granularity of Error log outputs (values are 'debug', 'info', 'warning', 'error', 'critical'|
...
|**ALS settings**||||
|ALS_KAFKA_SERVER_ID||rat_server, msghnd, afcserver, worker, uls_downloader, cert_db, rcache|Prefix for source container name for JSON log messages|
|ALS_KAFKA_CLIENT_BOOTSTRAP_SERVERS|als_kafka:9092|rat_server, msghnd, afcserver, worker, uls_downloader, cert_db, rcache, als_siphon|Kafka server(s) used by ALS|
|ALS_KAFKA_MAX_REQUEST_SIZE|10485760|rat_server, msghnd, afcserver, worker, uls_downloader, cert_db, rcache, als_siphon|Maximum Kafka message size|
|AFC_ALS_TOPIC_NAME|ALS|rat_server, msghnd, afcserver, worker, uls_downloader, cert_db, rcache, als_kafka|kafka topic name for ALS messages|
|AFC_JSON_TOPIC_PREFIX|""|rat_server, msghnd, afcserver, worker, uls_downloader, cert_db, rcache, als_kafka|Prefix for JSON log topic names|
|POSTGRES_ALS_CONN_STR|bulk_postgres|als_siphon|Full or partial (at least host name) DSN for ALS database|
|POSTGRES_LOG_CONN_STR|bulk_postgres|als_siphon|Full or partial (at least hist name) DSN for JSON log database|
|POSTGRES_ALS_PASSWORD_FILE|/run/secrets/ALS_DB_PASSWORD|als_siphon|File with ALS database password|
|POSTGRES_LOG_PASSWORD_FILE|/run/secrets/ALS_JSON_LOG_DB_PASSWORD|als_siphon|File with SON log database password|
