...
from utils import error, error_if, execute, ImageRegistryInfo, \
    parse_json_output, parse_k3d_reg
...
def main(argv: List[str]) -> None:
    ...
    argument_parser.add_argument(
        "TAG",
        help="Tag of images to put to registry")
    ...
    for image_base_name, image_info in images.items():
        execute(
            ["docker", "image", "tag", image_info.image_id,
             f"{reg_info.pull}/{image_base_name}:{args.TAG}"])
        if reg_info.push != reg_info.pull:
            execute(
                ["docker", "image", "tag", image_info.image_id,
                 f"{reg_info.push}/{image_base_name}:{args.TAG}"])
