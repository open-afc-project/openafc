...
from utils import ArgumentParserFromFile, auto_name, AUTO_NAME, \
    ...
    parse_k3d_reg, parse_kubecontext, ROOT_DIR, SCRIPTS_DIR, unused_argument, \
    yaml_load, yaml_loads
...
SERVICE_TYPE_PATH = "components.{component}.serviceType"
...
class ClusterHandler:
    ...
    def push_images(self) -> None:
        """ Push images to registry, used by Kubernetes """
    ...
    def get_helm_values(self, helm_args: List[str]) -> Dict[str, Any]:
        """ Retrieves resulting helm values for given helm command """
        dry_run = execute(helm_args + ["--dry-run", "--debug"],
                          return_output=True, fail_on_error=False)
        if dry_run is None:
            return {}
        m = re.search(r"\nCOMPUTED VALUES:\n((.|\n)+?)\n---", dry_run)
        error_if(not m,
                 "Can't obtain helm values: unrecognized helm output "
                 "structure")
...
class ClusterHandlerK3d(ClusterHandler):
    ...
    def push_images(self) -> None:
        """ Push images of given tag to registry, used by Kubernetes """
        assert self._tag is not None
    ...
    def pull_registry(self) -> Optional[str]:
        """ Name of pull image registry or None """
        return self._registry_info.pull if self._registry_info else None

    def max_workers(self, helm_args: List[str]) -> Optional[int]:
        """ Maximum number of worker replicas or None """
        if self._args.max_workers:
            return super().max_workers(helm_args)
        v = self.get_helm_values(helm_args) or {}
...
def main(argv: List[str]) -> None:
    ...
    argument_parser = \
        ArgumentParserFromFile(
            description=f"Starts (internal) AFC server from helmcharts. "
            f"Command line parameters may also be passed via file (prefixed "
            f"with '@') or {ARG_ENV_VAR} environment variable",
    ...
    args = \
        argument_parser.parse_args(
            shlex.split(os.environ.get(ARG_ENV_VAR, "")) + argv)
    ...
    error_if(
        ...
        "Image registry requireds but not found/specified")
    ...
    try:
        ...
        if was_running and (not args.upgrade):
        ...
        # ... assumed settings ...
        if tag:
            install_args += ["--set", f"{PUSH_IMAGES_SCRIPT}={tag}"]
