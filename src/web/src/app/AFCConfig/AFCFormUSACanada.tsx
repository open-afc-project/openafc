...
> {
  private trimmedRegionStr = () => {
    if (
      !!this.props.config.regionStr &&
      (this.props.config.regionStr?.startsWith('TEST_') || this.props.config.regionStr?.startsWith('DEMO_'))
    ) {
  ...
  private setMinEIRPIndoor = (n: number) =>
    this.props.updateConfig(Object.assign(this.props.config, { minEIRPIndoor: n }));
  private setMinOutdoorEIRP = (n: number) =>
    this.props.updateConfig(Object.assign(this.props.config, { minEIRPOutdoor: n }));
  ...
  private setFeederLoss = (f: FSReceiverFeederLoss) =>
    this.props.updateConfig(Object.assign(this.props.config, { receiverFeederLoss: f }));
  private setReceiverNoise = (f: FSReceiverNoise) =>
    this.props.updateConfig(Object.assign(this.props.config, { fsReceiverNoise: f }));
      ...
      case 'CA':
        return (
          <>
            <FormSelectOption
              key="2020 Land Cover "
              value="rat_transfer/nlcd/ca/landcover-2020-classification_resampled.tif"
              label="Land Cover of Canada"
            />
          </>
        );
      case 'BR':
        return (
          <>
            <FormSelectOption
              key="Brazilian Land Cover"
              value="rat_transfer/nlcd/br/landcover-for-brazil.tbd.tif"
              label="Land Cover of Brazil (TBD)"
            />
          </>
        );
      case 'GB':
        return (
          <>
            <FormSelectOption
              ...
              value="rat_transfer/nlcd/eu/U2018_CLC2012_V2020_20u1_resampled.tif"
      ...
      default:
        return (
          <>
            <FormSelectOption
              key="Production NLCD "
              value="rat_transfer/nlcd/nlcd_production"
              label="Production NLCD"
            />
  ...
  render = () => (
    <>
        ...
        <FormGroup
          label={
            this.props.limit.indoorEnforce
              ? 'AP Min. Indoor EIRP (Min: ' + this.props.limit.indoorLimit + ' dBm)'
              : 'AP Min. Indoor EIRP'
          }
          fieldId="horizontal-form-min-eirp"
        >
          <InputGroup>
            <TextInput
        ...
        <FormGroup
          label={
            this.props.limit.outdoorEnforce
              ? 'AP Min. Outdoor EIRP (Min: ' + this.props.limit.outdoorLimit + ' dBm)'
              : 'AP Min. Outdoor EIRP'
          }
          fieldId="horizontal-form-min-eirp"
        >
        ...
        <FormGroup label="AP Max. EIRP" fieldId="horizontal-form-max-eirp">
          <InputGroup>
            <TextInput
              ...
              validated={this.props.limit.indoorEnforce || this.props.limit.outdoorEnforce
                  ? this.props.config.minEIRPIndoor <= this.props.config.maxEIRP &&
        ...
        <FormGroup label="FS Receiver Feeder Loss" fieldId="horizontal-form-receiver-feeder-loss">
          ...
          <Tooltip
            ...
            content={
              <>
                ...
                <ul>
                  <li>the Feederloss in the FS Database (if present)</li>
          ...
          <InputGroup>
            ...
            <TextInput
              ...
              onChange={(_event, x) => this.setFeederLoss({ ...this.props.config.receiverFeederLoss, UNKNOWN: Number(x) })}
              type="number"
              step="any"
        ...
        <FormGroup label="FS Receiver Noise Floor" fieldId="horizontal-form-receiver-noise">
          ...
          <FormGroup label="FS Freq <= 6425 MHz" fieldId="horizontal-form-noiseFloor-0">
            <InputGroup>
              <TextInput
                value={
                  this.hasValueExists(() => this.props.config.fsReceiverNoise?.noiseFloorList[0])
                    ? this.props.config.fsReceiverNoise?.noiseFloorList[0]
                    : -110
                }
                type="number"
                onChange={(_event, x) =>
                ...
                validated={this.hasValueExists(() => this.props.config.fsReceiverNoise?.noiseFloorList[0]) ? 'default' : 'error'}
                style={{ textAlign: 'right' }}
              />
              <InputGroupText>dBm/MHz</InputGroupText>
            </InputGroup>
          ...
          <FormGroup label="FS Freq > 6425 MHz" fieldId="horizontal-form-noiseFloor-1">
            <InputGroup>
              <TextInput
                value={
                  this.hasValueExists(() => this.props.config.fsReceiverNoise?.noiseFloorList[1])
                ...
                validated={this.hasValueExists(() => this.props.config.fsReceiverNoise.noiseFloorList[1]) ? 'default' : 'error'}
                style={{ textAlign: 'right' }}
              />
      ...
      <GalleryItem>
        <BodyLossForm data={this.props.config.bodyLoss} onChange={this.setBodyLoss} />
      </GalleryItem>
      <GalleryItem>
      ...
      <GalleryItem>
        <FormGroup label="Max Link Distance to Consider" fieldId="propogation-model-max-link-distance">
          <InputGroup>
            <TextInput
      ...
      <GalleryItem>
        <APUncertaintyForm data={this.props.config.APUncertainty} onChange={this.setAPUncertainty} />
      </GalleryItem>
      <GalleryItem>
      ...
      {(this.props.config.propagationModel.kind === 'ITM with no building data' ||
        ...
        <GalleryItem>
          <FormGroup label="AP Propagation Environment" fieldId="propogation-env">
            {' '}
            <Tooltip
              position={TooltipPosition.top}
              enableFlip={true}
              ...
              content={
                <>
                  ...
                  <ul>
                    <li>
                      - "Land Cover Point" assigns propagation environment based on single Land Cover tile class the
                      RLAN resides in: Urban if Land Cover tile = 23 or 24, Suburban if Land Cover tile = 22, Rural-D
                      (Deciduous trees) if Land Cover tile = 41, 43 or 90, Rural-C (Coniferous trees) if Land Cover tile
                      = 42, and Rural-V (Village Center) otherwise. The various Rural types correspond to the P.452
                      clutter category.
                    </li>
                    <li>
            ...
            >
              ...
              <FormSelectOption
                key="Population Density Map"
                value="Population Density Map"
      ...
      <GalleryItem>
        <Tooltip
          ...
          content={
            <p>
              When distance &gt; 1km and FS Receiver (Rx) is in Urban/Suburban, P.2108 clutter loss is added at FS Rx
              when FS Receiver AGL height &lt; <b>Max FS AGL Height</b>
            </p>
          }
        >
        ...
        {this.props.config.clutterAtFS == true ? (
          <FormGroup fieldId="fsclutter-maxFsAglHeight" label="Max FS AGL Height (m)">
            <InputGroup>
              <TextInput
                type="number"
      ...
      <GalleryItem>
        <FormGroup
          label="AP Height below Min Allowable AGL Height Behavior"
        ...
        >
          ...
          <Tooltip
            ...
            content={
              <>
                ...
                <p>
                  Note that this is meant to mainly prevent the portion of uncertainty region to be underground due to
                  height uncertainty and terrain variation.
                </p>
                <p>
                  If the AGL height of all scan points is below ground (without height uncertainty), an error will be
                  reported.
                </p>
              </>
            }
          ...
          >
            <FormSelectOption
              key={'truncate'}
              value={'truncate'}
              label={'Truncate AP height to min allowable AGL height'}
            />
      ...
      <GalleryItem>
        <Tooltip
          ...
          content={
            <p>
              If enabled, the Virtual AP page will add map information via the Vendor extension and present a Google Map
              on the page
            </p>
          }
        >
      ...
      <GalleryItem>
        <FormGroup label="Visiblilty Threshold" fieldId="visiblity-threshold">
          <InputGroup>
            <TextInput
              ...
              style={{ textAlign: 'right' }}
              validated={!!this.props.config.visibilityThreshold || this.props.config.visibilityThreshold === 0 ? 'default' : 'error'}
            />
