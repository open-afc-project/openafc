 ...
 */
const penetrationLossModels = ['Fixed Value'];

/**
 * Sub form component for body loss model
 */
export default class BodyLossForm extends React.PureComponent<{
  data: BodyLossModel;
  onChange: (x: BodyLossModel) => void;
}> {
  ...
  private getExpansion = () => {
    switch (this.props.data.kind) {
      ...
      case 'Fixed Value':
        return (
          <>
            <FormGroup label="RLAN Indoor Body Loss" fieldId="body-loss-fixed-value-in">
              <InputGroup>
                <TextInput
                  type="number"
            ...
            <FormGroup label="RLAN Outdoor Body Loss" fieldId="body-loss-fixed-value-out">
              <InputGroup>
                <TextInput
                  type="number"
                  id="body-loss-fixed-value-out"
                  name="body-loss-fixed-value-out"
                  validated={this.props.data.valueOutdoor <= 0 || this.props.data.valueOutdoor > 0 ? 'default' : 'error'}
