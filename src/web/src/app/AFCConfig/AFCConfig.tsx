...
> {
  constructor(
    ...
    if (props.afcConf.kind === 'Success') {
      ...
      if (props.afcConf.result.version !== guiConfig.version) {
        for (var p in state.config) {
          ...
          if (!props.afcConf.result.hasOwnProperty(p)) {
            incompatible = true;
            logger.error(
              'Could not load most recent AFC Config Defaults.',
              'Missing parameters. Current version ' +
                guiConfig.version +
                ' Incompatible version ' +
                props.afcConf.result.version,
            );
            Object.assign(state, {
              config: getDefaultAfcConf(lastRegFromCookie),
              messageType: 'Warn',
              ...
              messageValue:
                'The current version (' +
                guiConfig.version +
                ') is not compatible with the loaded configuration. The loaded configuration was created for version ' +
                props.afcConf.result.version +
                '. The default configuration has been loaded instead. To resolve this AFC Config will need to be updated below.',
            });
            break;
          }
        }
      ...
      logger.error(
        'Could not load most recent AFC Config Defaults.',
        'error code: ',
        props.afcConf.errorCode,
        'description: ',
        props.afcConf.description,
      );
    ...
    if (props.ulsFiles.kind === 'Success') {
      ...
      logger.error(
        'Could not load ULS Database files',
        'error code: ',
        props.ulsFiles.errorCode,
        'description: ',
        props.ulsFiles.description,
      );
    ...
    if (props.antennaPatterns.kind === 'Success') {
      ...
      logger.error(
        'Could not load antenna pattern files',
        'error code: ',
        props.antennaPatterns.errorCode,
        'description: ',
        props.antennaPatterns.description,
      );
  ...
  private submit = (conf: AFCConfigFile): Promise<RatResponse<string>> =>
    putAfcConfigFile(conf).then((resp) => {
    ...
    return (
      <PageSection>
        ...
        <AFCForm
          ...
          antennaPatterns={this.state.antennaPatterns}
          regions={this.state.regions}
          onSubmit={(x) => this.submit(x)}
        />
      </PageSection>
    );
  }
