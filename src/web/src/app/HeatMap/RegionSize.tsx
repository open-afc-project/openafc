...
const getLonDistance = (bounds: Bounds) =>
  ((Math.abs(bounds.east - bounds.west) * Math.PI) / 180) *
  earthRadius *
  Math.cos((minAbsLat(bounds.north, bounds.south) * Math.PI) / 180);
...
export const RegionSize: React.FunctionComponent<RegionSizeProps> = (props: RegionSizeProps) => (
  <p>
    Selected Region Size (m):{' '}
    {props.bounds
      ? getLatDistance(props.bounds.north, props.bounds.south).toFixed(0) +
        ' x ' +
        getLonDistance(props.bounds).toFixed(0)
