...
> {
  ...
  private static centerFrequencies: Map<number, number[]> = new Map([
    [
      ...
      Array(59)
        .fill(0)
        .map((_, i) => i * 20 + HeatMapForm.startingFreq + 10),
    ],
    [
      40,
      Array(29)
        .fill(0)
        .map((_, i) => i * 40 + HeatMapForm.startingFreq + 20),
    ],
    [
      80,
      Array(14)
        .fill(0)
        .map((_, i) => i * 80 + HeatMapForm.startingFreq + 40),
    ],
    [
      160,
      Array(7)
        .fill(0)
        .map((_, i) => i * 160 + HeatMapForm.startingFreq + 80),
    ],
  ]);
    ...
    if (this.state.analysisType === HeatMapAnalysisType.ItoN) {
      if (this.state.insideOutside === IndoorOutdoorType.BUILDING) {
        if (this.props.limit.indoorEnforce) {
          return (
            s.kind === 'b' && [
              Number.isFinite(s.in!) && s.in! >= this.props.limit.indoorLimit,
  ...
  private submit = () => {
    const allValid = [true]
      .concat(
        ...
        [
          this.state.fsIdType === HeatMapFsIdType.All ||
            (this.state.fsIdType === HeatMapFsIdType.Single && !!this.state.fsId),
        ],
      )
      .reduce((p, c) => p && c);
  ...
  private setParams = (v: HeatMapFormData) => {
    ...
    this.updateOperatingClass(
      {
        num: v.inquiredChannel.globalOperatingClass,
        include: OperatingClassIncludeType.Some,
        channels: [v.inquiredChannel.channelCfi],
      },
      0,
    );
  ...
  updateOperatingClass(
    ...
    if (!!x.channels && x.channels.length >= 1) {
      this.setState({
        inquiredChannel: { globalOperatingClass: x.num, channelCfi: x.channels[0] },
        operatingClasses: ocs,
      });
    } else {
            ...
            <FormGroup label="Indoor/Outdoor" fieldId="horizontal-form-indoor-outdoor">
              <InputGroup>
                ...
                >
                  ...
                  <FormSelectOption
                    isDisabled={false}
                    key={IndoorOutdoorType.INDOOR}
                    value={IndoorOutdoorType.INDOOR}
                    label={'Indoor'}
                  />
                  <FormSelectOption
                    isDisabled={false}
                    key={IndoorOutdoorType.OUTDOOR}
                    value={IndoorOutdoorType.OUTDOOR}
                    label={'Outdoor'}
                  />
                  <FormSelectOption
                    isDisabled={false}
                    key={IndoorOutdoorType.BUILDING}
                    value={IndoorOutdoorType.BUILDING}
                    label={'Selected per Building Data'}
                  />
                </FormSelect>
              </InputGroup>
            </FormGroup>
              ...
              {this.state.eirp.kind === 's' ? (
                <FormGroup
                  label={
                    this.props.limit.indoorEnforce
                      ? 'RLAN EIRP (Min: ' + this.props.limit.indoorLimit + ' dBm)'
                      : 'RLAN EIRP'
                  }
                ...
                <>
                  <FormGroup
                    label={
                      this.props.limit.indoorEnforce
                        ? 'Indoor RLAN EIRP (Min: ' + this.props.limit.indoorLimit + ' dBm)'
                        : 'Indoor RLAN EIRP'
                    }
                    fieldId="horizontal-form-eirp-in"
                  >
                    <InputGroup>
                      <TextInput
                  ...
                  <FormGroup
                    label={
                      this.props.limit.outdoorEnforce
                        ? 'Outdoor RLAN EIRP (Min: ' + this.props.limit.outdoorLimit + ' dBm)'
                        : 'Outdoor RLAN EIRP'
                    }
                    fieldId="horizontal-form-eirp-out"
                  >
                    <InputGroup>
                      <TextInput
                        ...
                        // @ts-ignore
                        onChange={(_event, x) =>
                          this.setState({ eirp: { kind: 'b', out: Number(x), in: this.state.eirp.in } })
                        }
                        type="number"
                        step="any"
              ...
              <InputGroup>
                ...
                >
                  <FormSelectOption
                    isDisabled={false}
                    key={'eirp'}
                    value={HeatMapAnalysisType.EIRP}
                    label="Availability (EIRP)"
                  />
