...
> {
  ...
  private formSubmitted(form: HeatMapFormData) {
    if (!this.state.selectionRectangle)
      return Promise.resolve(
        error('No Bounding Box Specified', undefined, 'Specifiy a bounding box to submit a request'),
      );
    const validBounds = [
      ...[this.validN, this.validE, this.validS, this.validW].map((f) => f(this.state.selectionRectangle)),
      this.validSpacing(this.state.spacing),
    ].reduce((p, c) => p && c);

    if (!validBounds)
      return Promise.resolve(
        error('Invalid Bounding Box', undefined, 'One or more parameters for the bounding box are invalid'),
      );
    ...
    if (form.indoorOutdoor.kind === IndoorOutdoorType.BUILDING) {
      ...
      if (stringHeight !== '') {
        ...
        this.setState({
          ...
          extraWarning: `The AP height has been truncated so that its minimum height is 1m above the terrain. The new height is ${stringHeight}`,
        });
      }
    } else {
      if (
        form.indoorOutdoor.heightType === 'AGL' &&
        form.indoorOutdoor.height - form.indoorOutdoor.heightUncertainty < 1
      ) {
    ...
    this.setState({
      ...
      messageValue: '',
      progress: { percent: 0, message: 'Submitting...' },
      submittedAnalysisType: form.analysis,
    });
    ...
    return spectrumInquiryRequestByString('1.4', JSON.stringify(dummyRequest))
        ...
        if (r.kind === 'Success') {
          ...
          if (
            ...
            if (!!geoJsonData) {
              ...
              this.heatMapStyle.set('HMAP', (feature: any) => ({
                strokeWeight: 0,
                fillColor: feature.getProperty('fill'),
                'fill-opacity': feature.getProperty('fill-opacity'),
                zIndex: 5,
              }));
              this.setMapState({
                val: geojson.geoJson as GeoJson,
                valid: true,
              ...
              this.setState({
                ...
                messageTitle: '',
                messageValue: '',
                mapCenter: { lng: centerLon, lat: centerLat },
              });
            }
          }
          ...
          this.setState({
            canCancelTask: false,
            messageType: 'danger',
            messageTitle: 'An error occured while running your request',
            messageValue: r.description,
          });
          return success('');
        }
      })
      .catch((e) => {
        this.setState({
          canCancelTask: false,
          messageType: 'danger',
          messageTitle: 'An unexpected error occured',
          messageValue: JSON.stringify(e),
        });
  ...
  private setConfig(value: string) {
    try {
      ...
      this.setMapState({ versionId: this.state.mapState.versionId + 1 });
      if (this.formUpdateCallback) this.formUpdateCallback(params);
    } catch (e) {
          ...
          <CardBody>
            ...
            >
              ...
              <Gallery hasGutter>
                  ...
                  <FormGroup label="North" fieldId="horizontal-form-north">
                    <InputGroup>
                      <TextInput
                        ...
                        validated={this.state.selectionRectangle && this.validN(this.state.selectionRectangle) ? 'default' : 'error'}
                ...
                <GalleryItem>
                  <FormGroup label="South" fieldId="horizontal-form-south">
                    <InputGroup>
                      <TextInput
                        ...
                        validated={this.state.selectionRectangle && this.validS(this.state.selectionRectangle) ? 'default' : 'error'}
                        style={{ textAlign: 'right' }}
                      />
                      <InputGroupText>degrees</InputGroupText>
                    </InputGroup>
                ...
                <GalleryItem>
                  <FormGroup label="East" fieldId="horizontal-form-east">
                    <InputGroup>
                      <TextInput
                        ...
                        validated={this.state.selectionRectangle && this.validE(this.state.selectionRectangle) ? 'default' : 'error'}
                        style={{ textAlign: 'right' }}
                      />
                      <InputGroupText>degrees</InputGroupText>
                    </InputGroup>
                ...
                <GalleryItem>
                  <FormGroup label="West" fieldId="horizontal-form-west">
                    <InputGroup>
                      <TextInput
                        ...
                        validated={this.state.selectionRectangle && this.validW(this.state.selectionRectangle) ? 'default' : 'error'}
                        style={{ textAlign: 'right' }}
                      />
                      <InputGroupText>degrees</InputGroupText>
                    </InputGroup>
...
const correctBounds = (rect?: Bounds): Bounds | undefined =>
  !rect || [rect.north, rect.south, rect.east, rect.west].map((x) => !Number.isFinite(x)).reduce((a, b) => a || b)
    ? undefined
    : rect;
