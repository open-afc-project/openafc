...
import { PAWSRequest, PAWSResponse, RatResponse, success, error } from './RatApiTypes';
import { logger } from './Logger';

/**
 * PawsApi.ts: types and functions for utilizing server's paws api
 * this file embeds a javascript json-rpc library
 * author: Sam Smucny
 */

/**
 * global jsonrpc object
 */
var simple_jsonrpc: any = null;
...
(function (root) {
  ...
  /*
     ...
     version: 1.0.0
     source: https://github.com/jershell/simple-jsonrpc-js/blob/master/simple-jsonrpc-js.js
     license: MIT
     */
  ...
  simple_jsonrpc = function () {
    ...
    function isRequest(message: any) {
      return !!message.method;
    }

    function isResponse(message: any) {
      return message.hasOwnProperty('result') && message.hasOwnProperty('id');
    }

    function beforeResolve(message: any) {
      var promises = [];
      if (isArray(message)) {
        forEach(message, function (msg: any) {
          // @ts-ignore
    ...
    self.messageHandler = function (rawMessage: any): Promise<any> {
      try {
...
// add handler for json-rpc
jrpc.toStream = function (msg: { jsonrpc: string; method: any; id: number }) {
  ...
  xhr.onreadystatechange = function () {
    ...
    try {
      ...
      jrpc.messageHandler(JSON.stringify({ id: JSON.parse(msg as any).id, error: e }));
    }
  };
  xhr.open('POST', guiConfig.paws_url, true);
  xhr.setRequestHeader('Content-type', 'application/json; charset=utf-8');
  xhr.send(msg as any);
};

/**
 * Retreives an analysis just like a real AP would
 * @param params JSON request object
 * @returns PAWSResponse or error
 */
export const PAWSAvailableSpectrum = (params: PAWSRequest): Promise<RatResponse<PAWSResponse>> =>
  jrpc.call('spectrum.paws.getSpectrum', params).then((resp: any) => {
    if (resp.deviceDesc) {
      return success(resp);
    } else {
      return error(resp.message, resp.code, resp);
    }
  });
