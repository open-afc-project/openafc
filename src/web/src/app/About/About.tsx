...
//
        ...
        setAboutAfc(this.state.name, this.state.email, this.state.org, this.state.token).then(res => {
            if (res.kind === "Success") {
                    this.setState({ messageType: "success", messageValue: "Request for " + this.state.email + " has been submitted"});
                } else {
    ...
    render() {
		...
		<div>
                ...
                <Button variant="primary" icon={<PlusCircleIcon />} onClick={() => this.submit()}>Submit</Button>

                <Card>
                {
                this.state.sitekey && <ReCAPTCHA sitekey={this.state.sitekey}
