...
export class About extends React.Component<any, any> {
  constructor(props: Readonly<{ content: any; sitekey: string }>) {
    super(props);
    this.state = { content: props.content.result, name: '', email: '', org: '', token: '', sitekey: props.sitekey, recaptchaError: false };
      ...
      <PageSection>
        <div>
          ...
          {this.state.sitekey && !this.state.recaptchaError && (
            <Card>
              <CardBody>
                <ReCAPTCHA sitekey={this.state.sitekey} onChange={this.onChange} onErrored={() => this.setState({ recaptchaError: true })} />
              </CardBody>
            </Card>
          )}
        </div>
