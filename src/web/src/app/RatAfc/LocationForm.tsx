              ...
              <FormGroup label="Latitude" fieldId="horizontal-form-latitude">
                <InputGroup>
                  <TextInput
                    ...
                    validated={ellipse.center.latitude !== undefined &&
                      ellipse.center.latitude >= -90 &&
                      ellipse.center.latitude <= 90 ? 'default' : 'error'}
                    style={{ textAlign: 'right' }}
                  />
              ...
              <FormGroup label="Longitude" fieldId="horizontal-form-longitude">
                <InputGroup>
                  <TextInput
                    ...
                    validated={ellipse.center.longitude !== undefined &&
                      ellipse.center.longitude >= -180 &&
                      ellipse.center.longitude <= 180 ? 'default' : 'error'}
                    style={{ textAlign: 'right' }}
                  />
                  <InputGroupText>degrees</InputGroupText>
                </InputGroup>
        ...
        {locType === 'Linear Polygon' && linearPoly && (
          <GalleryItem>
            <PairList
              ...
              onChange={(values) =>
                this.updateLinearPoly({ outerBoundary: values.map((p) => ({ latitude: p[0], longitude: p[1] })) })
              }
            />
          </GalleryItem>
        )}
          ...
          <>
              ...
              <FormGroup label="Center Latitude" fieldId="horizontal-form-latitude">
                <InputGroup>
                  <TextInput
                    ...
                    id="horizontal-form-latitude"
                    name="horizontal-form-latitude"
                    validated={radialPolygon.center.latitude !== undefined &&
                      radialPolygon.center.latitude >= -90 &&
                      radialPolygon.center.latitude <= 90 ? 'default' : 'error'}
                    style={{ textAlign: 'right' }}
                  />
                  <InputGroupText>degrees</InputGroupText>
                </InputGroup>
              ...
              <FormGroup label="Center Longitude" fieldId="horizontal-form-longitude">
                <InputGroup>
                  <TextInput
                    ...
                    id="horizontal-form-longitude"
                    name="horizontal-form-longitude"
                    validated={radialPolygon.center.longitude !== undefined &&
                      radialPolygon.center.longitude >= -180 &&
                      radialPolygon.center.longitude <= 180 ? 'default' : 'error'}
                    style={{ textAlign: 'right' }}
                  />
                  <InputGroupText>degrees</InputGroupText>
                </InputGroup>
            ...
            <GalleryItem>
              <PairList
                ...
                onChange={(values) =>
                  this.updateRadialPoly({ outerBoundary: values.map((p) => ({ angle: p[0], length: p[1] })) })
                }
              />
            </GalleryItem>
          </>
