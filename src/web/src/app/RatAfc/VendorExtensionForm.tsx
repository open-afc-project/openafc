...
import { GalleryItem, FormGroup, InputGroup, Radio, TextInput, InputGroupText, Select, SelectOption, SelectVariant, CheckboxSelectGroup, Checkbox, Gallery, Button, Label, ClipboardCopy, ClipboardCopyVariant } from "@patternfly/react-core";
...
export class VendorExtensionForm extends React.PureComponent<VendorExtensionFormParams, VendorExtensionFormState> {
    ...
    setExtensionId(x: string, idx: number): void {
        ...
        this.setState({ VendorExtensions: newVeList }, () => { this.props.onChange(this.state.VendorExtensions) });
    }
    ...
    setParameters(x: string, idx: number): void {
        try {
            ...
            msgArray[idx] = '';
            this.setState({ VendorExtensions: newVeList, parseValidationMessages: msgArray }, () => { this.props.onChange(this.state.VendorExtensions) });
        } catch (err) {
            ...
            <GalleryItem width={"50%"}>
                {
                    (this.state.VendorExtensions.map((ve, idx) => (
                        ...
                        <FormGroup fieldId={"ve-form-group-" + idx}  >
                            ...
                            <Button id={"delete-ve-" + idx} onClick={() => this.deleteVendorExtension(idx)} variant="tertiary">Delete this Extension</Button>
                        </FormGroup>

                    )))
                }
                <Button id="add-new-ve" onClick={() => this.addVendorExtension()} variant="tertiary">Add New Vendor Extension</Button>
            </GalleryItem>
        </Gallery>
    }
}