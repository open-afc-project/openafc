...
import {
  getDeniedRegions,
  getDeniedRegionsCsvFile,
  updateDeniedRegions,
  putAccessPointDenyList,
  getAccessPointsDeny,
  addAccessPointDeny,
} from '../Lib/Admin';
import {
...
export class DRList extends React.Component<DRListProps, DRListState> {
  ...
  onAdd = (dr: DeniedRegion) => {
    this.setState({
      deniedRegions: this.state.deniedRegions.concat(dr),
  ...
  deleteDR = (id: string) => {
    this.setState({
      deniedRegions: this.state.deniedRegions.filter(
        (x) => !(x.regionStr == this.state.regionStr && x.name + '===' + x.zoneType == id),
      ),
      deniedRegionsNeedSaving: true,
    });
  ...
  forceChangeRegion = () => {
    this.setState(
      {
        showingSaveWarning: false,
        regionStr: this.state.newRegionString!,
        newRegionString: undefined,
        deniedRegionsNeedSaving: false,
      },
      () => {
        this.loadCurrentDeniedRegions();
  ...
  putDeniedRegions = () => {
    updateDeniedRegions(
      this.state.deniedRegions.filter((x) => x.regionStr == this.state.regionStr),
      this.state.regionStr,
    ).then((res) => {
      if (res.kind === 'Success') {
        this.setState({ messageSuccess: 'Changes saved', deniedRegionsNeedSaving: false });
      } else {
        logger.error('Could not save denied regions', 'error code: ', res.errorCode, 'description: ', res.description);
      ...
      <PageSection>
        ...
        <Card>
          ...
          <CardBody>
            ...
            <br />
            {hasRole('Admin') && <Button onClick={() => this.exportList()}>Download Denied APs</Button>}
            <br />
          </CardBody>
        </Card>
      </PageSection>
    ...
    return (
      <PageSection id="ap-deny-list-page">
        <UserContext.Consumer>
          {(u: UserState) =>
            u.data.loggedIn && <DRList regions={this.state.regions} userId={u.data.userId} org={u.data.org} />
          }
        </UserContext.Consumer>
      </PageSection>
    );
