...
const insertNullPoints = (spectra: SpectrumProfile[][]): { hz: number; dbm?: number | null }[] =>
  spectra
    .reduce((prev, curr) => prev.concat([{ hz: (prev[prev.length - 1].hz + curr[0].hz) / 2, dbm: -Infinity }], curr))
...
export const SpectrumDisplayPAWS: React.FunctionComponent<{ spectrum?: PAWSResponse; greyOutUnii: boolean }> = (
  ...
  !props.spectrum || props.spectrum.spectrumSpecs.length === 0 ? (
    <Card>
      <CardHeader><CardTitle>There is no data to display. Try sending a request above.</CardTitle></CardHeader>
    </Card>
  ) : (
    <>
            ...
            <CardBody>
              <ResponsiveContainer width="100%" height={400}>
                ...
                >
                  <CartesianGrid />
                  <XAxis
                    label={{ value: 'Frequency (MHz)', position: 'bottom', fill: 'var(--pf-t--global--text--color--regular)' }}
                    domain={[5925, 7125]}
                    type="number"
                    dataKey="hz"
                    name="Frequency (MHz)"
                  ...
                  <YAxis
                    ...
                    name="Max. EIRP (dBm)"
                    tick={{ fill: 'var(--pf-t--global--text--color--regular)' }}
                    label={{ value: 'Max. EIRP (dBm)', angle: -90, position: 'insideLeft', fill: 'var(--pf-t--global--text--color--regular)' }}
                  />
                  <Tooltip cursor={true} />
...
export const SpectrumDisplayAFC: React.FunctionComponent<{ spectrum?: AvailableSpectrumInquiryResponse }> = (props) =>
  !props.spectrum || !props.spectrum.availableFrequencyInfo || props.spectrum.availableFrequencyInfo.length === 0 ? (
    <Card>
      <CardHeader><CardTitle>There is no spectrum data to display.</CardTitle></CardHeader>
    </Card>
  ) : (
    <>
        ...
        <CardHeader>
          <Title headingLevel="h3" style={{ textAlign: 'center' }}>
            {'Request ' +
              ...
              ': expires at ' +
              (props.spectrum?.availabilityExpireTime || 'No expiration')}
          </Title>
        </CardHeader>
        <CardBody>
          <ResponsiveContainer width="100%" height={400}>
            ...
            >
              ...
              <YAxis
                ...
                name="PSD (dBm/MHz)"
                tick={{ fill: 'var(--pf-t--global--text--color--regular)' }}
                label={{ value: 'PSD (dBm/MHz)', angle: -90, position: 'insideLeft', fill: 'var(--pf-t--global--text--color--regular)' }}
              />
              <Tooltip cursor={true} />
              <Scatter
                key={1}
...
const convert137OpClassData = (
  spectra: AvailableChannelInfo,
  channelIndex: number,
): { hz: number; dbm: number | undefined; cfi: number }[] => {
  var desc = channelDescription.find(
    (x) => x.OperatingClass == spectra.globalOperatingClass && x.OCDisplayname.endsWith(String(channelIndex)),
  );
  if (!!desc) {
    return desc.channels
      .map((e, i) => {
