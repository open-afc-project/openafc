# Indoor Certification in OpenAFC
...
For the certification database, the Indoor/Outdoor value is set in the Cert ID Sweep function (src/ratapi/ratapi/manage.py:903 and following)

### For US sweeps

The service queries for items on the 6SD list and skips any items with a different equipment class or missing the FCC ID for the entry.

The flag is set for OUTDOOR.

For items with a grant note of BX, the INDOOR flag is OR'd giving both flags set.

If the ID is already in the database, it gets the refresh time updated and the flag is reset as above. If it is a new entry, it gets added to the database.
