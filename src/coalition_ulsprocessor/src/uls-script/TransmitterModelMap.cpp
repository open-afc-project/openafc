...
void TransmitterModelMapClass::readModelList(const std::string filename)
{
    ...
    fieldLabelList.push_back("radioModelPrefix");
    fieldIdxList.push_back(&architectureFieldIdx);
    fieldLabelList.push_back("architecture");
        ...
        switch (lineType) {
            ...
            case dataLineType:
                ...
                if (!errorFlag) {
                    ...
                    if (strval.empty()) {
                        errStr << "ERROR: Transmitter Model List file \"" << filename << "\" line " << linenum << " missing architecture"
                               << std::endl;
                        ...
                        errStr << "ERROR: Transmitter Model List file \"" << filename << "\" line " << linenum << " invalid architecture: " << strval
                               << std::endl;
        ...
        for (ib = 0; ib < (int)transmitterModelList.size(); ++ib) {
            if (ib != ia) {
                ...
                if ((ma->architecture != TransmitterModelClass::UnknownArchitecture) && (mb->name.compare(0, ma->name.size(), ma->name) == 0)) {
                    numError++;
                    std::cout << "(" << numError << ") " << ma->name << "[" << TransmitterModelClass::architectureStr(ma->architecture)
                              << "] is a prefix of " << mb->name << "[" << TransmitterModelClass::architectureStr(mb->architecture) << "]"
                              << (ma->architecture != mb->architecture ? " DIFFERENT ARCHITECTURE" : "") << std::endl;
                }
            }
        }
    }
