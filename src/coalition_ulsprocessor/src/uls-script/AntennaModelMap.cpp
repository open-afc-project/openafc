...
/******************************************************************************************/
AntennaModelMapClass::AntennaModelMapClass(std::string antModelListFile, std::string antPrefixListFile, std::string antModelMapFile)
{
        ...
        switch (lineType) {
            ...
            case dataLineType:
                ...
                if (strval == "HP") {
                    ...
                    errStr << "ERROR: Antenna Model List file \"" << filename << "\" line " << linenum << " invalid category: " << strval
                           << std::endl;
                    throw std::runtime_error(errStr.str());
                }
                /**************************************************************************/
                ...
                if (strval.empty()) {
                    ...
                    if (diameterM <= 0.0) {
                        errStr << "ERROR: Antenna Model List file \"" << filename << "\" line " << linenum << " invalid diameter: \"" << strval
                               << "\"" << std::endl;
                        throw std::runtime_error(errStr.str());
                    }
                ...
                if (strval.empty()) {
                    ...
                    if (reflectorWidthM <= 0.0) {
                        errStr << "ERROR: Antenna Model List file \"" << filename << "\" line " << linenum << " invalid reflector width: \"" << strval
                               << "\"" << std::endl;
                        throw std::runtime_error(errStr.str());
                    }
                }
                ...
                if (strval.empty()) {
                    ...
                    if (reflectorHeightM <= 0.0) {
                        errStr << "ERROR: Antenna Model List file \"" << filename << "\" line " << linenum << " invalid reflector height: \""
                               << strval << "\"" << std::endl;
                        throw std::runtime_error(errStr.str());
                    }
                }
...
void AntennaModelMapClass::readPrefixList(const std::string filename)
{
    ...
    if (!(fp = fopen(filename.c_str(), "rb"))) {
        str = std::string("ERROR: Unable to open Antenna Prefix File \"") + filename + std::string("\"\n");
        throw std::runtime_error(str);
    }
    ...
    while (fgetline(fp, line, false)) {
        ...
        switch (lineType) {
            ...
            case dataLineType:
                ...
                if (strval.empty()) {
                    errStr << "ERROR: Antenna Prefix file \"" << filename << "\" line " << linenum << " missing prefix" << std::endl;
                    throw std::runtime_error(errStr.str());
                }
                ...
                strval = fieldList.at(typeFieldIdx);
                if (strval.empty()) {
                    errStr << "ERROR: Antenna Prefix file \"" << filename << "\" line " << linenum << " missing type" << std::endl;
                    throw std::runtime_error(errStr.str());
                }
                ...
                if ((strval == "Ant") || (strval == "Antenna")) {
                    ...
                    errStr << "ERROR: Antenna Prefix file \"" << filename << "\" line " << linenum << " invalid type: " << strval << std::endl;
                    throw std::runtime_error(errStr.str());
                }
                /**************************************************************************/
                ...
                strval = fieldList.at(categoryFieldIdx);
                if (strval.empty()) {
                    errStr << "ERROR: Antenna Prefix file \"" << filename << "\" line " << linenum << " missing category" << std::endl;
                    throw std::runtime_error(errStr.str());
                }
                ...
                if (strval == "HP") {
                    ...
                    errStr << "ERROR: Antenna Prefix file \"" << filename << "\" line " << linenum << " invalid category: " << strval << std::endl;
                    throw std::runtime_error(errStr.str());
                }
                /**************************************************************************/
    ...
    while (fgetline(fp, line, false)) {
        ...
        switch (lineType) {
            case labelLineType:
                ...
                for (fIdx = 0; fIdx < (int)fieldIdxList.size(); fIdx++) {
                    if (*fieldIdxList.at(fIdx) == -1) {
                        errStr << "ERROR: Invalid Antenna Model Map file \"" << filename << "\" label line missing \"" << fieldLabelList.at(fIdx)
                               << "\"" << std::endl;
                        throw std::runtime_error(errStr.str());
                    }
                }
...
/******************************************************************************************/
AntennaModelClass *AntennaModelMapClass::find(
    std::string antPfx, std::string modelName, AntennaModel::CategoryEnum &category, AntennaModel::CategoryEnum modelNameBlankCategory)
{
    bool found = false;
    int antIdx;
    int i;
    ...
    if (found) {
        ...
        for (i = 0; (i < (int)antennaModelList.size()) && (!found); ++i) {
            ...
            if ((!forceModelNameEqualFlag) && (m->type == AntennaModel::AntennaType) &&
                ((m->category == AntennaModel::HPCategory) || (m->category == AntennaModel::B1Category))) {
                prefixFlag = true;
            } else {
                prefixFlag = false;
            }

            if (((prefixFlag) && (modelName.compare(0, m->name.size(), m->name) == 0)) || (m->name == modelName)) {
                found = true;
                antennaModel = m;
            }
        }
