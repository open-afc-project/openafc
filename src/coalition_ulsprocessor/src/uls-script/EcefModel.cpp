...
Vector3 EcefModel::geodeticToEcef(double lat, double lon, double alt)
{
    ...
    const double esq = MathConstants::WGS84EarthFirstEccentricitySquared; // 6.694379901e-3; // First
    ...
    return Vector3((a / chi + alt) * cosLat * cosLon, (a / chi + alt) * cosLat * sinLon, (a * (1 - esq) / chi + alt) * sinLat);
}
