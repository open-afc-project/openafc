...
/**************************************************************************/
UlsFileReader::UlsFileReader(const char *fpath, FILE *fwarn, bool alignFederatedFlag, double alignFederatedScale)
{
...
/**************************************************************************/
void UlsFileReader::readIndividualLocationUS(const std::vector<std::string> &fieldList, bool alignFederatedFlag, double alignFederatedScale)
{
    UlsLocation current;
    ...
    if (alignFederatedFlag) {
        current.longitude = std::floor(current.longitude * alignFederatedScale + 0.5) / alignFederatedScale;
        current.latitude = std::floor(current.latitude * alignFederatedScale + 0.5) / alignFederatedScale;
    }
...
void UlsFileReader::readStationDataCA(const std::vector<std::string> &fieldList, FILE *fwarn, bool alignFederatedFlag, double alignFederatedScale)
{
    ...
    if (alignFederatedFlag) {
        current.longitudeDeg = std::floor(current.longitudeDeg * alignFederatedScale + 0.5) / alignFederatedScale;
...
/**** computeStatisticsUS                                                              ****/
/******************************************************************************************/
int UlsFileReader::computeStatisticsUS(
    FreqAssignmentClass &freqAssignment, bool includeUnii5, bool includeUnii6, bool includeUnii7, bool includeUnii8)
{
    int n = 0;
    int maxNumSegment;
    ...
    foreach(const UlsFrequency &freq, frequencies())
    {
        ...
        {
            if (strcmp(e.callsign, freq.callsign) == 0 && e.locationId == freq.locationNumber && e.antennaId == freq.antennaNumber &&
        ...
        foreach(const UlsEmission &e, allTxEm)
        {
            ...
            if (!invalidFlag) {
                ...
                if (!((includeUnii5 && overlapUnii5) || (includeUnii6 && overlapUnii6) || (includeUnii7 && overlapUnii7) ||
