...
void processUS(UlsFileReader &r, int maxNumPassiveRepeater, CsvWriter &wt, CsvWriter &anomalous, FILE *fwarn, AntennaModelMapClass &antennaModelMap,
    FreqAssignmentClass &freqAssignment, TransmitterModelMapClass &transmitterModelMap);
...
void makeLink(const StationDataCAClass &station, const QList<PassiveRepeaterCAClass> &prList, std::vector<int> &idxList, double &azimuthPtg,
    double &elevationPtg);
...
int main(int argc, char **argv)
{
    ...
    if (argc != 11) {
        fprintf(stderr,
            "Syntax: %s [ULS file.csv] [Output FS File.csv] [Output RAS File.csv] "
            "[AntModelListFile.csv] [AntPrefixFile.csv] [AntModelMapFile.csv] "
            "[freqAssignmentFile] [transmitterModelListFile] [uniiStr] [mode]\n",
            argv[0]);
        return -1;
    }
    ...
    FreqAssignmentClass &freqAssignment, TransmitterModelMapClass &transmitterModelMap)
{
    ...
    foreach(const UlsFrequency &freq, r.frequencies())
    {
        ...
        foreach(const UlsPath &p, r.pathsMap(freq.callsign))
        {
            if (strcmp(p.callsign, freq.callsign) == 0) {
                if ((freq.locationNumber == p.txLocationNumber) && (freq.antennaNumber == p.txAntennaNumber)) {
                    pathList << p;
                }
            }
        }
        ...
        foreach(const UlsPath &path, pathList)
        {
            ...
            foreach(const UlsAntenna &ant, r.antennasMap(path.callsign))
            {
                if (strcmp(ant.callsign, path.callsign) == 0) {
                    ...
                    // locationClass 'T'
                    if ((ant.locationNumber == txLoc.locationNumber) && (ant.antennaNumber == path.txAntennaNumber) &&
            ...
            foreach(const UlsAntenna &ant, r.antennasMap(path.callsign))
            {
                if (strcmp(ant.callsign, path.callsign) == 0) {
                    // Implicitly matches with an Antenna record with
                    // locationClass 'R'
                    if ((ant.locationNumber == rxLoc.locationNumber) && (ant.antennaNumber == path.rxAntennaNumber) &&
                        (ant.pathNumber == path.pathNumber)) {
                        rxAnt = ant;
                        rxAntFound = true;
                        break;
            ...
            for (int segIdx = 0; segIdx < segList.size(); ++segIdx) {
                ...
                if (segIdx) {
                    ...
                    foreach(const UlsAntenna &ant, r.antennasMap(path.callsign))
                    {
                        if ((ant.antennaType == 'P') && (ant.locationNumber == prLocList.last().locationNumber) &&
                            (ant.pathNumber == path.pathNumber)) {
                            prAntList << ant;
                            found = true;
                            break;
            ...
            foreach(const UlsEmission &e, r.emissionsMap(path.callsign))
            {
                if (strcmp(e.callsign, path.callsign) == 0) {
                    if (e.locationId == txLoc.locationNumber && e.antennaId == txAnt.antennaNumber && e.frequencyId == txFreq.frequencyNumber) {
                        allTxEm << e;
                        txEmFound = true;
                    }
                }
            ...
            if (!txHeaderFound) {
                // qDebug() << "Unable to locate header data for" << path.callsign
                // << "path"
                //  << path.pathNumber;
                continue;
            } else if (txHeader.licenseStatus != 'A' && txHeader.licenseStatus != 'L') {
                // qDebug() << "Skipping non-Active tx for" << path.callsign <<
            ...
            if (!txEntityFound) {
                // qDebug() << "Unable to locate entity data for" << path.callsign
                // << "path"
                //          << path.pathNumber;
                continue;
            }
            ...
            foreach(const UlsEmission &e, allTxEm)
            {
                ...
                if (isnan(startFreq) || isnan(stopFreq)) {
                    ...
                    if (!((includeUnii5US && overlapUnii5) || (includeUnii6US && overlapUnii6) || (includeUnii7US && overlapUnii7) ||
                ...
                anomalousReason.append(
                    UlsFunctionsClass::hasNecessaryFields(e, path, rxLoc, txLoc, rxAnt, txAnt, txHeader, prLocList, prAntList, removeMobile));
                    ...
                    if (freqUpperBandMissingFlag) {
                        ...
                        if (debugFlag) {
                            std::cout << "Inband link with Frequency "
                                      ...
                                      << path.callsign << "," << txFreq.frequencyAssigned << "," << txFreq.frequencyUpperBand << std::endl;
                        }
                    }
                }
                ...
                for (prIdx = 1; prIdx <= maxNumPassiveRepeater; ++prIdx) {
                    if ((prIdx <= prLocList.size()) && (prIdx <= prAntList.size())) {
                        ...
                        row << UlsFunctionsClass::makeNumber(prLoc.longitude); // Passive Repeater
                                                                               // Lon Coords
                        row << UlsFunctionsClass::makeNumber(prLoc.groundElevation); // Passive Repeater
                                                                                     // Ground Elevation
                        ...
                        row << AntennaModel::typeStr(prAntennaType).c_str(); // Passive Repeater Ant
                                                                             // Type
                        row << AntennaModel::categoryStr(prAntennaCategory).c_str(); // Passive Repeater Ant
                                                                                     // Category
                        row << UlsFunctionsClass::makeNumber(prAnt.backtobackTxGain); // Passive Repeater
                                                                                      // Back-To-Back Tx
                                                                                      // Gain
                        row << UlsFunctionsClass::makeNumber(prAnt.backtobackRxGain); // Passive Repeater
                                                                                      // Back-To-Back Rx
                                                                                      // Gain
                        row << UlsFunctionsClass::makeNumber(prAnt.reflectorHeight); // Passive Repeater
                                                                                     // ULS Reflector
                                                                                     // Height
                        row << UlsFunctionsClass::makeNumber(prAnt.reflectorWidth); // Passive Repeater
                                                                                    // ULS Reflector
                                                                                    // Width
                        row << UlsFunctionsClass::makeNumber(prAntennaDiameter); // Passive Repeater Ant
                                                                                 // Model Diameter (m)
                        row << UlsFunctionsClass::makeNumber(prAntennaMidbandGain); // Passive Repeater
                                                                                    // Ant Model Midband
                                                                                    // Gain (dB)
                        row << UlsFunctionsClass::makeNumber(prAntennaReflectorHeight); // Passive
                                                                                        ...
                                                                                        // Reflector
                                                                                        // Height
                        row << UlsFunctionsClass::makeNumber(prAntennaReflectorWidth); // Passive
                                                                                       // Repeater Ant
                                                                                       // Model Reflector
                                                                                       // Width
                        row << UlsFunctionsClass::makeNumber(prAnt.lineLoss); // Passive Repeater
                                                                              // Line Loss
                        row << UlsFunctionsClass::makeNumber(prAnt.heightToCenterRAAT); // Passive
                                                                                        // Repeater
                                                                                        // Height to
                                                                                        // Center RAAT Tx
                        row << UlsFunctionsClass::makeNumber(prAnt.heightToCenterRAAT); // Passive
    ...
    std::cout << "US Processed " << r.frequencies().count() << " frequency records and output to file; a total of " << numRecs << " output" << '\n';
}
/******************************************************************************************/
...
/******************************************************************************************/
void processCA(
    UlsFileReader &r, int maxNumPassiveRepeater, CsvWriter &wt, CsvWriter &anomalous, FILE * /* fwarn */, AntennaModelMapClass &antennaModelMap)
{
    int prIdx;
        ...
        foreach(const StationDataCAClass &station, r.stationsMap(authorizationNumber.c_str()))
        {
            ...
            for (int prCount = 1; prCount <= maxNumPassiveRepeater; ++prCount) {
                if (prCount <= numPR) {
                    ...
                    row << UlsFunctionsClass::makeNumber(repFlag ? pr.antennaGainA : pr.antennaGainB); // Passive Repeater
                                                                                                       // Back-To-Back Tx Gain
                    row << UlsFunctionsClass::makeNumber(repFlag ? pr.antennaGainB : pr.antennaGainA); // Passive Repeater
                                                                                                       // Back-To-Back Rx Gain
                    row << UlsFunctionsClass::makeNumber(pr.reflectorHeight); // Passive Repeater ULS
                                                                              // Reflector Height
                    row << UlsFunctionsClass::makeNumber(pr.reflectorWidth); // Passive Repeater ULS
                                                                             // Reflector Width
                    row << UlsFunctionsClass::makeNumber(prAntennaDiameter); // Passive Repeater Ant
                                                                             // Model Diameter (m)
                    row << UlsFunctionsClass::makeNumber(prAntennaMidbandGain); // Passive Repeater Ant Model
                                                                                // Midband Gain (dB)
                    row << UlsFunctionsClass::makeNumber(prAntennaReflectorHeight); // Passive Repeater Ant
                                                                                    // Model Reflector Height
                    row << UlsFunctionsClass::makeNumber(prAntennaReflectorWidth); // Passive Repeater Ant
                    ...
                    row << UlsFunctionsClass::makeNumber(repFlag ? pr.heightAGLA : pr.heightAGLB); // Passive Repeater Height
                                                                                                   // to Center RAAT Tx
                    row << UlsFunctionsClass::makeNumber(repFlag ? pr.heightAGLB : pr.heightAGLA); // Passive Repeater Height
                                                                                                   // to Center RAAT Rx
...
void testAntennaModelMap(AntennaModelMapClass &antennaModelMap, std::string inputFile, std::string outputFile)
{
    ...
    fieldIdxList.push_back(&antennaModelFieldIdx);
    fieldLabelList.push_back("antennaModel");
            ...
            case labelLineType:
                ...
                for (fIdx = 0; fIdx < (int)fieldIdxList.size(); fIdx++) {
                    if (*fieldIdxList.at(fIdx) == -1) {
                        errStr << "ERROR: Invalid input file \"" << inputFile << "\" label line missing \"" << fieldLabelList.at(fIdx) << "\""
                               << std::endl;
                        throw std::runtime_error(errStr.str());
                    }
                }
...
void testTransmitterModelMap(TransmitterModelMapClass &transmitterModelMap, std::string inputFile, std::string outputFile)
{
    ...
    std::vector<int *> fieldIdxList;
    std::vector<std::string> fieldLabelList;
    fieldIdxList.push_back(&transmitterModelFieldIdx);
    fieldLabelList.push_back("transmitterModel");
