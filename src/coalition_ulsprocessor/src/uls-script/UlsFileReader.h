...
class UlsFileReader
{
	public:
		...
		const QList<UlsEmission> &emissions()
		{
			return allEmissions;
		}
		const QList<UlsAntenna> &antennas()
		{
			return allAntennas;
		}
		const QList<UlsFrequency> &frequencies()
		{
			return allFrequencies;
		}
		const QList<UlsLocation> &locations()
		{
			return allLocations;
		}
		const QList<UlsHeader> &headers()
		{
			return allHeaders;
		}
		...
		const QList<UlsEntity> &entities()
		{
			return allEntities;
		}
		const QList<UlsControlPoint> &controlPoints()
		{
			return allControlPoints;
		}
		const QList<UlsSegment> &segments()
		{
			return allSegments;
		}

		const QList<StationDataCAClass> &stations()
		{
			return allStations;
		}
		const QList<BackToBackPassiveRepeaterCAClass> &backToBackPassiveRepeaters()
		{
			return allBackToBackPassiveRepeaters;
		}
		const QList<ReflectorPassiveRepeaterCAClass> &reflectorPassiveRepeaters()
		{
			return allReflectorPassiveRepeaters;
		}
		const QList<TransmitterCAClass> &transmitters()
		{
		...
		const QList<BackToBackPassiveRepeaterCAClass> backToBackPassiveRepeatersMap(
			const QString &s) const
		{
			return backToBackPassiveRepeaterMap[s];
		}

		const QList<ReflectorPassiveRepeaterCAClass> reflectorPassiveRepeatersMap(
			const QString &s) const
		{
