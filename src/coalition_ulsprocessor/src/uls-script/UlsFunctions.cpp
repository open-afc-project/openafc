...
/**************************************************************************/
QString UlsFunctionsClass::hasNecessaryFields(const UlsEmission &e, UlsPath path, UlsLocation rxLoc, UlsLocation txLoc, UlsAntenna rxAnt,
    UlsAntenna txAnt, UlsHeader txHeader, QList<UlsLocation> prLocList, QList<UlsAntenna> prAntList, bool removeMobile)
{
    QString failReason = "";
    ...
    if ((failReason == "") && (fabs(txLoc.longitude - rxLoc.longitude) <= 1.0e-5) && (fabs(txLoc.latitude - rxLoc.latitude) <= 1.0e-5)) {
