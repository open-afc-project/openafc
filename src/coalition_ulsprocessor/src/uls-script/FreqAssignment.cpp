...
void FreqAssignmentClass::readFreqAssignment(const std::string filename)
{
	...
	fieldIdxList.push_back(&frequencyFieldIdx);            fieldLabelList.push_back("channelFrequency");
	fieldIdxList.push_back(&bandwidthFieldIdx);            fieldLabelList.push_back("channelBandwidth");
	...
	if ( !(fp = fopen(filename.c_str(), "rb")) ) {
		str = std::string("ERROR: Unable to open Frequency Assignment File \"") + filename + std::string("\"\n");
	...
	while (fgetline(fp, line, false)) {
		...
		switch(lineType) {
			case   labelLineType:
				...
				for(fIdx=0; fIdx < (int) fieldIdxList.size(); fIdx++) {
					if (*fieldIdxList.at(fIdx) == -1) {
						errStr << "ERROR: Invalid Frequency Assignment file \"" << filename << "\" label line missing \"" << fieldLabelList.at(fIdx) << "\"" << std::endl;
			...
			case    dataLineType:
				...
				if (strval.empty()) {
					errStr << "ERROR: Frequency Assignment file \"" << filename << "\" line " << linenum << " missing frequency" << std::endl;
					throw std::runtime_error(errStr.str());
                } else {
                    frequency = std::strtod(strval.c_str(), &chptr);
                    if (frequency <= 0.0) {
					    errStr << "ERROR: Frequency Assignment file \"" << filename << "\" line " << linenum << " invalid frequency: \"" << strval << "\"" << std::endl;
					    throw std::runtime_error(errStr.str());
                    }
                }
				/**************************************************************************/
				...
				if (strval.empty()) {
					errStr << "ERROR: Frequency Assignment file \"" << filename << "\" line " << linenum << " missing bandwidth" << std::endl;
					throw std::runtime_error(errStr.str());
                } else {
                    bandwidth = std::strtod(strval.c_str(), &chptr);
                    if (bandwidth <= 0.0) {
					    errStr << "ERROR: Frequency Assignment file \"" << filename << "\" line " << linenum << " invalid bandwidth: \"" << strval << "\"" << std::endl;
					    throw std::runtime_error(errStr.str());
                    }
                }
				/**************************************************************************/
