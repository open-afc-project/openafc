...
function(add_dist_executable)
    ...
    if(TARGET Threads::Threads)
        target_link_libraries(${ADD_DIST_BIN_TARGET} Threads::Threads)
    endif()
...
#  SOURCES: All of the dependencies of the target, python files or otherwise.
#
...
# CMAKE_CURRENT_SOURCE_DIR_ESCAPED.
#
function(add_dist_pythonlibrary)
    set(PARSE_OPTS )
    set(PARSE_ARGS_SINGLE TARGET SETUP_TEMPLATE COMPONENT)
    set(PARSE_ARGS_MULTI SOURCES)
    cmake_parse_arguments(ADD_DIST_LIB "${PARSE_OPTS}" "${PARSE_ARGS_SINGLE}" "${PARSE_ARGS_MULTI}" ${ARGN})
    if("${ADD_DIST_LIB_TARGET}" STREQUAL "")
    ...
    if("${ADD_DIST_LIB_SOURCES}" STREQUAL "")
        message(FATAL_ERROR "add_dist_pythonlibrary missing SOURCES parameter")
    endif()
    ...
    if(WIN32)
        ...
        set(CMAKE_CURRENT_SOURCE_DIR_ESCAPED ${CMAKE_CURRENT_SOURCE_DIR})
    endif(WIN32)
    ...
    add_custom_command(
        ...
        COMMAND ${PYTHON} "${CMAKE_CURRENT_BINARY_DIR}/setup.py" build --quiet
    ...
    set(ADD_DIST_LIB_INSTALL_CMD "${PYTHON} \"${CMAKE_CURRENT_BINARY_DIR}/setup.py\" install --root=\$DESTDIR/${CMAKE_INSTALL_PREFIX} --prefix=")
...
function(add_nosetest_run TEST_NAME)
    set(PARSE_OPTS )
    set(PARSE_ARGS_SINGLE PYTHONPATH)
    set(PARSE_ARGS_MULTI )
    cmake_parse_arguments(ADD_NOSETEST "${PARSE_OPTS}" "${PARSE_ARGS_SINGLE}" "${PARSE_ARGS_MULTI}" ${ARGN})
    ...
    set_property(
        ...
        APPEND PROPERTY
            ENVIRONMENT
                "PYTHONPATH=${ADD_NOSETEST_PYTHONPATH}:${CMAKE_INSTALL_PREFIX}/${PKG_INSTALL_PYTHONSITEDIR}:$ENV{PYTHONPATH}"
                "TEST_SOURCE_DIR=${CMAKE_CURRENT_SOURCE_DIR}"
                "TEST_BINARY_DIR=${CMAKE_CURRENT_BINARY_DIR}"
    )
    if(UNIX)
