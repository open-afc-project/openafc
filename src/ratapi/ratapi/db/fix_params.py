...
def isTypicalReflectorDimension(height, width):
    ...
    for refDim in typicalReflectorDimensions:
        ...
        if (abs(typicalHeight - height) <=
                0.1) and (abs(typicalWidth - width) <= 0.1):
            found = True
            break
    return found
...
def fixParams(inputPath, outputPath, logFile, backwardCompatiblePR):
    ...
    for prIdx in range(maxNumPR):
        prTxGainULSFixStr = 'Passive Repeater ' + \
            str(prIdx + 1) + ' ULS Fixed Back-to-Back Gain Tx (dBi)'
        prRxGainULSFixStr = 'Passive Repeater ' + \
            str(prIdx + 1) + ' ULS Fixed Back-to-Back Gain Rx (dBi)'
        prTxGainStr = 'Passive Repeater ' + \
            str(prIdx + 1) + ' Back-to-Back Gain Tx (dBi)'
        prRxGainStr = 'Passive Repeater ' + \
            str(prIdx + 1) + ' Back-to-Back Gain Rx (dBi)'
        prRxAntDiameterStr = 'Passive Repeater ' + \
            str(prIdx + 1) + ' Rx Ant Diameter (m)'
        prTxAntDiameterStr = 'Passive Repeater ' + \
            str(prIdx + 1) + ' Tx Ant Diameter (m)'

        prWidthULSFixStr = 'Passive Repeater ' + \
            str(prIdx + 1) + ' ULS Fixed Reflector Width (m)'
        prHeightULSFixStr = 'Passive Repeater ' + \
            str(prIdx + 1) + ' ULS Fixed Reflector Height (m)'
        prWidthStr = 'Passive Repeater ' + \
            str(prIdx + 1) + ' Reflector Width (m)'
        prHeightStr = 'Passive Repeater ' + \
            str(prIdx + 1) + ' Reflector Height (m)'
    ...
    with open(outputPath, 'w') as fout:
        ...
        for count, row in enumerate(csvreader):
            ...
            for prIdx in range(numPR):
                prNum = prIdx + 1
                prTxGainULSFixStr = 'Passive Repeater ' + \
                    str(prNum) + ' ULS Fixed Back-to-Back Gain Tx (dBi)'
                prRxGainULSFixStr = 'Passive Repeater ' + \
                    str(prNum) + ' ULS Fixed Back-to-Back Gain Rx (dBi)'
                prTxGainStr = 'Passive Repeater ' + \
                    str(prNum) + ' Back-to-Back Gain Tx (dBi)'
                prRxGainStr = 'Passive Repeater ' + \
                    str(prNum) + ' Back-to-Back Gain Rx (dBi)'
                prRxAntDiameterStr = 'Passive Repeater ' + \
                    str(prNum) + ' Rx Ant Diameter (m)'
                prTxAntDiameterStr = 'Passive Repeater ' + \
                    str(prNum) + ' Tx Ant Diameter (m)'
                prWidthULSFixStr = 'Passive Repeater ' + \
                    str(prNum) + ' ULS Fixed Reflector Width (m)'
                prHeightULSFixStr = 'Passive Repeater ' + \
                    str(prNum) + ' ULS Fixed Reflector Height (m)'
                prWidthStr = 'Passive Repeater ' + \
                    str(prNum) + ' Reflector Width (m)'
                prHeightStr = 'Passive Repeater ' + \
                    str(prNum) + ' Reflector Height (m)'

                rxGainULSStr = 'Passive Repeater ' + \
                    str(prNum) + ' ULS Back-to-Back Gain Rx (dBi)'
                txGainULSStr = 'Passive Repeater ' + \
                    str(prNum) + ' ULS Back-to-Back Gain Tx (dBi)'
                prHeightULSStr = 'Passive Repeater ' + \
                    str(prNum) + ' ULS Reflector Height (m)'
                prWidthULSStr = 'Passive Repeater ' + \
                    str(prNum) + ' ULS Reflector Width (m)'
                prAntModelDiameterStr = 'Passive Repeater ' + \
                    str(prNum) + ' Ant Model Diameter (m)'
            ...
            if region == 'US':
                if (highFreq > unii5StartFreqMHz) and (
                        lowFreq < unii5StopFreqMHz):
                    uniiband = 5
                    ...
                    sys.exit(
                        'ERROR in fix_params.py: freq found not in UNII-5, UNII-6, UNII-7, UNII-8')
        ...
        for keyv in sorted(all_cs):
            ...
            for ri in range(len(csmap[keyv])):
                ...
                for prIdx in range(numPR):
                    ...
                    txGainULSStr = 'Passive Repeater ' + \
                        str(prNum) + ' ULS Back-to-Back Gain Tx (dBi)'
                    rxGainULSFixStr = 'Passive Repeater ' + \
                        str(prNum) + ' ULS Fixed Back-to-Back Gain Rx (dBi)'
                    txGainULSFixStr = 'Passive Repeater ' + \
                        str(prNum) + ' ULS Fixed Back-to-Back Gain Tx (dBi)'
                    prAntModelDiameterStr = 'Passive Repeater ' + \
                        str(prNum) + ' Ant Model Diameter (m)'
                    rxAntDiameterStr = 'Passive Repeater ' + \
                        str(prNum) + ' Rx Ant Diameter (m)'
                    txAntDiameterStr = 'Passive Repeater ' + \
                        str(prNum) + ' Tx Ant Diameter (m)'
                    ...
                    prHeightAntennaStr = 'Passive Repeater ' + \
                        str(prNum) + ' Ant Model Reflector Height (m)'
                    prWidthAntennaStr = 'Passive Repeater ' + \
                        str(prNum) + ' Ant Model Reflector Width (m)'
                    ...
                    if keyv[0] == 'US':
                        ...
                        if (r[prTypeStr] == 'Ref'):
                            # R2-AIP-30 (b)
                            if (r[prHeightULSStr].strip() == '') or (
                                    r[prWidthULSStr].strip() == ''):
                            ...
                            if (r[prHeightAntennaStr].strip() == '') or (
                                ...
                                if (abs(prHeightAnt - prHeightULS) <=
                                        0.1) and (abs(prWidthAnt - prWidthULS) <= 0.1):
                                    prHeight = prHeightULS
                                    prWidth = prWidthULS
                                else:
                                    ulsTypicalFlag = isTypicalReflectorDimension(
                                        prHeightULS, prWidthULS)
                                    antTypicalFlag = isTypicalReflectorDimension(
                                        prHeightAnt, prWidthAnt)

                                    # R2-AIP-29-b-iii-3
                                    # R2-AIP-29-b-iii-5
                                    if (ulsTypicalFlag == antTypicalFlag):
                                        if (prHeightULS * prWidthULS >
                                                prHeightAnt * prWidthAnt):
                                            prHeight = prHeightULS
                                            prWidth = prWidthULS
                                        else:
                ...
                if backwardCompatiblePR:
                    if numPR == 0:
                        ...
                        r["Passive Repeater Lat Coords"] = r['Passive Repeater ' +
                                                             str(numPR) + ' Lat Coords']
                        r["Passive Repeater Long Coords"] = r['Passive Repeater ' +
                                                              str(numPR) + ' Long Coords']
                        r["Passive Repeater Height to Center RAAT (m)"] = r['Passive Repeater ' + str(
                            numPR) + ' Height to Center RAAT (m)']
            ...
            for ri in range(len(csmap[keyv])):
                ...
                if keyv[0] == 'US':
                    if (keyv[2] == 5):
                        Fc_unii = (
                            unii5StartFreqMHz + unii5StopFreqMHz) * 0.5e6
                    elif (keyv[2] == 6):
                        Fc_unii = (
                            unii6StartFreqMHz + unii6StopFreqMHz) * 0.5e6
                    elif (keyv[2] == 7):
                        Fc_unii = (
                            unii7StartFreqMHz + unii7StopFreqMHz) * 0.5e6
                    elif (keyv[2] == 8):
                        Fc_unii = (
                            unii8StartFreqMHz + unii8StopFreqMHz) * 0.5e6
                    else:
                        sys.exit(
                            'ERROR in fix_params.py: freq found not in UNII-5, UNII-6, UNII-7, UNII-8')
                    ...
                    for prNum in range(numPR + 1):
                        ...
                        for direction in range(numDirection):
                            ...
                            if prNum == 0:
                                ...
                                fwdGainStrULS = 'Passive Repeater ' + \
                                    str(prNum) + ' ULS Fixed Back-to-Back Gain ' + \
                                    fwd + ' (dBi)'
                                fwdGainStr = 'Passive Repeater ' + \
                                    str(prNum) + ' Back-to-Back Gain ' + \
                                    fwd + ' (dBi)'
                                retGainStrULS = 'Passive Repeater ' + \
                                    str(numPR + 1 - prNum) + \
                                ...
                                fwdAntDiameterStr = 'Passive Repeater ' + \
                                    str(prNum) + ' ' + fwd + \
                                    ' Ant Diameter (m)'
                                retAntDiameterStr = 'Passive Repeater ' + \
                                    str(numPR + 1 - prNum) + ' ' + \
                                    ret + ' Ant Diameter (m)'
                                fwdHeightStr = 'Passive Repeater ' + \
                                    str(prNum) + \
                                    ' Height to Center RAAT ' + fwd + ' (m)'
                                retHeightStr = 'Passive Repeater ' + \
                                    str(numPR + 1 - prNum) + \
                                    ' Height to Center RAAT ' + ret + ' (m)'
                                fwdAntMidbandGain = 'Passive Repeater ' + \
                                    str(prNum) + ' Ant Model Midband Gain (dB)'
                                retAntMidbandGain = 'Passive Repeater ' + \
                                    str(numPR + 1 - prNum) + \
                                    ' Ant Model Midband Gain (dB)'

                            # R2-AIP-05 (e)
                            if (antType == 'Ant') and (
                                    r[fwdGainStrULS].strip() == ''):
                                if (keyv[2] == 5):
                                    fwdGain = 38.8
                                elif (keyv[2] == 6):
                                    fwdGain = 39.5
                            ...
                            # R2-AIP-05 (a)
                                ...
                                Gtypical = 10 * \
                                    math.log10(
                                        Eta * (math.pi * Fc_unii * Drx / c)**2)
                                if (fwdGainULS >= Gtypical -
                                    ...
                                    if (not foundMatch) and (mi != ri):
                                        ...
                                        if ((abs(rxLon - m_txLon) < 2.78e-4) and (abs(rxLat - m_txLat) < 2.78e-4)
                                            ...
                                            while (prMatch and (
                                                ...
                                                if (abs(prLon - m_prLon) < 2.78e-4) and (
                                                        abs(prLat - m_prLat) < 2.78e-4) and (prType == m_prType):
                                                    prIdx += 1
                                                else:
                                                    prMatch = False
                                            if prMatch:
                    ...
                    if (r[rxAntModelMatchedStr] != ''):
                        ...
                        if ((rxGainULS >= 32.0) and (rxGainULS <= 48.0)):
                            ...
                            if (rxGainULS <= 34.55):
                                ...
                                diameterFt = 15.0
                            # ****************************************************************************#
                            rxNearFieldAntennaDiameter = diameterFt * \
                                12.0 * 2.54 * 0.01     # convert ft to m
                            method = 2
                        else:
                            diameterFt = 6.0
                            rxNearFieldAntennaDiameter = diameterFt * 12.0 * 2.54 * 0.01  # convert ft to m
                    ...
                    if (r[rxGainStrULS] == ''):
                        ...
                        if (method == 1) and (not foundDiameter):
                            ...
                            effDB = rxGainULS + 20.0 * \
                                math.log10(
                                    c / (math.pi * Fc_unii * rxNearFieldAntennaDiameter))

                    rxNearFieldDistLimit = 2 * Fc_unii * \
                        rxNearFieldAntennaDiameter * rxNearFieldAntennaDiameter / c
                    ...
                    for prIdx in range(2 * numPR + 1):
                        ...
                        if prNum == 0:
                            ...
                            fwdGainStr = 'Passive Repeater ' + \
                                ...
                                prDirStr + ' (dBi)'
                            fwdHeightStr = 'Passive Repeater ' + \
                                str(prNum) + ' Height to Center RAAT ' + \
                                prDirStr + ' (m)'
