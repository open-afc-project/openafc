...
def processAntFiles(inputDir, processCA, combineAntennaRegionFlag, antennaListFile, antennaPrefixFile, antennaPatternFile, logFile):
    ...
    for ind, antennaModel in enumerate(standardModelList):
        ...
        if antennaModel in antmap:
            ...
            if (r['Diameter (m)'] != diameter) or (r['Midband Gain (dBi)'] != midbandGain):
                logFile.write('WARNING: Antenna model ' + antennaModel + ' defined multiple times in file '
                    + antennaModelDiamGainCsvFilePath + ' with different parameters\n')
        else:
            row = {}
    ...
    for ind, antennaModel in enumerate(reflectorAntennaModelList):
        ...
        if antennaModel in antmap:
            r = antmap[antennaModel]
            if (r['Reflector Height (m)'] != reflectorHeight) or (r['Reflector Width (m)'] != reflectorWidth):
                logFile.write('WARNING: Antenna model ' + antennaModel + ' defined multiple times in file '
                    + billboardReflectorCsvFilePath + ' with different parameters\n')
        else:
            row = {}
            row['Ant Model'] = antennaModel
            row['Type'] = antType
        ...
        if type == "HH":
            ...
            if antennaModel in antmap:
                ...
                if (antmap[antennaModel]['Diameter (m)'] != diameter):
                    logFile.write('WARNING: Antenna model ' + antennaModel + ' multiple diameters specified ' + diameter + ' and '
                         + antmap[antennaModel]['Diameter (m)'] + '\n')
                    antmap[antennaModel]['Diameter (m)'] = diameter
                if (antmap[antennaModel]['Midband Gain (dBi)'] != antennaGain):
                    logFile.write('WARNING: Antenna model ' + antennaModel + ' multiple gain values specified ' + antennaGain + ' and '
    ...
    for antennaModel in sorted(antpatternRaw.keys()):
        ...
        if (minAOB > 0.0) or (maxAOB < 180.0):
            useFlag = False
            logFile.write('WARNING: Antenna model ' + antennaModel
                + ' gain values do not cover the angle off boresight range [0,180], range is [' + minAOB + ',' + maxAOB + ']\n')
        ########################################################################
        ...
        if useFlag:
            ...
            for i in range(numAOB):
                ...
                if k==0:
                    ...
                    attn = (attn0*(aob1 - aob) + attn1*(aob - aob0)) / (aob1 - aob0)
    ...
    with open(antennaPatternFile, 'w') as fout:
        ...
        for i in range(numAOB):
            ...
            for antennaModel in antModelList:
                row[antennaModel] = str(-1.0*antpatternInterp[antennaModel][i])
            csvwriter.writerow(row)
