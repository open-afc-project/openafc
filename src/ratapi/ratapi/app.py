...
def create_app(config_override=None):
    ...
    if flaskapp.config['OIDC_LOGIN']:
        ...
        def load_user(_id):
            ...
            try:
                ...
                db.session.rollback()
                return None
    elif flaskapp.config['AFC_APP_TYPE'] != 'msghnd':
        ...
        # msghnd must not register any session-establishing interactive
        # routes: the msghnd blueprint gate below promises only the AP-AFC
        # blueprint on the bridge interface, and Flask-Security's
        # sign-in/sign-out views would otherwise expose an unthrottled
        # session-minting surface on msghnd:8000 with no dispatcher
        # mediation. Install only the session-consuming user_loader that
        # RatAfcSec's auth() needs; the interactive login flow is served
        # exclusively by the 'server' app type (rat_server, behind the
        # dispatcher).
        from flask_login import LoginManager
        login_manager = LoginManager()
        login_manager.init_app(flaskapp)
    ...
    # (served by WsgiDAV DispatcherMiddleware, external to Flask routing).
