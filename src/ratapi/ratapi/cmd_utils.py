...
class Command(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.action_name:
            raise RuntimeError(
                'Command {0} must define an "action_name"'.format(self))
        ...
        while unchecked_classes:
            ...
            for cls in add_classes:
                ...
                unchecked_classes = unchecked_classes.union(
                    set(cls.depend_mixins))
        self.logger.debug('Using command mixins: %s', [
                          str(obj) for obj in all_mixin_classes])
...
class CommandSet(object):
    ...
    def add_command(self, cmd):
        ...
        helptext = next(
            (item for item in helpsources if item is not None), None)
...
class RootCommandSet(CommandSet):
    ...
    def parse_and_run(self, argv):
        ...
        try:
            ...
            LOGGER.error('Failed to run: ({0}) {1}'.format(
...
class RdbMixin(CmdMixin):
    ...
    def _db_contextmgr(self, args):
        ...
        try:
            ...
            raise RuntimeError('Unable to find config parameter "{0}"'.format(
                self.config_file_element))
