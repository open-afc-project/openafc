...
def upgrade():
    ...
    for ap in connection.execute('select id, user_id from access_point'):
        if ap[0]:
            if ap[1]:
                result = connection.execute(sa.text('select org from aaa_user where id = :id limit 1'), {"id": ap[1]})
                for org in result:
                    connection.execute(
                        sa.text("update access_point set org = :org where id = :id"), {"org": org[0], "id": ap[0]})
                    break
            else:
                connection.execute(
                        sa.text("update access_point set org = '' where id = :id"), {"id": ap[0]})
        else:
            break
