...
def auth(ignore_active=False, roles=None, is_user=None, org=None):
    ...
    if "Super" not in user_roles:
        # Fail closed on unassigned tenancy (CWE-639): an empty org is not a
        # tenant.  Without this, cur_org == target_org == "" satisfies the
        # equality test below and collapses every unassigned user (the
        # default state of new OIDC registrations) into one implicit tenant
        # administered by any empty-org Admin.  A non-Super caller with no
        # organization may only act on itself, and only via calls that are
        # not org-scoped (org is None).
        if not cur_org and (is_user != user_id or org is not None):
            if is_user != user_id:
                # Uniform 404 (see below): a cross-tenant target must be
...
class SuperCheckAPI(MethodView):
    ...
    Used by Nginx auth_request for monitoring locations that expose
    elevated system metrics (Grafana, Prometheus, cAdvisor, RabbitMQ, Kafka UI,
    Alloy)."""
