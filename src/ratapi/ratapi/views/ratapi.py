...
class ReloadAnalysis(MethodView):
    ...
    def _open(self, rel_path, mode, username, user=None):
        ...
        '''
        # username input parameter: it
        # is used as a glob/path component below, so validate separators,
        # '..' and glob metacharacters outright
        if not isinstance(username, str) or not username or \
                '..' in username or \
                any(c in username
                    for c in ('/', '\\', '*', '?', '[', ']')):
            raise werkzeug.exceptions.Forbidden(
                'Invalid username for history lookup')
        ...
        # Defence in depth: everything created or opened below must resolve
        # inside HISTORY_DIR (mirrors the C++ SearchPaths::forReading
        # containment guard)
        _history_real = os.path.realpath(
            flask.current_app.config['HISTORY_DIR'])
        if not os.path.realpath(os.path.join(
                flask.current_app.config['HISTORY_DIR'],
                fileName)).startswith(_history_real + os.sep):
            raise werkzeug.exceptions.Forbidden(
                'History path escapes HISTORY_DIR')
...
class AfcConfigFile(MethodView):
    ...
    def put(self, filename):
        ...
        if len(raw_bytes) > 10 * 1024 * 1024:
            raise werkzeug.exceptions.RequestEntityTooLarge()
        try:
            rcrd = json.loads(raw_bytes)
            if not isinstance(rcrd, dict) or 'regionStr' not in rcrd:
                raise werkzeug.exceptions.BadRequest("Invalid AFC config JSON payload")
        except (json.JSONDecodeError, TypeError) as ex:
            raise werkzeug.exceptions.BadRequest(f"Malformed JSON payload: {ex}")
...
class About(MethodView):
    ...
    def post(self):
        ...
        now = _time.time()
        with _rate_lock:
            # Purge expired entries if dict grows large to prevent unbounded memory growth
            if len(_rate_counter) > 1000:
                expired = [k for k, (_, w_start) in _rate_counter.items() if now - w_start > 3600]
                for k in expired:
                    _rate_counter.pop(k, None)
                if len(_rate_counter) > 5000:
                    _rate_counter.clear()
        ...
        try:
            ...
            if flask.request.content_length and flask.request.content_length > 1 * 1024 * 1024:
                raise werkzeug.exceptions.RequestEntityTooLarge()
            raw_bytes = flask.request.stream.read(1 * 1024 * 1024 + 1)
            if len(raw_bytes) > 1 * 1024 * 1024:
                raise werkzeug.exceptions.RequestEntityTooLarge()
            try:
                rcrd = json.loads(raw_bytes)
                if not isinstance(rcrd, dict):
                    raise werkzeug.exceptions.BadRequest("Invalid JSON object payload")
                name = re.sub(r'[\r\n]', ' ', str(rcrd.get('name', '')))
                email = re.sub(r'[\r\n]', ' ', str(rcrd.get('email', '')))
                org = re.sub(r'[\r\n]', ' ', str(rcrd.get('org', '')))
...
class Phase1Analysis(MethodView):
    ...
    def post(self, request_type):
        ...
        if flask.request.content_length and flask.request.content_length > 10 * 1024 * 1024:
            raise werkzeug.exceptions.RequestEntityTooLarge()
        raw_bytes = flask.request.get_data(max_for_len=10 * 1024 * 1024 + 1)
        if len(raw_bytes) > 10 * 1024 * 1024:
            raise werkzeug.exceptions.RequestEntityTooLarge()
        args = raw_bytes
...
# buffering into gunicorn worker memory (fst.DataIntHttp.read max_bytes).
    ...
    def get(self, task_id):
        ...
        try:
            with dataif.open(
                    ...
                    as hfile:
                kml_data = hfile.read(max_bytes=MAX_RESPONSE_BODY_BYTES)
        except Exception:
            raise werkzeug.exceptions.NotFound(
    ...
    def get(self, task_id):
            ...
            try:
                with dataif.open(
                    ...
                    error_data = hfile.read(
                        max_bytes=MAX_ERROR_TXT_BYTES).decode(
        ...
        if status == afctask.Task.STAT_SUCCESS:
            ...
            with dataif.open(
                ...
                resp.data = hfile.read(max_bytes=MAX_RESPONSE_BODY_BYTES)
            resp.content_type = 'application/json'
            resp.content_encoding = "gzip"
            return resp
...
class UlsParse(MethodView):
    ...
    def put(self):
        ...
        try:
            ...
            if not isinstance(args, dict) or "hours" not in args or "mins" not in args:
                raise werkzeug.exceptions.BadRequest("hours and mins required in JSON payload")
            hours = args["hours"]
            mins = args["mins"]
        except (json.JSONDecodeError, TypeError, KeyError) as ex:
            raise werkzeug.exceptions.BadRequest(f"Malformed JSON payload: {ex}")
...
def _derive_objst_bearer_token(key):
    """ Purpose-bound objst/hist bearer token:
    HMAC-SHA256(key, 'afc-objst-bearer-v1') - see fstorage fst.py; the raw
    provisioned secret must never transit the wire because the RMQ
    response-signing key is derived from the same secret.
    """
...
def _load_objst_api_key():
    """ Return the objst/hist bearer token (derived from
...
class History(MethodView):
    def get(self, path=None):
        ...
        try:
            ...
            raise
        if path is not None and not re.fullmatch(r'[A-Za-z0-9._/\-]+', path):
            # Conservative charset guard: control chars / whitespace / markup
            # in the path would otherwise be echoed back by requests/urllib3
            # exception messages.
            raise werkzeug.exceptions.BadRequest("invalid history path")
        ...
        try:
            ...
            for chunk in response.iter_content(chunk_size=65536):
                ...
                if len(body_bytes) > MAX_RESPONSE_BODY_BYTES:
                    LOGGER.warning("History response body exceeded max bytes (%s)", MAX_RESPONSE_BODY_BYTES)
                    return flask.Response("History artifact exceeds maximum allowed size.", status=413, mimetype='text/plain')
            ...
            if response.headers.get('Content-Type', '').startswith("application/octet-stream") \
                ...
                resp = flask.make_response(body, response.status_code)
                # flask.make_response defaults Content-Type to text/html —
                # re-assert the upstream non-executable type and download
                # disposition so stored-file bytes are never rendered
                # inline in the /fbrat origin.
                resp.headers['Content-Type'] = 'application/octet-stream'
                resp.headers['Content-Disposition'] = response.headers.get(
                    'Content-Disposition', 'attachment')
                resp.headers['X-Content-Type-Options'] = 'nosniff'
                return resp
            else:
                return flask.Response(body, status=response.status_code,
                                      content_type=response.headers.get('Content-Type'))
        except Exception as exc:
            LOGGER.error(
                f"History request failed: {type(exc).__name__}: {exc}\n"
                f"{traceback.format_exc()}")
            # Constant message with explicit text/plain: exception text must
            # never be interpolated into an HTML-typed body (the exception is
            # already logged server-side above).
            return flask.Response("Unreachable history host.", status=502,
                                  mimetype='text/plain')
