...
#: Conservative RFC-5322-subset e-mail shape. user.email doubles as a
#: filesystem/glob key (ReloadAnalysis._open), so beyond format hygiene this
#: must exclude path separators, '..', whitespace and glob metacharacters.
...
def _validate_email_format(email):
    """Reject e-mail strings that are unsafe as identity or filesystem keys.

    Raises BadRequest unless *email* matches a conservative RFC-5322 subset
    (no '/', '\\', '..', whitespace or glob metacharacters).
    """
    if not isinstance(email, str) or len(email) > 254 or \
            ".." in email or not _EMAIL_RE.match(email):
        raise exceptions.BadRequest("Invalid email address format.")
...
def _apply_self_setprops(user, content):
    ...
    if not oidc and "email" in content:
        ...
        if new_email != user.email:
            _validate_email_format(new_email)
...
class User(MethodView):
    ...
    def get(self, user_id):
        ...
        if user_id == 0:
            ...
            if "Super" in roles:
                ...
                users = aaa.User.query.filter_by(org=cur_user.org).all()
            else:
                users = [cur_user]
    ...
    def post(self, user_id):
        ...
        content = flask.request.get_json(silent=True) or {}
        if "setProps" not in content or current_user.id != user_id:
            auth(roles=["Admin"])

        user = aaa.User.query.filter_by(id=user_id).first()
        if user is None:
            # Bare NotFound(): byte-identical to the uniform 404 auth()
            # raises for cross-org targets (see User.delete).
        ...
        if "setProps" in content:
            ...
            if is_self:
                ...
                if changing_email:
                    _validate_email_format(user_props["email"])
            ...
            # Tenancy gate first, with is_user so auth() returns a uniform
            # 404 for cross-org targets (the 403 from the org-only call
            # shape was a user-ID existence oracle across organizations).
            # The Super escalation check runs only after the caller is
            # entitled to know the target exists. Super callers bypass the
            # org scope inside auth(), matching the previous
            # auth(roles=["Super"], org=org) behaviour.
            auth(roles=["Admin"], org=org, is_user=user_id)
            if role == "Super":
                auth(roles=["Super"])
            if "Super" in [r.name for r in user.roles]:
                auth(roles=["Super"])
            ...
            # Tenancy gate first, with is_user (uniform 404 for cross-org
            # targets — see addRole above); Super escalation follows only
            # once the caller is entitled to know the target exists.
            auth(roles=["Admin"], org=org, is_user=user_id)
            if role == "Super":
    ...
    def delete(self, user_id):
        ...
        user = aaa.User.query.filter_by(id=user_id).first()
        if user is None:
            # Bare NotFound(): the response must be byte-identical to the
            # uniform 404 auth() raises for cross-org targets below, or the
            # body text re-opens the existence oracle the status-code fix
            # closes.
            raise exceptions.NotFound()
        org = user.org if user.org else ""
        # Pass is_user so auth() takes its uniform-404 branch: a cross-org
        # target must be indistinguishable from a nonexistent one (the 403
        # raised by the org-only call shape was a user-ID existence oracle
        # across organizations — see the uniform-404 comment in auth()).
        auth(roles=["Admin"], org=org, is_user=user_id)
...
class AccessPointDeny(MethodView):
        ...
        if "Super" in roles:
            ...
            if user.org:
                organization = (
                    ...
                    .filter(aaa.Organization.name == user.org)
                    .first()
                )
                if organization:
    ...
    def put(self, id):
        ...
        # Super-only: deny entries require global administrative
        ...
        # Deny enforcement compares byte-exact against the canonical CertId
        # form; an entry matching no cert_id row can never deny a grantable
        # request, so fail loudly instead of storing an inert revocation.
        if not aaa.CertId.query.filter_by(certification_id=cert_id).first():
            raise exceptions.BadRequest(
                "Certification Id does not match any known certification")
    ...
    def post(self, id):
        ...
        try:
            ...
            if isinstance(payload, str):
                ...
                raise exceptions.BadRequest("accessPoints field required")
            ruleset_list = rcrd["rulesets"]
            access_points = rcrd["access_points"]["data"]
            rows = list(map(lambda a: a.strip("\r").split(","), access_points))
        except (json.JSONDecodeError, TypeError, KeyError, AttributeError) as ex:
            raise exceptions.BadRequest(
                f"Invalid accessPoints payload format: {ex}"
            )
        ...
        for row in rows:
            ...
            cert_id = row[1].strip()
            # Same loud-fail cross-check as the single-entry PUT path.
            if not aaa.CertId.query.filter_by(
                    certification_id=cert_id).first():
                raise exceptions.BadRequest(
                    "Certification Id {} does not match any known "
                    "certification".format(cert_id))
            try:
...
class DeniedRegion(MethodView):
    ...
    def put(self, regionStr):
        ...
        raw_bytes = flask.request.stream.read(10 * 1024 * 1024 + 1)
    ...
    def _rebuild_cert_bundle(self) -> None:
        ...
        with DataIf().open("certificate/client.bundle.pem") as hfile:
            if len(bundle_data) == 0:
                hfile.delete()
                # Also delete the signed .hmac sidecar: the bundle bytes are
                # public PEM CA material, so a surviving genuine sidecar lets
                # any objstorage writer re-complete the (bundle, .hmac) pair
                # and resurrect the revoked trust anchor after a dispatcher
                # container recreation (acceptor.py's monotonic floor is
                # container-local and resets to -1, leaving only the 90-day
                # BUNDLE_MAX_AGE_NS ceiling). Trust removal must leave no
                # replay-completable signed material behind.
                with DataIf().open(
...
class CreateDb(MethodView):
    ...
    def post(self):
        ...
        if not super_authed:
            ...
            # enumerated in AFC_DB_CREATOR_SERVICE_DSN_* env vars, so an
            # internal-token holder cannot mint arbitrary
            ...
            # Bind placement as well as identity: keying the allowlist on
            # (user, db) alone lets a token holder mint its own principal
            # on ANY server ensure_dsn's creator allowlist knows about
            # (e.g. mint the bulk_postgres service role on the ratdb
            # server, gaining a LOGIN foothold there). Require the
            # caller's host:port to match the operator-declared service
            # DSN for the matched principal. DsnInfo requires host and
            # defaults port, so the tuple comparison is well-defined on
            # both sides.
            if (req_info.host, req_info.port) != \
                    (svc_info.host, svc_info.port):
                flask.abort(
                    403,
                    "Service token may only create the pre-declared "
                    "principal on its declared host:port")
            password = _read_file(
                os.environ.get(
                    self.SERVICE_PASSWORD_FILE_ENV_PREFIX + svc_suffix))
            if not password:
                if svc_info.password_defaulted:
                    # Fail closed on DsnInfo's documented invariant: never
                    # provision a role with a defaulted fallback
                    # password. Forwarding svc_info.password to ensure_dsn
                    # as an explicit argument would reset password_defaulted
                    # in its DsnInfo and bypass its own fail-closed guard.
                    flask.abort(
                        500,
                        "No password provisioned for service principal "
                        f"'{svc_suffix}': set "
                        f"{self.SERVICE_PASSWORD_FILE_ENV_PREFIX}"
                        f"{svc_suffix} to a secret file or embed a "
                        "password in "
                        f"{self.SERVICE_DSN_ENV_PREFIX}{svc_suffix}")
                password = svc_info.password
