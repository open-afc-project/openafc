...
class AccessPointDenyCreate:
    ...
    def _create_ap(self, flaskapp, serial, cert_id, ruleset, org):
        from afcmodels.aaa import AccessPointDeny, CertId, Organization, \
            Ruleset
        ...
        with flaskapp.app_context():
            ...
            # Canonicalize identically to the web write paths (admin.py) and
            # the enforcement path (ratafc.py _auth_ap); a non-canonical deny
            # row can never match and the revocation is silently inert.
            if serial is not None:
                serial = serial.strip().upper()
            cert_id = cert_id.strip()
            if not CertId.query.filter_by(certification_id=cert_id).first():
                raise RuntimeError(
    ...
    def __init__(self, flaskapp=None, serial_id=None,
        ...
        if flaskapp:
            # Pass serial_id through unchanged so None stays a NULL wildcard
            # instead of becoming the literal string 'None'.
            self._create_ap(flaskapp, serial_id, cert_id, ruleset, org)
...
class CertIdSweep:
    ...
    def sweep_canada(self, flaskapp):
        ...
        from afcmodels.aaa import CertId, Ruleset
        import datetime
        # Naive UTC: cert_id_is_stale() ages refreshed_at against utcnow()
