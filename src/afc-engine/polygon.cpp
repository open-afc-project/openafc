	...
	for (int polyIdx = 0; (size_t)polyIdx < polyList.size(); ++polyIdx) {
		...
		for (int segment_idx = 0; segment_idx <= poly->num_segment - 1; segment_idx++) {
			...
			for (int bdy_pt_idx = 0; bdy_pt_idx < numPt; bdy_pt_idx++) {
				combinedPoly->bdy_pt_x[segIdx][bdy_pt_idx] =
					poly->bdy_pt_x[segment_idx][bdy_pt_idx];
				combinedPoly->bdy_pt_y[segIdx][bdy_pt_idx] =
					poly->bdy_pt_y[segment_idx][bdy_pt_idx];
			}
			segIdx++;
		}
	}
