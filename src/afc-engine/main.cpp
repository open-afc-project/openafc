...
{ // Accepts input from command line
    try {
        ...
        try {
            ...
            throw std::runtime_error(ErrStream() << "Failed to import configuration from GUI: " << err.what());
        }
        ...
        try {
            ...
            throw std::runtime_error(ErrStream() << "Failed to import user inputs from GUI: " << err.what());
        }
        ...
        try {
            ...
            LOGGER_INFO(logger) << "Databases initialized in: " << std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count() << " seconds";
        } catch (std::exception &err) {
            throw std::runtime_error(ErrStream() << "Failed to initialize databases: " << err.what());
        }
        /**************************************************************************************/
        ...
        afcManager.compute();
        auto t2 = std::chrono::high_resolution_clock::now();
        LOGGER_INFO(logger) << "Computations completed in: " << std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count() << " seconds";
        /**************************************************************************************/
