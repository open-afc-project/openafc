...
CachedGdalBase::GdalInfo::GdalInfo(
    const GdalDatasetHolder *gdalDataset, int minBands, const boost::optional<std::function<void(GdalTransform *)>> &transformationModifier) :
    ...
    numBands(minBands)
{
    ...
    if (gdalDataset->gdalDataset->GetRasterCount() < minBands) {
        errStr << "ERROR: CachedGdalBase::GdalData::GdalData(): GDAL data file '" << baseName << "' has only "
               << gdalDataset->gdalDataset->GetRasterCount() << " bands, whereas at least " << minBands << "is expected";
...
CachedGdalBase::TileKey::TileKey(int band_, int latOffset_, int lonOffset_, const std::string &baseName_) :
    band(band_), latOffset(latOffset_), lonOffset(lonOffset_), baseName(baseName_)
{
}

CachedGdalBase::TileKey::TileKey() : band(0), latOffset(0), lonOffset(0), baseName()
{
}
...
bool CachedGdalBase::TileKey::operator<(const TileKey &other) const
{
    ...
    if (latOffset != other.latOffset)
        return latOffset < other.latOffset;
    if (lonOffset != other.lonOffset)
        return lonOffset < other.lonOffset;
...
CachedGdalBase::TileInfo::TileInfo(CachedGdalBase *cachedGdal_, const GdalTransform &transformation_, const GdalInfo *gdalInfo_) :
    cachedGdal(cachedGdal_),
    transformation(transformation_),
    boundRect(transformation_.makeBoundRect()),
    gdalInfo(gdalInfo_),
    tileVector(cachedGdal_->createTileVector(transformation_.latSize, transformation_.lonSize), [cachedGdal_](void *p) {
...
CachedGdalBase::GdalDatasetHolder::GdalDatasetHolder(const std::string &fullFileName_) : fullFileName(fullFileName_)
{
    std::ostringstream errStr;
    boost::system::error_code systemErr;
...
CachedGdalBase::PixelInfo::PixelInfo(const std::string &baseName_, int row_, int column_) : baseName(baseName_), row(row_), column(column_)
{
}
...
CachedGdalBase::CachedGdalBase(std::string fileOrDir, const std::string &dsName, std::unique_ptr<GdalNameMapperBase> nameMapper, int numBands,
    ...
    _gdalDsCache(GDAL_CACHE_SIZE),
    _recentGdalInfo(nullptr),
    _allSeen(false)
{
    GDALAllRegister();
}
...
{
    LOGGER_INFO(logger) << "Initializing access to '" << _fileOrDir << (isMonolithic() ? "' GDAL file " : "' GDAL file directory ") << "containing "
                        << (dsName().empty() ? "?some?" : dsName()) << " data. Assumed pixel data type is " << GDALGetDataTypeName(_pixelType)
    ...
    if (isMonolithic()) {
        ...
        if (!forEachGdalInfo([](const GdalInfo &gdalInfo) {
            ...
            errStr << "ERROR: CachedGdalBase::initialize(): "
                      "GDAL data directory '"
                   << _fileOrDir << "' does not contain files matching fnmatch pattern '" << _nameMapper->fnmatchPattern() << "'";
            throw std::runtime_error(errStr.str());
        }
    }
}
...
void CachedGdalBase::setTransformationModifier(std::function<void(GdalTransform *)> modifier)
{
...
bool CachedGdalBase::forEachGdalInfo(const std::function<bool(const CachedGdalBase::GdalInfo &gdalInfo)> &op)
{
    ...
    for (boost::filesystem::directory_iterator di(_fileOrDir); di != boost::filesystem::directory_iterator(); ++di) {
        std::string baseName = di->path().filename().native();
        // Skipping seen, nonmatching and nonfiles
        if ((_gdalInfos.find(baseName) != _gdalInfos.end()) || (fnmatch(fnmatch_pattern.c_str(), baseName.c_str(), 0) == FNM_NOMATCH) ||
...
const void *CachedGdalBase::getTileVector(int band, double latDeg, double lonDeg, int *pixelIndex)
{
    checkBandIndex(band);
...
bool CachedGdalBase::getPixelDirect(int band, double latDeg, double lonDeg, void *pixelBuf)
{
    ...
    if (readError != CPLErr::CE_None) {
        std::ostringstream errStr;
        errStr << "ERROR: CachedGdalBase::getPixelDirect(): Reading GDAL pixel from '" << gdalInfo->baseName << "' (band: " << band
               << ", xOffset: " << fileLonIdx << ", yOffset: " << fileLatIdx << ") failed: " << CPLGetLastErrorMsg();
        throw std::runtime_error(errStr.str());
    }
    return true;
}
...
bool CachedGdalBase::findTile(int band, double latDeg, double lonDeg)
{
    ...
    if (_tileCache.recentValue() && (_tileCache.recentValue()->boundRect.contains(latDeg, lonDeg)) &&
        (_tileCache.recentValue()->gdalInfo->boundRect.contains(latDeg, lonDeg)) && (_tileCache.recentKey()->band == band)) {
        return true;
    }
    ...
    int latTileSize = std::min(_maxTileSize, gdalInfo->transformation.latSize - tileKey.latOffset - intMargin);
    int lonTileSize = std::min(_maxTileSize, gdalInfo->transformation.lonSize - tileKey.lonOffset - intMargin);
    TileInfo tileInfo(this, GdalTransform(gdalInfo->transformation, tileKey.latOffset, tileKey.lonOffset, latTileSize, lonTileSize), gdalInfo);

    // Now reading pixel data into buffer of tile object
    const GdalDatasetHolder *datasetHolder = getGdalDatasetHolder(gdalInfo->baseName);
    CPLErr readError = datasetHolder->gdalDataset->GetRasterBand(tileKey.band)
                           ->RasterIO(GF_Read, tileKey.lonOffset, tileKey.latOffset, lonTileSize, latTileSize,
                               getTileBuffer(tileInfo.tileVector.get()), lonTileSize, latTileSize, _pixelType, 0, 0);
    if (readError != CPLErr::CE_None) {
        std::ostringstream errStr;
        errStr << "ERROR: CachedGdalBase::findTile(): Reading GDAL data from '" << tileKey.baseName << "' (band: " << tileKey.band
               << ", xOffset: " << tileKey.lonOffset << ", yOffset: " << tileKey.latOffset << ", xSize: " << lonTileSize << ", ySize: " << latTileSize
               << ") failed: " << CPLGetLastErrorMsg();
        throw std::runtime_error(errStr.str());
    }
    LOGGER_DEBUG(logger) << "[" << latTileSize << " X " << lonTileSize << "] tile retrieved from (" << tileKey.latOffset << ", " << tileKey.lonOffset
                         << ") of band " << tileKey.band << " of '" << gdalInfo->baseName << "'";
    // Finally adding tile to cache
    _tileCache.add(tileKey, tileInfo);
    return true;
}

bool CachedGdalBase::getGdalPixel(double latDeg, double lonDeg, const GdalInfo **gdalInfo, int *fileLatIdx, int *fileLonIdx)
{
...
const CachedGdalBase::GdalDatasetHolder *CachedGdalBase::getGdalDatasetHolder(const std::string &baseName)
{
    if ((_gdalDsCache.recentKey() && (baseName == *_gdalDsCache.recentKey())) || _gdalDsCache.get(baseName)) {
    ...
    auto ret = _gdalDsCache.add(baseName, std::shared_ptr<GdalDatasetHolder>(new GdalDatasetHolder(filePath.native())))->get();
    return ret;
}

const CachedGdalBase::GdalInfo *CachedGdalBase::addGdalInfo(const std::string &baseName, const GdalDatasetHolder *gdalDatasetHolder)
{
    if (gdalDatasetHolder) {
        auto p = _gdalInfos.emplace(
            baseName, std::move(std::unique_ptr<GdalInfo>(new GdalInfo(gdalDatasetHolder, _numBands, _transformationModifier))));
        _recentGdalInfo = p.first->second.get();
        LOGGER_DEBUG(logger) << "GDAL file '" << gdalDatasetHolder->fullFileName << "' covers area from ["
                             << formatPosition(_recentGdalInfo->boundRect.latDegMin, _recentGdalInfo->boundRect.lonDegMin) << "] (Lower Left) to ["
                             << formatPosition(_recentGdalInfo->boundRect.latDegMax, _recentGdalInfo->boundRect.lonDegMax)
                             << "] (Upper Right). "
                                "Image resolution "
                             << formatDms(_recentGdalInfo->transformation.latPixPerDeg) << " by "
                             << formatDms(_recentGdalInfo->transformation.lonPixPerDeg) << " pixels per degree. Image size is "
                             << _recentGdalInfo->transformation.latSize << " by " << _recentGdalInfo->transformation.lonSize << " pixels";
...
bool CachedGdalBase::getGdalInfo(const std::string &filename, const CachedGdalBase::GdalInfo **gdalInfo)
{
...
void CachedGdalBase::checkBandIndex(int band) const
{
    ...
    std::ostringstream errStr;
    errStr << "ERROR: CachedGdalBase::checkBandIndex(): Invalid band index " << band << ". Should be in [1.." << _numBands << "] range";
    throw std::runtime_error(errStr.str());
}

bool CachedGdalBase::covers(double latDeg, double lonDeg)
{
    return forEachGdalInfo([latDeg, lonDeg](const GdalInfo &gdalInfo) {
...
GdalTransform::BoundRect CachedGdalBase::boundRect()
{
    ...
    if (!isMonolithic()) {
        forEachGdalInfo([&ret](const GdalInfo &gdalInfo) {
            ret.combine(gdalInfo.boundRect);
            return false;
        });
    }
    return ret;
...
boost::optional<CachedGdalBase::PixelInfo> CachedGdalBase::getPixelInfo(double latDeg, double lonDeg)
{
    const GdalInfo *gdalInfo;
    int fileLatIdx, fileLonIdx;
    if (!getGdalPixel(latDeg, lonDeg, &gdalInfo, &fileLatIdx, &fileLonIdx)) {
...
GDALDataType CachedGdalBase::gdalDataType(uint8_t)
{
    return GDT_Byte;
}
GDALDataType CachedGdalBase::gdalDataType(uint16_t)
{
    return GDT_UInt16;
}
GDALDataType CachedGdalBase::gdalDataType(int16_t)
{
    return GDT_Int16;
}
GDALDataType CachedGdalBase::gdalDataType(uint32_t)
{
    return GDT_UInt32;
}
GDALDataType CachedGdalBase::gdalDataType(int32_t)
{
    return GDT_Int32;
}
GDALDataType CachedGdalBase::gdalDataType(float)
{
    return GDT_Float32;
}
GDALDataType CachedGdalBase::gdalDataType(double)
{
    return GDT_Float64;
}
