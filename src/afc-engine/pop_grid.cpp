    ...
    ) : densityThrUrban(densityThrUrbanVal), densityThrSuburban(densityThrSuburbanVal), densityThrRural(densityThrRuralVal)
{
    ...
    // for regions contained in another region, specify interior region first.  For example, if regions are defined
    // for india and asia, specifying india first effectively defines two regions as india and "asia except india".
    ...
    GDALDataset *gdalDataset = static_cast<GDALDataset*>(GDALOpen(worldPopulationFile.c_str(), GA_ReadOnly));
    ...
    if (readError != CPLErr::CE_None) {
        errStr << "ERROR: getting GEO Transform"
               << worldPopulationFile << ", throwing CPLErr = " << readError;
    ...
    if (    (fabs(worldMinLon+180.0)>1.0e-8) || (fabs(worldMaxLon-180.0)>1.0e-8)
        ...
        errStr << "ERROR: world population file: " << worldPopulationFile << " does not cover region LON: -180,180 LAT: -90,90";
        throw std::runtime_error(errStr.str());
    }
    ...
    std::cout << "Analysis region wraps around LON discontinuity at +/- 180 deg: " << (wrapLonFlag ? "YES" : "NO") << std::endl;
    ...
    double areaGridEquator = CConst::earthRadius*CConst::earthRadius*(deltaLonDeg*M_PI/180.0)*(deltaLatDeg*M_PI/180.0);

    for(latIdx=0; latIdx<numLat; latIdx++) {
        if (wrapLonFlag) {
            rasterBand->RasterIO( GF_Read, minLonIdx, nYSize-1-minLatIdx-latIdx, nXSize-minLonIdx, 1, pafScanline, nXSize-minLonIdx, 1, GDT_Float32, 0, 0 );
            rasterBand->RasterIO( GF_Read, 0, nYSize-1-minLatIdx-latIdx, numLon-(nXSize-minLonIdx), 1, pafScanline+nXSize-minLonIdx, numLon-(nXSize-minLonIdx), 1, GDT_Float32, 0, 0 );
        } else {
            rasterBand->RasterIO( GF_Read, minLonIdx, nYSize-1-minLatIdx-latIdx, numLon, 1, pafScanline, numLon, 1, GDT_Float32, 0, 0 );
        }
        double latitudeDeg = (maxLatDeg*(2*latIdx+1) + minLatDeg*(2*numLat-2*latIdx-1))/(2*numLat);
...
// }
            ...
            if (pafScanline[lonIdx] != origNodataValueFloat) {
                double longitudeDeg = (maxLonDeg*(2*lonIdx+1) + minLonDeg*(2*numLon-2*lonIdx-1))/(2*numLon);
// std::cout << longitudeDeg << " " << latitudeDeg << std::endl;
                ...
                for(regionIdx=0; (regionIdx<regionPolygonList.size())&&(!foundRegion); ++regionIdx) {
                    ...
                    if (regionPolygon->in_bdy_area(polygonX, polygonY)) {
                        ...
                        double density = pafScanline[lonIdx]*1.0e-6;  // convert from people/sq-km to people/sqm;
    ...
    if ((totalPop > 0.0)&&(totalArea > 0.0)) {
        for(regionIdx=0; regionIdx<numRegion; regionIdx++) {
            PolygonClass *regionPolygon = regionPolygonList[regionIdx];
            double regionPop = urbanPop[regionIdx] + suburbanPop[regionIdx] + ruralPop[regionIdx] + barrenPop[regionIdx];
            std::cout << "REGION " << regionPolygon->name << " URBAN    POPULATION: " << urbanPop[regionIdx]
                      << " " << 100*urbanPop[regionIdx]/totalPop    << " % (total)"
                      << " " << 100*urbanPop[regionIdx]/regionPop   << " % (region)" << std::endl;
            std::cout << "REGION " << regionPolygon->name << " SUBURBAN POPULATION: " << suburbanPop[regionIdx]
                      << " " << 100*suburbanPop[regionIdx]/totalPop  << " % (total)"
                      << " " << 100*suburbanPop[regionIdx]/regionPop << " % (region)" << std::endl;
            std::cout << "REGION " << regionPolygon->name << " RURAL    POPULATION: " << ruralPop[regionIdx]
                      << " " << 100*ruralPop[regionIdx]/totalPop     << " % (total)"
                      << " " << 100*ruralPop[regionIdx]/regionPop    << " % (region)" << std::endl;
            std::cout << "REGION " << regionPolygon->name << " BARREN   POPULATION: " << barrenPop[regionIdx]
                      << " " << 100*barrenPop[regionIdx]/totalPop    << " % (total)"
                      << " " << 100*barrenPop[regionIdx]/regionPop   << " % (region)" << std::endl;
            std::cout << std::endl;

            double regionArea = urbanArea[regionIdx] + suburbanArea[regionIdx] + ruralArea[regionIdx] + barrenArea[regionIdx] + zeroArea[regionIdx];
            std::cout << "REGION " << regionPolygon->name << " URBAN_NZ    AREA: " << urbanArea[regionIdx]
                      << " " << 100*urbanArea[regionIdx]/totalArea     << " % (total)"
                      << " " << 100*urbanArea[regionIdx]/regionArea    << " % (region)" << std::endl;
            std::cout << "REGION " << regionPolygon->name << " SUBURBAN_NZ AREA: " << suburbanArea[regionIdx]
                      << " " << 100*suburbanArea[regionIdx]/totalArea  << " % (total)"
                      << " " << 100*suburbanArea[regionIdx]/regionArea << " % (region)" << std::endl;
            std::cout << "REGION " << regionPolygon->name << " RURAL_NZ    AREA: " << ruralArea[regionIdx]
                      << " " << 100*ruralArea[regionIdx]/totalArea     << " % (total)"
                      << " " << 100*ruralArea[regionIdx]/regionArea    << " % (region)" << std::endl;
            std::cout << "REGION " << regionPolygon->name << " BARREN_NZ   AREA: " << barrenArea[regionIdx]
                      << " " << 100*barrenArea[regionIdx]/totalArea    << " % (total)"
                      << " " << 100*barrenArea[regionIdx]/regionArea   << " % (region)" << std::endl;
            std::cout << "REGION " << regionPolygon->name << " ZERO-POP    AREA: " << zeroArea[regionIdx]
                      << " " << 100*zeroArea[regionIdx]/totalArea      << " % (total)"
                      << " " << 100*zeroArea[regionIdx]/regionArea     << " % (region)" << std::endl;
            std::cout << std::endl;
        }
    }
    std::cout << std::flush;
...
/**** GET/SET functions                                                                ****/
/******************************************************************************************/
int PopGridClass::getNumLon() { return(numLon); }
int PopGridClass::getNumLat() { return(numLat); }
double PopGridClass::getDensityThrUrban() { return(densityThrUrban); }
double PopGridClass::getDensityThrSuburban() { return(densityThrSuburban); }
double PopGridClass::getDensityThrRural() { return(densityThrRural); }

void PopGridClass::setPop(int lonIdx, int latIdx, double popVal) { pop[lonIdx][latIdx] = popVal; return; }
void PopGridClass::setPropEnv(int lonIdx, int latIdx, char propEnvVal) { propEnv[lonIdx][latIdx] = propEnvVal; return; }

double PopGridClass::getPropEnvPop(CConst::PropEnvEnum propEnvVal, int regionIdx) const
{
    double popVal;
    switch(propEnvVal) {
        case CConst::urbanPropEnv:    popVal = urbanPop[regionIdx];    break;
        case CConst::suburbanPropEnv: popVal = suburbanPop[regionIdx]; break;
        case CConst::ruralPropEnv:    popVal = ruralPop[regionIdx];    break;
        case CConst::barrenPropEnv:   popVal = barrenPop[regionIdx];   break;
        default:
            throw std::runtime_error(ErrStream() << "ERROR in PopGridClass::getPropEnvPop: propEnvVal = " << propEnvVal << " INVALID value");
            break;
    }
...
/******************************************************************************************/
void PopGridClass::readData(std::string filename, const std::vector<std::string> &regionNameListVal, const std::vector<int> &regionIDListVal,
                                                  int numLonVal, double deltaLonDegVal, double minLonDegVal,
                                                  int numLatVal, double deltaLatDegVal, double minLatDegVal)
{
    ...
    for (int r = 0; r < rows.size(); r++)
    {
                ...
                // When we store in db we only check that lat/lon values are valid coordinates, but not that they are on grid
                if (fabs(lonIdxDbl-lonIdx-0.5) > 0.05) {
                    throw std::runtime_error(ErrStream() << "ERROR: Invalid population density data file \"" << filename << "(" << r << ")\" longitude value not on grid, lonIdxDbl = " << lonIdxDbl);
                }

                if (fabs(latIdxDbl-latIdx-0.5) > 0.05) {
                    throw std::runtime_error(ErrStream() << "ERROR: Invalid population density data file \"" << filename << "(" << r << ")\" latitude value not on grid, latIdxDbl = " << latIdxDbl);
                }

                // area of population tile
                area = CConst::earthRadius*CConst::earthRadius*cos(latitudeDeg*M_PI/180.0)*deltaLonDeg*deltaLatDeg*(M_PI/180.0)*(M_PI/180.0);
                ...
                if (density >= densityThrUrban) { // Check density against different thresholds to determine what the population environment is (urban, suburban, etc.)
                ...
                } // Char is used since working with big files, so 8 bit char is better than 32 bit int
    ...
    for(regionIdx=0; regionIdx<numRegion; regionIdx++) {
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " URBAN    POPULATION: " << urbanPop[regionIdx]    << " " << 100*urbanPop[regionIdx]/totalPop    << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " SUBURBAN POPULATION: " << suburbanPop[regionIdx] << " " << 100*suburbanPop[regionIdx]/totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " RURAL    POPULATION: " << ruralPop[regionIdx]    << " " << 100*ruralPop[regionIdx]/totalPop    << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " BARREN   POPULATION: " << barrenPop[regionIdx]   << " " << 100*barrenPop[regionIdx]/totalPop   << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " URBAN    AREA: " << urbanArea[regionIdx]    << " " << 100*urbanArea[regionIdx]/totalArea    << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " SUBURBAN AREA: " << suburbanArea[regionIdx] << " " << 100*suburbanArea[regionIdx]/totalArea << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " RURAL    AREA: " << ruralArea[regionIdx]    << " " << 100*ruralArea[regionIdx]/totalArea    << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " BARREN   AREA: " << barrenArea[regionIdx]   << " " << 100*barrenArea[regionIdx]/totalArea   << " %";
    }
    /**************************************************************************************/
...
/******************************************************************************************/
void PopGridClass::scale(std::vector<double> urbanPopVal, std::vector<double> suburbanPopVal, std::vector<double> ruralPopVal, std::vector<double> barrenPopVal)
{
    int regionIdx;
    ...
    for(regionIdx=0; regionIdx<numRegion; regionIdx++) {
        ...
        scaleBarren[regionIdx]   = ((barrenPopVal[regionIdx] == 0.0) ? 0.0 : barrenPopVal[regionIdx]/barrenPop[regionIdx]);
    ...
    for(regionIdx=0; regionIdx<numRegion; regionIdx++) {
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " RLAN DEVICE URBAN    POPULATION: " << urbanPop[regionIdx]    << " " << 100*urbanPop[regionIdx]/totalPop    << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " RLAN DEVICE SUBURBAN POPULATION: " << suburbanPop[regionIdx] << " " << 100*suburbanPop[regionIdx]/totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " RLAN DEVICE RURAL    POPULATION: " << ruralPop[regionIdx]    << " " << 100*ruralPop[regionIdx]/totalPop    << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " RLAN DEVICE BARREN   POPULATION: " << barrenPop[regionIdx]   << " " << 100*barrenPop[regionIdx]/totalPop   << " %";
        totalScaledPopulation += urbanPop[regionIdx] + suburbanPop[regionIdx] + ruralPop[regionIdx] + barrenPop[regionIdx];
    }
...
void PopGridClass::writeDensity(std::string filename, bool dumpPopGrid)
{
    ...
    if ( !(fp = fopen(filename.c_str(), "wb")) ) {
        throw std::runtime_error(ErrStream() << "ERROR: Unable to write to file \"" << filename << "\"");
    }
    ...
    if (dumpPopGrid) {
        ...
        fprintf(fp, "Longitude (deg),Latitude (deg),Device density (#/sqkm),Propagation Environment\n");
        for(lonIdx=0; lonIdx<numLon; lonIdx++) {
            ...
            for(latIdx=0; latIdx<numLat; latIdx++) {
                if ( (propEnv[lonIdx][latIdx] != 'X') && (propEnv[lonIdx][latIdx] != 'B') ) {
                    ...
                    fprintf(fp, "%.5f,%.5f,%.3f,%c\n", longitudeDeg, latitudeDeg, (pop[lonIdx][latIdx]/area)*1.0e6, propEnv[lonIdx][latIdx]);
                }
            }
        }
    }
...
int PopGridClass::adjustRegion(double centerLongitudeDeg, double centerLatitudeDeg, double radius)
{
    ...
    double maxLonOffset = 2*asin(radius / (2*CConst::earthRadius*cos(centerLatitudeDeg*M_PI/180.0)));
    ...
    if (minLonIdx < 0) { minLonIdx = 0; }
    if (minLatIdx < 0) { minLatIdx = 0; }
    if (maxLonIdx > numLon-1) { maxLonIdx = numLon-1; }
    if (maxLatIdx > numLat-1) { maxLatIdx = numLat-1; }
...
double PopGridClass::adjustRegion(ListClass<ULSClass *> *ulsList, double maxRadius)
{
    ...
    double maxDistGridCenterToEdge = CConst::earthRadius*sqrt((deltaLonDeg*deltaLonDeg + deltaLatDeg*deltaLatDeg)*(M_PI/180.0)*(M_PI/180.0))/2;
    double maxDistKMSQ = (maxRadius + maxDistGridCenterToEdge)*(maxRadius + maxDistGridCenterToEdge)*1.0e-6;
    ...
    for(int ulsIdx=0; ulsIdx < ulsList->getSize(); ulsIdx++) {
        ...
        if (lonIdxStart < 0) { lonIdxStart = 0;}
        int lonIdxStop = ulsLonIdx + offsetLonIdx;
        if (lonIdxStop > numLon-1) { lonIdxStop = numLon-1;}

        int latIdxStart = ulsLatIdx - offsetLatIdx;
        if (latIdxStart < 0) { latIdxStart = 0;}
        int latIdxStop = ulsLatIdx + offsetLatIdx;
        if (latIdxStop > numLat-1) { latIdxStop = numLat-1;}
    ...
    for(lonIdx=0; lonIdx<numLon; lonIdx++) {
        ...
        for(latIdx=0; latIdx<numLat; latIdx++) {
            ...
            if (useFlag) {
                if (initFlag || (lonIdx < minLonIdx)) { minLonIdx = lonIdx; }
                if (initFlag || (lonIdx > maxLonIdx)) { maxLonIdx = lonIdx; }
                if (initFlag || (latIdx < minLatIdx)) { minLatIdx = latIdx; }
                if (initFlag || (latIdx > maxLatIdx)) { maxLatIdx = latIdx; }
                initFlag = false;
            } else {
                pop[lonIdx][latIdx] = 0.0;
                propEnv[lonIdx][latIdx] = 'X';
    ...
    for (lonIdx=0; lonIdx<newNumLon; lonIdx++) {
        ...
        for (latIdx=0; latIdx<newNumLat; latIdx++) {
            double latitudeDeg = minLatDeg + latIdx*deltaLatDeg;
            double area = CConst::earthRadius*CConst::earthRadius*cos(latitudeDeg*M_PI/180.0)*deltaLonDeg*deltaLatDeg*(M_PI/180.0)*(M_PI/180.0);
    ...
    for(regionIdx=0; regionIdx<numRegion; regionIdx++) {
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED URBAN    POPULATION: " << urbanPop[regionIdx]    << " " << 100*urbanPop[regionIdx]/totalPop    << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED SUBURBAN POPULATION: " << suburbanPop[regionIdx] << " " << 100*suburbanPop[regionIdx]/totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED RURAL    POPULATION: " << ruralPop[regionIdx]    << " " << 100*ruralPop[regionIdx]/totalPop    << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED BARREN   POPULATION: " << barrenPop[regionIdx]   << " " << 100*barrenPop[regionIdx]/totalPop   << " %";

        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED URBAN    AREA: " << urbanArea[regionIdx]    << " " << 100*urbanArea[regionIdx]/totalArea    << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED SUBURBAN AREA: " << suburbanArea[regionIdx] << " " << 100*suburbanArea[regionIdx]/totalArea << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED RURAL    AREA: " << ruralArea[regionIdx]    << " " << 100*ruralArea[regionIdx]/totalArea    << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED BARREN   AREA: " << barrenArea[regionIdx]   << " " << 100*barrenArea[regionIdx]/totalArea   << " %";
    }
...
/******************************************************************************************/
void PopGridClass::findDeg(double longitudeDeg, double latitudeDeg, int &lonIdx, int &latIdx, char &propEnvVal, int &regionIdx) const
{
    if (longitudeDeg < minLonDeg) {
        longitudeDeg += 360.0;
    }
