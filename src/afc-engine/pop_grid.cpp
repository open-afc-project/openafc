...
PopGridClass::PopGridClass(std::string worldPopulationFile, const std::vector<PolygonClass *> &regionPolygonList, double regionPolygonResolution,
    double densityThrUrbanVal, double densityThrSuburbanVal, double densityThrRuralVal, double minLat, double minLon, double maxLat, double maxLon) :
    densityThrUrban(densityThrUrbanVal), densityThrSuburban(densityThrSuburbanVal), densityThrRural(densityThrRuralVal)
{
    ...
    if (readError != CPLErr::CE_None) {
        errStr << "ERROR: getting GEO Transform" << worldPopulationFile << ", throwing CPLErr = " << readError;
    ...
    if ((fabs(worldMinLon + 180.0) > 1.0e-8) || (fabs(worldMaxLon - 180.0) > 1.0e-8) || (fabs(worldMinLat + 90.0) > 1.0e-8) ||
        (fabs(worldMaxLat - 90.0) > 1.0e-8)) {
    ...
    printf("Block=%dx%d Type=%s, ColorInterp=%s\n", nBlockXSize, nBlockYSize, GDALGetDataTypeName(rasterBand->GetRasterDataType()),
    ...
    for (latIdx = 0; latIdx < numLat; latIdx++) {
        if (wrapLonFlag) {
            rasterBand->RasterIO(
                GF_Read, minLonIdx, nYSize - 1 - minLatIdx - latIdx, nXSize - minLonIdx, 1, pafScanline, nXSize - minLonIdx, 1, GDT_Float32, 0, 0);
            rasterBand->RasterIO(GF_Read, 0, nYSize - 1 - minLatIdx - latIdx, numLon - (nXSize - minLonIdx), 1, pafScanline + nXSize - minLonIdx,
...
void PopGridClass::readData(std::string filename, const std::vector<std::string> &regionNameListVal, const std::vector<int> &regionIDListVal,
    int numLonVal, double deltaLonDegVal, double minLonDegVal, int numLatVal, double deltaLatDegVal, double minLatDegVal)
{
    ...
    PopulationDatabase::loadPopulationData(QString::fromStdString(filename), rows, minLatDeg, maxLatDeg, minLonDeg,
    ...
    for (int r = 0; r < (int)rows.size(); r++) {
        ...
        if (fabs(lonIdxDbl - lonIdx - 0.5) > 0.05) {
            throw std::runtime_error(ErrStream() << "ERROR: Invalid population density data file \"" << filename << "(" << r
        ...
        area = CConst::earthRadius * CConst::earthRadius * cos(latitudeDeg * M_PI / 180.0) * deltaLonDeg * deltaLatDeg * (M_PI / 180.0) *
            (M_PI / 180.0);
    ...
    for (regionIdx = 0; regionIdx < numRegion; regionIdx++) {
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " URBAN    POPULATION: " << urbanPop[regionIdx] << " "
                            << 100 * urbanPop[regionIdx] / totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " SUBURBAN POPULATION: " << suburbanPop[regionIdx] << " "
                            << 100 * suburbanPop[regionIdx] / totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " RURAL    POPULATION: " << ruralPop[regionIdx] << " "
                            << 100 * ruralPop[regionIdx] / totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " BARREN   POPULATION: " << barrenPop[regionIdx] << " "
                            << 100 * barrenPop[regionIdx] / totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " URBAN    AREA: " << urbanArea[regionIdx] << " "
                            << 100 * urbanArea[regionIdx] / totalArea << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " SUBURBAN AREA: " << suburbanArea[regionIdx] << " "
                            << 100 * suburbanArea[regionIdx] / totalArea << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " RURAL    AREA: " << ruralArea[regionIdx] << " "
                            << 100 * ruralArea[regionIdx] / totalArea << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " BARREN   AREA: " << barrenArea[regionIdx] << " "
...
/******************************************************************************************/
void PopGridClass::setDimensions(int numLonVal, double deltaLonDegVal, double minLonDegVal, int numLatVal, double deltaLatDegVal, double minLatDegVal)
{
    int lonIdx, latIdx;
...
/******************************************************************************************/
void PopGridClass::scale(
    std::vector<double> urbanPopVal, std::vector<double> suburbanPopVal, std::vector<double> ruralPopVal, std::vector<double> barrenPopVal)
{
    int regionIdx;
    ...
    for (regionIdx = 0; regionIdx < numRegion; regionIdx++) {
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " RLAN DEVICE URBAN    POPULATION: " << urbanPop[regionIdx] << " "
                            << 100 * urbanPop[regionIdx] / totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " RLAN DEVICE SUBURBAN POPULATION: " << suburbanPop[regionIdx] << " "
                            << 100 * suburbanPop[regionIdx] / totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " RLAN DEVICE RURAL    POPULATION: " << ruralPop[regionIdx] << " "
                            << 100 * ruralPop[regionIdx] / totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " RLAN DEVICE BARREN   POPULATION: " << barrenPop[regionIdx] << " "
                            << 100 * barrenPop[regionIdx] / totalPop << " %";
...
double PopGridClass::adjustRegion(ListClass<ULSClass *> *ulsList, double maxRadius)
{
    ...
    double maxDistGridCenterToEdge = CConst::earthRadius *
        sqrt((deltaLonDeg * deltaLonDeg + deltaLatDeg * deltaLatDeg) * (M_PI / 180.0) * (M_PI / 180.0)) / 2;
    ...
    for (lonIdx = 0; lonIdx < newNumLon; lonIdx++) {
        ...
        for (latIdx = 0; latIdx < newNumLat; latIdx++) {
            double latitudeDeg = minLatDeg + latIdx * deltaLatDeg;
            double area = CConst::earthRadius * CConst::earthRadius * cos(latitudeDeg * M_PI / 180.0) * deltaLonDeg * deltaLatDeg * (M_PI / 180.0) *
                (M_PI / 180.0);
    ...
    for (regionIdx = 0; regionIdx < numRegion; regionIdx++) {
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED URBAN    POPULATION: " << urbanPop[regionIdx] << " "
                            << 100 * urbanPop[regionIdx] / totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED SUBURBAN POPULATION: " << suburbanPop[regionIdx] << " "
                            << 100 * suburbanPop[regionIdx] / totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED RURAL    POPULATION: " << ruralPop[regionIdx] << " "
                            << 100 * ruralPop[regionIdx] / totalPop << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED BARREN   POPULATION: " << barrenPop[regionIdx] << " "
                            << 100 * barrenPop[regionIdx] / totalPop << " %";

        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED URBAN    AREA: " << urbanArea[regionIdx] << " "
                            << 100 * urbanArea[regionIdx] / totalArea << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED SUBURBAN AREA: " << suburbanArea[regionIdx] << " "
                            << 100 * suburbanArea[regionIdx] / totalArea << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED RURAL    AREA: " << ruralArea[regionIdx] << " "
                            << 100 * ruralArea[regionIdx] / totalArea << " %";
        LOGGER_INFO(logger) << "REGION " << regionNameList[regionIdx] << " ADJUSTED BARREN   AREA: " << barrenArea[regionIdx] << " "
                            << 100 * barrenArea[regionIdx] / totalArea << " %";
    }
