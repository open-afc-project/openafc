...
double ITUDataClass::getSurfaceRefractivityValue(double latDeg, double lonDeg)
{
    ...
    double surf_refract = (val00 * (latIdx1 - latIdxDbl) * (lonIdx1 - lonIdxDbl) + val01 * (latIdx1 - latIdxDbl) * (lonIdxDbl - lonIdx0) +
        val10 * (latIdxDbl - latIdx0) * (lonIdx1 - lonIdxDbl) + val11 * (latIdxDbl - latIdx0) * (lonIdxDbl - lonIdx0));
