...
double adiff(double d, prop_type &prop, propa_type &propa, struct adiff_statc &sc)
{
	...
	if (d == 0) {
		...
		sc.afo = mymin(15.0,
			       2.171 * log(1.0 + 4.77e-4 * prop.hg[0] * prop.hg[1] * prop.wn * q));
...
double ascat(double d, prop_type &prop, propa_type &propa, struct ascat_statc &sc)
{
	...
	if (d == 0.0) {
		...
		if (sc.h0s > 15.0)
			...
			et = (sc.etq * exp(-pow(mymin(1.7, z0 / 8.0e3), 6.0)) + 1.0) * z0 /
			     1.7556e3;
			ett = mymax(et, 1.0);
			...
			if (et < 1.0)
				h0 = et * h0 +
				     (1.0 - et) * 4.343 *
					     log(pow((1.0 + 1.4142 / r1) * (1.0 + 1.4142 / r2),
						     2.0) *
						 (r1 + r2) / (r1 + r2 + 2.8284));
...
void lrprop(double d, prop_type &prop, propa_type &propa, struct lrprop_statc &sc) // PaulM_lrprop
{
	...
	if (prop.mdp != 0) {
		...
		if (prop.ens < 250.0 || prop.ens > 400.0 || prop.gme < 75e-9 || prop.gme > 250e-9 ||
		    prop_zgnd.real() <= abs(prop_zgnd.imag()) || prop.wn < 0.419 ||
		    prop.wn > 420.0) {
			prop.kwx = 4;
		}
		for (j = 0; j < 2; j++)
...
struct avar_statc {
		int kdv;
		double dexa, de, vmd, vs0, sgl, sgtm, sgtp, sgtd, tgtd, gm, gp, cv1, cv2, yv1, yv2,
			yv3, csm1, csm2, ysm1, ysm2, ysm3, csp1, csp2, ysp1, ysp2, ysp3, csd1, zd,
			cfm1, cfm2, cfm3, cfp1, cfp2, cfp3;
		bool ws, w1;
};

double avar(double zzt,
	    double zzl,
	    double zzc,
	    prop_type &prop,
	    propv_type &propv,
	    struct avar_statc &sc)
{ // static int kdv;
...
void point_to_pointMDH(double elev[],
		       double tht_m,
		       double rht_m,
		       double eps_dielect,
		       double sgm_conductivity,
...
void point_to_pointDH(double elev[],
		      double tht_m,
		      double rht_m,
		      double eps_dielect,
		      double sgm_conductivity,
