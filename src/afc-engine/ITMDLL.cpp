...
double adiff(double d, prop_type &prop, propa_type &propa, struct adiff_statc &sc)
{
    ...
    if (d == 0) {
        ...
        sc.qk = prop.he[0] * prop.he[1] - q;
        if (prop.mdp < 0.0)
            q += 10.0;
        sc.wd1 = sqrt(1.0 + sc.qk / q);
        sc.xd1 = propa.dla + propa.tha / prop.gme;
        q = (1.0 - 0.8 * exp(-propa.dlsa / 50e3)) * prop.dh;
        q *= 0.78 * exp(-pow(q / 16.0, 0.25)); // Initial diffraction constants, 11
        sc.afo = mymin(15.0, 2.171 * log(1.0 + 4.77e-4 * prop.hg[0] * prop.hg[1] * prop.wn * q));
        ...
        wa = pow(a * prop.wn, THIRD);
        pk = sc.qk / wa;
        q = (1.607 - pk) * 151.0 * wa * th + sc.xht;
        ar = 0.05751 * q - 4.343 * log(q) - sc.aht;
        q = (sc.wd1 + sc.xd1 / d) * mymin(((1.0 - 0.8 * exp(-d / 50e3)) * prop.dh * prop.wn), 6283.2);
        wd = 25.1 / (25.1 + sqrt(q));
        adiffv = ar * wd + (1.0 - wd) * adiffv + sc.afo;
    }
    return adiffv;
}
...
double ascat(double d, prop_type &prop, propa_type &propa, struct ascat_statc &sc)
{
    complex<double> prop_zgnd(prop.zgndreal, prop.zgndimag);
    ...
    if (d == 0.0) {
        ...
        if (sc.h0s > 15.0)
            ...
            z0 = (d - sc.ad) * (d + sc.ad) * th * 0.25 / d;
            et = (sc.etq * exp(-pow(mymin(1.7, z0 / 8.0e3), 6.0)) + 1.0) * z0 / 1.7556e3;
            ett = mymax(et, 1.0);
...
double alos(double d, prop_type &prop, propa_type &propa, struct alos_statc &sc)
{
    ...
    double alosv;
    if (d == 0.0) {
        sc.wls = 0.021 / (0.021 + prop.wn * prop.dh / mymax(10e3, propa.dlsa));
        alosv = 0.0;
    } else {
        ...
        if (q > 1.57)
            q = 3.14 - 2.4649 / q;
        alosv = (-4.343 * log(abq_alos(complex<double>(cos(q), -sin(q)) + r)) - alosv) * sc.wls + alosv;
    }
    return alosv;
}
...
void lrprop(double d, prop_type &prop, propa_type &propa, struct lrprop_statc &sc) // PaulM_lrprop
{
    ...
    if (prop.mdp != 0) {
        ...
        for (j = 0; j < 2; j++)
            if (abs(prop.the[j]) > 200e-3 || prop.dl[j] < 0.1 * propa.dls[j] || prop.dl[j] > 3.0 * propa.dls[j]) {
                ...
                // abs(prop.the[j]), 200e-3); printf("prop.dl[%d] = %.8g,
                // prop.dls[%d] = %.8g\n", j, prop.dl[j], j, propa.dls[j]);
                // printf("prop.dl should be between 0.1 and 3 times dls,
                // below:\n"); printf("prop.dls * 0.1 = %.8g\n", 0.1 *
                // propa.dls[j]); printf("prop.dls * 3 = %.8g\n", 3 * propa.dls[j]);
                // printf("failures = %d %d %d\n", abs(prop.the[j]) >200e-3,
                // prop.dl[j]<0.1*propa.dls[j],
                //		prop.dl[j]>3.0*propa.dls[j]);
            }
        if (prop.ens < 250.0 || prop.ens > 400.0 || prop.gme < 75e-9 || prop.gme > 250e-9 || prop_zgnd.real() <= abs(prop_zgnd.imag()) ||
        ...
        sc.xae = pow(prop.wn * pow(prop.gme, 2), -THIRD);
        d3 = mymax(propa.dlsa, 1.3787 * sc.xae + propa.dla);
        d4 = d3 + 2.7574 * sc.xae;
        a3 = adiff(d3, prop, propa, ad_st);
        a4 = adiff(d4, prop, propa, ad_st);
    ...
    if (prop.dist < propa.dlsa) {
        if (!sc.wlos) {
            struct alos_statc asc;
            q = alos(0.0, prop, propa, asc);
            d2 = propa.dlsa;
    ...
    if (prop.dist <= 0.0 || prop.dist >= propa.dlsa) {
        if (!sc.wscat) {
            ...
            if (a5 < 1000.0) {
                ...
                propa.dx = mymax(
                    propa.dlsa, mymax(propa.dla + 0.3 * sc.xae * log(47.7 * prop.wn), (a5 - propa.aed - propa.ems * d5) / (propa.emd - propa.ems)));
...
struct avar_statc {
        int kdv;
        double dexa, de, vmd, vs0, sgl, sgtm, sgtp, sgtd, tgtd, gm, gp, cv1, cv2, yv1, yv2, yv3, csm1, csm2, ysm1, ysm2, ysm3, csp1, csp2, ysp1, ysp2,
            ysp3, csd1, zd, cfm1, cfm2, cfm3, cfp1, cfp2, cfp3;
        bool ws, w1;
};

double avar(double zzt, double zzl, double zzc, prop_type &prop, propv_type &propv, struct avar_statc &sc)
{ // static int kdv;
    ...
    if (propv.lvar > 0) {
        switch (propv.lvar) {
            default:
                ...
                sc.cv2 = bv2[temp_klim];
                sc.yv1 = xv1[temp_klim];
                sc.yv2 = xv2[temp_klim];
                sc.yv3 = xv3[temp_klim];
                sc.csm1 = bsm1[temp_klim];
                sc.csm2 = bsm2[temp_klim];
                sc.ysm1 = xsm1[temp_klim];
                sc.ysm2 = xsm2[temp_klim];
                sc.ysm3 = xsm3[temp_klim];
                sc.csp1 = bsp1[temp_klim];
                sc.csp2 = bsp2[temp_klim];
                sc.ysp1 = xsp1[temp_klim];
                sc.ysp2 = xsp2[temp_klim];
                sc.ysp3 = xsp3[temp_klim];
                sc.csd1 = bsd1[temp_klim];
                sc.zd = bzd1[temp_klim];
                sc.cfm1 = bfm1[temp_klim];
                sc.cfm2 = bfm2[temp_klim];
                sc.cfm3 = bfm3[temp_klim];
            ...
            case 2:
                sc.dexa = sqrt(18e6 * prop.he[0]) + sqrt(18e6 * prop.he[1]) + pow((575.7e12 / prop.wn), THIRD);
            case 1:
        ...
        sc.vmd = curve(sc.cv1, sc.cv2, sc.yv1, sc.yv2, sc.yv3, sc.de);
        sc.sgtm = curve(sc.csm1, sc.csm2, sc.ysm1, sc.ysm2, sc.ysm3, sc.de) * sc.gm;
        sc.sgtp = curve(sc.csp1, sc.csp2, sc.ysp1, sc.ysp2, sc.ysp3, sc.de) * sc.gp;
        sc.sgtd = sc.sgtp * sc.csd1;
        sc.tgtd = (sc.sgtp - sc.sgtd) * sc.zd;
        if (sc.w1)
            sc.sgl = 0.0;
        else {
        ...
        if (sc.ws)
            ...
            sc.vs0 = pow(5.0 + 3.0 * exp(-sc.de / 100e3), 2.0);
        propv.lvar = 0;
    }
    zt = zzt;
    zl = zzl;
...
void hzns(double pfl[], prop_type &prop)
{
    ...
    if (np >= 2) {
        ...
        for (int i = 1; i < np; i++) {
            ...
            // printf("For point %d, q = %.8g - (%.8g * %.8g + %.8g) * %.8g - %.8g =
...
void point_to_point(double elev[], double tht_m, double rht_m, double eps_dielect, double sgm_conductivity, double eno_ns_surfref, double frq_mhz,
    int radio_climate, int pol, double conf, double rel, double &dbloss, std::string &strmode, int &errnum)
...
// conf, rel: .01 to .99
// elev[]: [num points - 1], [delta dist(meters)], [height(meters) point 1], ..., [height(meters)
// point n] errnum: 0- No Error.
...
//                          Results are probably invalid.
{
    ...
    dbloss = avar(zr, 0.0, zc, prop, propv, asc) + fs;
    errnum = prop.kwx;
}

void point_to_pointMDH(double elev[], double tht_m, double rht_m, double eps_dielect, double sgm_conductivity, double eno_ns_surfref, double frq_mhz,
    int radio_climate, int pol, double timepct, double locpct, double confpct, double &dbloss, int &propmode, double &deltaH, int &errnum)
// pol: 0-Horizontal, 1-Vertical
// radio_climate: 1-Equatorial, 2-Continental Subtropical, 3-Maritime Tropical,
//                4-Desert, 5-Continental Temperate, 6-Maritime Temperate, Over Land,
//                7-Maritime Temperate, Over Sea
...
void point_to_pointDH(double elev[], double tht_m, double rht_m, double eps_dielect, double sgm_conductivity, double eno_ns_surfref, double frq_mhz,
...
void area(long ModVar, double deltaH, double tht_m, double rht_m, double dist_km, int TSiteCriteria, int RSiteCriteria, double eps_dielect,
    double sgm_conductivity, double eno_ns_surfref, double frq_mhz, int radio_climate, int pol, double pctTime, double pctLoc, double pctConf,
    double &dbloss, std::string &strmode, int &errnum)
{
    ...
    //         1 - Individual: pctTime is "Situation/Location", pctConf is "Confidence", pctLoc
    //         not used 2 - Mobile: pctTime is "Time/Locations (Reliability)", pctConf is
    //         "Confidence", pctLoc not used 3 - Broadcast: pctTime is "Time", pctLoc is
    ...
    if (propv.lvar < 1)
        propv.lvar = 1;
    struct lrprop_statc lrc;
    lrprop(dist_km * 1000.0, prop, propa, lrc);
    fs = 32.45 + 20.0 * log10(frq_mhz) + 20.0 * log10(prop.dist / 1000.0);
    struct avar_statc asc;
    xlb = fs + avar(zt, zl, zc, prop, propv, asc);
    dbloss = xlb;
    if (prop.kwx == 0)
        errnum = 0;
    else
...
double ITMAreadBLoss(long ModVar, double deltaH, double tht_m, double rht_m, double dist_km, int TSiteCriteria, int RSiteCriteria, double eps_dielect,
    double sgm_conductivity, double eno_ns_surfref, double frq_mhz, int radio_climate, int pol, double pctTime, double pctLoc, double pctConf)
{
    std::string strmode;
    int errnum;
    double dbloss;
    area(ModVar, deltaH, tht_m, rht_m, dist_km, TSiteCriteria, RSiteCriteria, eps_dielect, sgm_conductivity, eno_ns_surfref, frq_mhz, radio_climate,
