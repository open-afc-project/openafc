...
double AntennaClass::gainDB(double dx, double dy, double dz, double h_angle_rad)
{
    ...
    if (type == CConst::antennaOmni) {
        ...
        gain_db = (1.0 - fabs(phi) / M_PI) * (gv1 - gain_fwd_db) + (fabs(phi) / M_PI) * (gv2 - gain_back_db) + gh;
    } else {
...
int AntennaClass::checkGain(const char *flname, int orient, int numpts)
{
    ...
    LOGGER_INFO(logger) << "Checking " << (orient == 0 ? "HORIZONTAL" : "VERTICAL") << " antenna gain.  Writing " << numpts << " points to file \""
                        << flname << "\"";
