...
GdalTransform::BoundRect::BoundRect(double latDegMin_, double lonDegMin_, double latDegMax_, double lonDegMax_) :
...
bool GdalTransform::BoundRect::contains(double latDeg, double lonDeg) const
{
    ...
    return (latDegMin < latDeg) && (lonDegMin <= lonDeg) && (latDeg <= latDegMax) && (lonDeg < lonDegMax);
}
...
GdalTransform::GdalTransform(GDALDataset *gdalDataSet_, const std::string &filename_) : margin(0)
{
    ...
    if (!((gdalTransform[2] == 0) && (gdalTransform[4] == 0) && (gdalTransform[1] > 0) && (gdalTransform[5] < 0))) {
        errStr << "ERROR: GdalTransform::GdalTransform(): GDAL data file '" << filename_ << "': does not contain 'north up' data";
        throw std::runtime_error(errStr.str());
    }
...
GdalTransform::GdalTransform(const GdalTransform &gdalXform_, int latPixOffset_, int lonPixOffset_, int latSize_, int lonSize_) :
...
GdalTransform::GdalTransform() : latPixPerDeg(0), lonPixPerDeg(0), latPixMax(0), lonPixMin(0), latSize(0), lonSize(0), margin(0)
{
}
...
void GdalTransform::computePixel(double latDeg, double lonDeg, int *latIdx, int *lonIdx) const
{
    ...
    *lonIdx = (int)std::floor(lonDeg * lonPixPerDeg - lonPixMin);
    ...
    if ((*latIdx < -1) || (*latIdx > latSize) || (*lonIdx < -1) || (*lonIdx > lonSize)) {
        ...
        std::ostringstream errStr;
        errStr << "ERROR: GdalTransform::computePixel() internal error: point (" << latDeg << "N, " << lonDeg << "E is out of tile/GDAL bounds of ["
               << br.latDegMin << " - " << br.latDegMax << "]N X [" << br.lonDegMin << " - " << br.lonDegMax << "]E";
        throw std::runtime_error(errStr.str());
    }
...
{
    return BoundRect((latPixMax - latSize + margin) / latPixPerDeg, (lonPixMin + margin) / lonPixPerDeg, (latPixMax - margin) / latPixPerDeg,
