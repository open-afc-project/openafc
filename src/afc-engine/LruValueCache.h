...
class LruValueCache
{
    ...
    public:
         ...
         */
        LruValueCache(size_t capacity) : _capacity(capacity), _recentKey(nullptr), _recentValue(nullptr), _hits(0), _misses(0), _evictions(0)
        {
        }
        ...
        V *add(const K &key, const V &value)
        {
            ...
            if (i == _map.end()) {
                ...
                i = _map.emplace(std::piecewise_construct, std::forward_as_tuple(key), std::forward_as_tuple(value, _list.begin())).first;
            } else {
                // Key in cache
