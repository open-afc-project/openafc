...
{
    if (!configuredFlag) {
        throw std::runtime_error(ErrStream() << "ERROR: RlanRegionClass::getMinHeightAMSL() RlanRegion "
                                                "not configured");
    }
...
{
    if (!configuredFlag) {
        throw std::runtime_error(ErrStream() << "ERROR: RlanRegionClass::getMaxHeightAMSL() RlanRegion "
                                                "not configured");
    }
...
/******************************************************************************************/
EllipseRlanRegionClass::EllipseRlanRegionClass(DoubleTriplet rlanLLA, DoubleTriplet rlanUncerts_m, double rlanOrientationDeg, bool fixedHeightAMSLVal)
{
...
/******************************************************************************************/
double EllipseRlanRegionClass::calcMinAOB(
    LatLon ulsRxLatLon, Vector3 ulsAntennaPointing, double ulsRxHeightAMSL, double &minAOBLon, double &minAOBLat, double &minAOBHeghtAMSL)
{
    if (!boundaryPolygon) {
        int numPts = 32;
...
/******************************************************************************************/
double PolygonRlanRegionClass::calcMinAOB(
    LatLon ulsRxLatLon, Vector3 ulsAntennaPointing, double ulsRxHeightAMSL, double &minAOBLon, double &minAOBLat, double &minAOBHeghtAMSL)
{
    arma::vec ptg(3);
    ptg(0) = ulsAntennaPointing.dot(eastVec);
...
std::vector<LatLon> PolygonRlanRegionClass::getScan(CConst::ScanRegionMethodEnum method, double scanResolutionM, int pointsPerDegree)
{
    ...
    if ((method == CConst::xyAlignRegionNorthEastScanRegionMethod) || (method == CConst::xyAlignRegionMajorMinorScanRegionMethod)) {
...
double RlanRegionClass::calcMinAOB(PolygonClass *poly, double polyResolution, arma::vec &F, arma::vec &ptg, arma::vec &minLoc)
{
    ...
    if (((ptg(2) < 0.0) && (F(2) > 0.0)) || ((ptg(2) > 0.0) && (F(2) < 0.0))) {
        ...
        if ((xproj >= (minx - 1) * polyResolution) && (xproj <= (maxx + 1) * polyResolution) && (yproj >= (miny - 1) * polyResolution) &&
