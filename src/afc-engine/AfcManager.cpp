...
QString ISO8601TimeUTC(const int &dayStep = 0)
{
    ...
    std::string result = padStringFront(std::to_string(ptm->tm_year + 1900), '0', 4) + "-" + padStringFront(std::to_string(1 + ptm->tm_mon), '0', 2) +
        "-" + padStringFront(std::to_string(ptm->tm_mday), '0', 2) + "T" + padStringFront(std::to_string(ptm->tm_hour), '0', 2) + ":" +
        padStringFront(std::to_string(ptm->tm_min), '0', 2) + ":" + padStringFront(std::to_string(ptm->tm_sec), '0', 2) + "Z";
...
OpClass GlobalOpClass_131 = {131, // Operating class
    ...
    {1, 5, 9, 13, 17, 21, 25, 29, 33, 37, 41, 45, 49, 53, 57, 61, 65, 69, 73, 77, 81, 85, 89, 93, 97, 101, 105, 109, 113, 117, 121, 125, 129, 133,
        137, 141, 145, 149, 153, 157, 161, 165, 169, 173, 177, 181, 185, 189, 193, 197, 201, 205, 209, 213, 217, 221, 225, 229, 233}};
...
const OpClass GlobalOpClass_133 = {133, 80, 5950, {7, 23, 39, 55, 71, 87, 103, 119, 135, 151, 167, 183, 199, 215}};
...
const OpClass GlobalOpClass_135 = {135, 80, 5950, {7, 23, 39, 55, 71, 87, 103, 119, 135, 151, 167, 183, 199, 215}};

const OpClass GlobalOpClass_136 = {136, 20, 5925, {2}};

const std::vector<OpClass> USOpClass = {GlobalOpClass_131, GlobalOpClass_132, GlobalOpClass_133, GlobalOpClass_134,
...
static const std::map<int, std::string> pathLossModelNames = {{CConst::unknownPathLossModel, "unknownPathLossModel"},
    {CConst::ITMBldgPathLossModel, "ITMBldgPathLossModel"}, {CConst::CoalitionOpt6PathLossModel, "CoalitionOpt6PathLossModel"},
    {CConst::FCC6GHzReportAndOrderPathLossModel, "FCC6GHzReportAndOrderPathLossModel"}, {CConst::CustomPathLossModel, "CustomPathLossModel"},
    {CConst::ISEDDBS06PathLossModel, "ISEDDBS06PathLossModel"}, {CConst::BrazilPathLossModel, "BrazilPathLossModel"},
    {CConst::OfcomPathLossModel, "OfcomPathLossModel"}, {CConst::FSPLPathLossModel, "FSPLPathLossModel"}};
static const std::map<int, std::string> propEnvNames = {{CConst::unknownPropEnv, "unknownPropEnv"}, {CConst::urbanPropEnv, "urbanPropEnv"},
    {CConst::suburbanPropEnv, "suburbanPropEnv"}, {CConst::ruralPropEnv, "ruralPropEnv"}, {CConst::barrenPropEnv, "barrenPropEnv"}};
...
class EirpGzipCsv : public GzipCsv
{
    public:
        ...
        EirpGzipCsv(const std::string &filename) :
            ...
            chanType(this, "ChanType", {{INQUIRED_FREQUENCY, "ByFreq"}, {INQUIRED_CHANNEL, "ByNumber"}}),
            eirpLimit(this, "EIRP"),
            fspl(this, "FreeSpace"),
...
void AfcManager::initializeDatabases()
{
    ...
    switch (_pathLossModel) {
        ...
        case CConst::FSPLPathLossModel:
            ...
            break;
        default:
            throw std::runtime_error(ErrStream() << std::string("ERROR: Path Loss Model set to invalid value \"") +
    ...
    _terrainDataModel = new TerrainClass(_lidarDir, _cdsmDir, _srtmDir, _depDir, _globeDir, minLat, minLon, maxLat, maxLon, minLatBldg, minLonBldg,
        maxLatBldg, maxLonBldg, _maxLidarRegionLoadVal);
    ...
    if (_propEnvMethod == CConst::nlcdPointPropEnvMethod) {
        ...
        if (nlcdFileInfo.fileName().contains('{')) {
            ...
            nameMapper = GdalNameMapperPattern::make_unique(nlcdFileInfo.fileName().toStdString(), nlcdDirectory);
            ...
            nlcdDirectory = nlcdFileInfo.dir().path().toStdString();
            nameMapper = GdalNameMapperDirect::make_unique(nlcdFileInfo.fileName().toStdString(), nlcdDirectory);
        }
        ...
        _popGrid = new PopGridClass(_worldPopulationFile, _regionPolygonList, _regionPolygonResolution, _densityThrUrban, _densityThrSuburban,
            _densityThrRural, minLat, minLon, maxLat, maxLon);
    }
    /**************************************************************************************/
...
void AfcManager::importGUIjson(const std::string &inputJSONpath)
{
    ...
    LOGGER_DEBUG(logger) << "Contents of imported JSON file: " << std::endl << jsonDoc.toJson().toStdString() << std::endl;
    ...
    if (_guiJsonVersion == "1.4") {
        ...
        LOGGER_WARN(logger) << "VERSION NOT SUPPORTED: GUI JSON FILE \"" << inputJSONpath << "\": version: " << _guiJsonVersion;
...
void AfcManager::importGUIjsonVersion1_4(const QJsonObject &jsonObj)
{
    ...
    if ((_analysisType == "AP-AFC") || (_analysisType == "ScanAnalysis") || (_analysisType == "test_itm") || (_analysisType == "HeatmapAnalysis")) {
                ...
                case 1: /* Indoor */
                    if (_rulesetId == QString::fromStdString("US_47_CFR_PART_15_SUBPART_E")) {
                        ...
                        if (_certifiedIndoor) {
                            ...
                            LOGGER_INFO(logger) << _serialNumber.toStdString() << " indicated as deployed indoor "
        ...
        if (verticalUncertainty < 0.0) {
            ...
            LOGGER_WARN(logger) << "GENERAL FAILURE: verticalUncertainty = " << verticalUncertainty << " exceeds max value of "
                                << _maxVerticalUncertainty;
            _invalidParams << "verticalUncertainty";
        }
        ...
        if (hasEllipseFlag) {
            ...
            if (majorAxis < minorAxis) {
                ...
                if (majorAxis < 0.0) {
                    ...
                    LOGGER_WARN(logger) << "GENERAL FAILURE: 2*majorAxis = " << 2 * majorAxis << " exceeds max value of "
        ...
        for (auto inquiredFrequencyRangeVal : inquiredFrequencyRangeArray) {
            ...
            _inquiredFrequencyRangesMHz.push_back(
                std::make_pair(inquiredFrequencyRangeObj["lowFrequency"].toInt(), inquiredFrequencyRangeObj["highFrequency"].toInt()));
        }
            ...
            if (parametersObj.contains("type") && !parametersObj["type"].isUndefined()) {
                ...
                if (type == "rlanAntenna") {
                    ...
                    if (gainArray.size() != numAntennaAOB) {
                        LOGGER_WARN(logger) << "GENERAL FAILURE: numAntennaAOB = " << numAntennaAOB << " discriminationGainDB has "
        ...
        for (auto chanClass : _inquiredChannels) {
            LOGGER_INFO(logger) << (chanClass.second.empty() ? "ALL" : std::to_string(chanClass.second.size()))
...
void AfcManager::setCmdLineParams(std::string &inputFilePath, std::string &configFilePath, std::string &outputFilePath, std::string &tempDir,
    std::string &logLevel, int argc, char **argv)
{
    // Declare the supported options
    po::options_description optDescript{"Allowed options"};
    // Create command line options
    optDescript.add_options()("help,h", "Use input-file-path, config-file-path, or output-file-path.")(
        "request-type,r", po::value<std::string>()->default_value("AP-AFC"), "set request-type (AP-AFC, HeatmapAnalysis, ExclusionZoneAnalysis)")(
        "state-root,s", po::value<std::string>()->default_value("/var/lib/fbrat"), "set fbrat state root directory")("mnt-path,s",
        po::value<std::string>()->default_value("/mnt/"
                                                "nfs"),
        "set share with GeoData and config data")(
        "input-file-path,i", po::value<std::string>()->default_value("inputFile.json"), "set input-file-path level")("config-file-path,c",
        po::value<std::string>()->default_value("configFile."
                                                "json"),
        "set config-file-path level")("output-file-path,o", po::value<std::string>()->default_value("outputFile.json"), "set output-file-path level")(
        "temp-dir,t", po::value<std::string>()->default_value(""), "set temp-dir level")(
        "log-level,l", po::value<std::string>()->default_value("debug"), "set log-level")("runtime_opt,u", po::value<uint32_t>()->default_value(3),
...
void AfcManager::importConfigAFCjson(const std::string &inputJSONpath, const std::string &tempDir)
{
    ...
    if (jsonObj.contains("rlanITMTxClutterMethod") && !jsonObj["rlanITMTxClutterMethod"].isUndefined()) {
        _rlanITMTxClutterMethod = (CConst::ITMClutterMethodEnum)CConst::strITMClutterMethodList->str_to_type(
    ...
    if (jsonObj.contains("ulsDefaultAntennaType") && !jsonObj["ulsDefaultAntennaType"].isUndefined()) {
        _ulsDefaultAntennaType = (CConst::ULSAntennaTypeEnum)CConst::strULSAntennaTypeList->str_to_type(
    ...
    if ((_pathLossModel == CConst::CustomPathLossModel) || (_pathLossModel == CConst::ISEDDBS06PathLossModel) ||
        (_pathLossModel == CConst::BrazilPathLossModel) || (_pathLossModel == CConst::OfcomPathLossModel)) {
    ...
    // ***********************************
    _mapDataGeoJsonFile = "";
    if (jsonObj.contains("enableMapInVirtualAp") && !jsonObj["enableMapInVirtualAp"].isUndefined() && AfcManager::_createKmz) {
...
void AfcManager::addBuildingDatabaseTiles(OGRLayer *layer)
{
    ...
    if (_terrainDataModel->getNumLidarRegion()) {
        ...
        for (QRectF b : _terrainDataModel->getBounds()) {
            ...
            // the beam coverage
            GdalHelpers::GeomUniquePtr<OGRPolygon> boundBox(GdalHelpers::createGeometry<OGRPolygon>()); // Use GdalHelpers.h templates to have
            ...
            boundBox->addRingDirectly(boxPoints.release()); // Points unique-pointer to null and gives up
        ...
        for (int geomIdx = 0; geomIdx < numGeom; ++geomIdx) {
            ...
            if (rectFlag) {
                ...
                _terrainDataModel->getTerrainHeight(
                    longitudeCenter, latitudeCenter, drTerrainHeight, drBldgHeight, drLidarHeightResult, drHeightSource);
...
QJsonDocument AfcManager::generateRatAfcJson()
{
            ...
            if (!std::isnan(freqRange.psd_dBm_MHzList.at(i))) {
                spectrumInfos.append(
                    QJsonObject{{"frequencyRange",
                                    QJsonObject{{"lowFrequency", freqRange.freqMHzList.at(i)}, {"highFrequency", freqRange.freqMHzList.at(i + 1)}}},
    ...
    if (_responseCode == CConst::successResponseCode) {
        for (const auto &group : _inquiredChannels) {
            ...
            auto channelGroup = QJsonObject{{"globalOperatingClass", QJsonValue(operatingClass)}, {"channelCfi", indexArray}, {"maxEirp", eirpArray}};
            auto blackChannelGroup = QJsonObject{{"globalOperatingClass", QJsonValue(operatingClass)}, {"channelCfi", blackIndexArray}};
            auto redChannelGroup = QJsonObject{{"globalOperatingClass", QJsonValue(operatingClass)}, {"channelCfi", redIndexArray}};
    ...
    QJsonObject responses{{"version", _guiJsonVersion}, {"availableSpectrumInquiryResponses", QJsonArray{availableSpectrumInquiryResponse}}};
...
QJsonDocument AfcManager::generateExclusionZoneJson()
{
    ...
    GdalHelpers::GeomUniquePtr<OGRPolygon> exZone(GdalHelpers::createGeometry<OGRPolygon>()); // Use GdalHelpers.h templates to have
                                                                                              // unique pointers create these on the
                                                                                              // heap
    ...
    QJsonObject analysisJsonObj = {{"geoJson", geoJsonObj}, {"statusMessageList", generateStatusMessages(statusMessageList)}};
...
void AfcManager::addHeatmap(OGRLayer *layer)
{
    ...
    for (int lonIdx = 0; lonIdx < _heatmapNumPtsLon; lonIdx++) {
        for (int latIdx = 0; latIdx < _heatmapNumPtsLat; latIdx++) {
            ...
            // Intantiate unique-pointers to OGRPolygon and OGRLinearRing for storing
            // the beam coverage
            GdalHelpers::GeomUniquePtr<OGRPolygon> mapZone(GdalHelpers::createGeometry<OGRPolygon>()); // Use GdalHelpers.h templates to have
                                                                                                       // unique pointers create these on the
                                                                                                       // heap
            ...
            // Add exterior boundary of cone's polygon
            mapZone->addRingDirectly(mapExt.release()); // Points unique-pointer to null and gives up
                                                        // ownership of exteriorOfCone to beamCone
...
void AfcManager::generateMapDataGeoJson(const std::string &tempDir)
{
    ...
    for (const auto &ulsIdx : _ulsIdxList) {
        ...
        for (int segIdx = 0; segIdx < numPR + 1; ++segIdx) {
            ...
            // Intantiate unique-pointers to OGRPolygon and OGRLinearRing for storing
            // the beam coverage
            GdalHelpers::GeomUniquePtr<OGRPolygon> beamCone(GdalHelpers::createGeometry<OGRPolygon>()); // Use GdalHelpers.h templates to have
                                                                                                        // unique pointers create these on the
                                                                                                        // heap
            ...
            // Add exterior boundary of cone's polygon
            beamCone->addRingDirectly(exteriorOfCone.release()); // Points unique-pointer to null and gives up
                                                                 // ownership of exteriorOfCone to beamCone
    ...
    if (_rlanAntenna) {
        ...
        // coverage
        GdalHelpers::GeomUniquePtr<OGRPolygon> beamCone(GdalHelpers::createGeometry<OGRPolygon>()); // Use GdalHelpers.h templates
...
double AfcManager::getAngleFromDMS(std::string dmsStr)
{
    ...
    if ((dashPosn1 == std::string::npos) || (dashPosn1 == 0)) {
        ...
        if (!error) {
            ...
            std::string sStr = ((letterPosn == std::string::npos) ? dmsStr.substr(dashPosn2 + 1) :
    ...
    if (fabsMHz > BMHz / 2 + 1.0) {
        if (fabsMHz <= BMHz) {
            overlap += exp(log(10.0) * (-6 * BMHz + 28.0) / (5 * BMHz - 10.0)) *
                (exp(log(10.0) * ((-8.0) / (5 * BMHz - 10.0)) * (BMHz / 2 + 1.0)) - exp(log(10.0) * ((-8.0 * fabsMHz) / (5 * BMHz - 10.0)))) /
                ((8.0 * log(10.0)) / (5 * BMHz - 10.0));
        } else {
            overlap += exp(log(10.0) * (-6 * BMHz + 28.0) / (5 * BMHz - 10.0)) *
                (exp(log(10.0) * ((-8.0) / (5 * BMHz - 10.0)) * (BMHz / 2 + 1.0)) - exp(log(10.0) * ((-8.0 * BMHz) / (5 * BMHz - 10.0)))) /
                ((8.0 * log(10.0)) / (5 * BMHz - 10.0));
        }
    }
...
/******************************************************************************************/
bool AfcManager::computeSpectralOverlapLoss(double *spectralOverlapLossDBptr, double sigStartFreq, double sigStopFreq, double rxStartFreq,
...
/****                2: RX and TX                                                      ****/
/******************************************************************************************/
void AfcManager::readULSData(const std::vector<std::tuple<std::string, std::string>> &ulsDatabaseList, PopGridClass *popGridVal, int linkDirection,
    double minFreq, double maxFreq, bool removeMobileFlag, CConst::SimulationEnum simulationFlag, const double &minLat, const double &maxLat,
    const double &minLon, const double &maxLon)
{
    ...
    for (dbIdx = 0; dbIdx < (int)ulsDatabaseList.size(); ++dbIdx) {
        ...
        for (UlsRecord row : rows) {
            ...
            if (!_filterSimRegionOnly) {
                if (!ignoreFlag) {
                    ...
                    if (stopFreq < startFreq) {
                        throw std::runtime_error(ErrStream() << "ERROR reading ULS data: FSID = " << fsid << ", startFreq = " << startFreq
                                                             << ", stopFreq = " << stopFreq << ", must have startFreq < stopFreq");
                    }
                }
                            ...
                            if (!std::isnan(row.txHeightAboveTerrain)) {
                                ...
                                if (radioService == "CF") {
                                    ...
                                    fixedStr += "Fixed: missing Rx "
                                                "Height above "
                                                "Terrain for " +
                                        radioService + " set to " + std::to_string(rxHeightAboveTerrain);
                                    fixedMissingRxHeight = true;
                                    fixedFlag = true;
                ...
                if (!ignoreFlag) {
                    if (rxHeightAboveTerrain < 3.0) {
                        if (fixAnomalousEntries) {
                            ...
                            LOGGER_WARN(logger) << "WARNING: ULS data for FSID = " << fsid << ", rxHeightAboveTerrain = " << rxHeightAboveTerrain
                                                << " is < 3.0";
                        }
                    }
                }
                ...
                for (prIdx = 0; prIdx < numPR; ++prIdx) {
                    ...
                    if (!ignoreFlag) {
                        if (row.prHeightAboveTerrainRx[prIdx] <= 0.0) {
                            LOGGER_WARN(logger) << "WARNING: ULS data for FSID = " << fsid << ", Passive Repeater " << (prIdx + 1)
                            ...
                            if (fixAnomalousEntries) {
                                if (radioService == "CF") {
                                    ...
                                    fixedStr += "Fixed: invalid Rx "
                                                "Gain " +
                                        std::to_string(rxGain) + " for " + radioService + " set to 39.3";
                                    fixedFlag = true;
                                } else if (radioService == "MG") {
                                    rxGain = 41.0;
                                    fixedStr += "Fixed: invalid Rx "
                                                "Gain " +
                                        std::to_string(rxGain) + " for " + radioService + " set to 41.0";
                                    fixedFlag = true;
                                } else if (radioService == "MW") {
                                    rxGain = 39.9;
                                    fixedStr += "Fixed: invalid Rx "
                                                "Gain " +
                                        std::to_string(rxGain) + " for " + radioService + " set to 39.9";
                                    fixedFlag = true;
                                } else if (radioService == "TI") {
                                    rxGain = 41.8;
                                    fixedStr += "Fixed: invalid Rx "
                                                "Gain " +
                                        std::to_string(rxGain) + " for " + radioService + " set to 41.8";
                                    fixedFlag = true;
                                } else if (radioService == "TS") {
                                    rxGain = 41.5;
                                    fixedStr += "Fixed: invalid Rx "
                                                "Gain " +
                                        std::to_string(rxGain) + " for " + radioService + " set to 41.5";
                                    fixedFlag = true;
                                } else {
                                    ignoreFlag = true;
            ...
            if (!_filterSimRegionOnly) {
                if ((linkDirection == 1) || (linkDirection == 2) || (simulationFlag == CConst::MobileSimulation) ||
                    (simulationFlag == CConst::RLANSensingSimulation) || (simulationFlag == CConst::showFSPwrAtRLANSimulation)) {
                    /**************************************************************************/
                    /* txGain */
                    /**************************************************************************/
                    txGain = row.txGain;
            ...
            if (!ignoreFlag) {
                bool unii5Flag = computeSpectralOverlapLoss(
                    (double *)NULL, startFreq, stopFreq, 5925.0e6, 6425.0e6, false, CConst::psdSpectralAlgorithm);

                bool unii7Flag = computeSpectralOverlapLoss(
                    (double *)NULL, startFreq, stopFreq, 6525.0e6, 6875.0e6, false, CConst::psdSpectralAlgorithm);
                ...
                if (txLocFlag) {
                    if ((_terrainDataModel)) {
                        _terrainDataModel->getTerrainHeight(
                            txLongitudeDeg, txLatitudeDeg, terrainHeight, bldgHeight, lidarHeightResult, txHeightSource);
                ...
                for (prIdx = 0; prIdx < numPR; ++prIdx) {
                    ...
                    if ((_terrainDataModel)) {
                        _terrainDataModel->getTerrainHeight(
                            pr.longitudeDeg, pr.latitudeDeg, terrainHeight, bldgHeight, lidarHeightResult, prHeightSource);
        ...
        switch (lineType) {
                ...
                for (fIdx = 0; fIdx < (int)fieldIdxList.size(); fIdx++) {
                    if (*fieldIdxList.at(fIdx) == -1) {
                        errStr << "ERROR: Invalid Denied Region Data file "
                                  "\""
                               << filename << "\" label line missing \"" << fieldLabelList.at(fIdx) << "\"" << std::endl;
            ...
            case dataLineType:
                ...
                if (strval == "One Rectangle") {
                    ...
                    errStr << "ERROR: Invalid Denied Region Data file \"" << filename << "\" line " << linenum
                           << " exclusion zone set to unrecognized value " << strval << std::endl;
                    throw std::runtime_error(errStr.str());
                }
                /**************************************************************************/
                        ...
                        if (!horizonDistFlag) {
                            ...
                            if (strval.empty()) {
                                errStr << "ERROR: Invalid Denied "
                                          "Region Data file \""
                                       << filename << "\" line " << linenum << " missing Circle Radius" << std::endl;
                                throw std::runtime_error(errStr.str());
                            }
...
void AfcManager::fixFSTerrain()
{
    ...
    for (ulsIdx = 0; ulsIdx <= _ulsList->getSize() - 1; ulsIdx++) {
        ...
        if (!uls->getRxTerrainHeightFlag()) {
            _terrainDataModel->getTerrainHeight(
                uls->getRxLongitudeDeg(), uls->getRxLatitudeDeg(), terrainHeight, bldgHeight, lidarHeightResult, heightSource);
            updateFlag = true;
        ...
        if (!uls->getTxTerrainHeightFlag()) {
            _terrainDataModel->getTerrainHeight(
                uls->getTxLongitudeDeg(), uls->getTxLatitudeDeg(), terrainHeight, bldgHeight, lidarHeightResult, heightSource);
            updateFlag = true;
            uls->setTxTerrainHeightFlag(true);
...
/******************************************************************************************/
double AfcManager::computeBuildingPenetration(CConst::BuildingTypeEnum buildingType, double elevationAngleDeg, double frequency,
...
/******************************************************************************************/
void AfcManager::computePathLoss(CConst::PathLossModelEnum pathLossModel, bool itmFSPLFlag, CConst::PropEnvEnum propEnv,
    CConst::PropEnvEnum propEnvRx, CConst::NLCDLandCatEnum nlcdLandCatTx, CConst::NLCDLandCatEnum nlcdLandCatRx, double distKm, double fsplDistKm,
    double win2DistKm, double frequency, double txLongitudeDeg, double txLatitudeDeg, double txHeightM, double elevationAngleTxDeg,
    double rxLongitudeDeg, double rxLatitudeDeg, double rxHeightM, double elevationAngleRxDeg, double &pathLoss, double &pathClutterTxDB,
    double &pathClutterRxDB, std::string &pathLossModelStr, double &pathLossCDF, std::string &pathClutterTxModelStr, double &pathClutterTxCDF,
    std::string &pathClutterRxModelStr, double &pathClutterRxCDF, std::string *txClutterStrPtr, std::string *rxClutterStrPtr, double **ITMProfilePtr,
    ...
    if (pathLossModel == CConst::ITMBldgPathLossModel) {
            ...
            if (win2DistKm * 1000 < _closeInDist) {
                if (_closeInPathLossModel == "WINNER2") {
                    ...
                    if (win2DistKm * 1000 <= 50.0) {
                        winner2LOSValue = 1;
                    } else if ((_winner2LOSOption == CConst::BldgDataLOSOption) || (_winner2LOSOption == CConst::BldgDataReqTxLOSOption) ||
                        (_winner2LOSOption == CConst::BldgDataReqRxLOSOption) || (_winner2LOSOption == CConst::BldgDataReqTxRxLOSOption) ||
                        ...
                        _terrainDataModel->getTerrainHeight(
                            txLongitudeDeg, txLatitudeDeg, terrainHeight, bldgHeight, lidarHeightResult, txHeightSource, false);
                        _terrainDataModel->getTerrainHeight(
                            rxLongitudeDeg, rxLatitudeDeg, terrainHeight, bldgHeight, lidarHeightResult, rxHeightSource, false);
                        ...
                        if (((!reqTx) || (txHeightSource == CConst::lidarHeightSource)) &&
                            ...
                            bool losFlag = UlsMeasurementAnalysis::isLOS(_terrainDataModel, QPointF(txLatitudeDeg, txLongitudeDeg), txHeightM,
                                QPointF(rxLatitudeDeg, rxLongitudeDeg), rxHeightM, distKm, numPts, isLOSProfilePtr, isLOSSurfaceFracPtr);
                            if (losFlag) {
                    ...
                    if (propEnv == CConst::urbanPropEnv) {
                        // Winner2 C2: urban
                        pathLoss = Winner2_C2urban(
                            1000 * win2DistKm, rxHeightM, txHeightM, frequency, sigma, pathLossModelStr, pathLossCDF, probLOS, winner2LOSValue);
                    } else if (propEnv == CConst::suburbanPropEnv) {
                        // Winner2 C1: suburban
                        pathLoss = Winner2_C1suburban(
                            1000 * win2DistKm, rxHeightM, txHeightM, frequency, sigma, pathLossModelStr, pathLossCDF, probLOS, winner2LOSValue);
                    }
                } else {
                ...
                pathLoss = UlsMeasurementAnalysis::runPointToPoint(_terrainDataModel, true, QPointF(txLatitudeDeg, txLongitudeDeg), txHeightM,
                    QPointF(rxLatitudeDeg, rxLongitudeDeg), rxHeightM, distKm, _itmEpsDielect, _itmSgmConductivity, surfaceRefractivity, frequencyMHz,
                    radioClimate, _itmPolarization, u, _reliabilityITM, numPts, NULL, ITMProfilePtr);
        ...
        if ((propEnv == CConst::urbanPropEnv) || (propEnv == CConst::suburbanPropEnv)) {
            if (distKm * 1000 < _closeInDist) {
                if (_closeInPathLossModel == "WINNER2") {
                    ...
                    if (distKm * 1000 <= 50.0) {
                        ...
                        _terrainDataModel->getTerrainHeight(
                            txLongitudeDeg, txLatitudeDeg, terrainHeight, bldgHeight, lidarHeightResult, txHeightSource);
                        _terrainDataModel->getTerrainHeight(
                            rxLongitudeDeg, rxLatitudeDeg, terrainHeight, bldgHeight, lidarHeightResult, rxHeightSource);

                        if ((txHeightSource == CConst::lidarHeightSource) && (rxHeightSource == CConst::lidarHeightSource)) {
                            int numPts = std::min(((int)floor(distKm * 1000 / _itmMinSpacing)) + 1, _itmMaxNumPts);
                            bool losFlag = UlsMeasurementAnalysis::isLOS(_terrainDataModel, QPointF(txLatitudeDeg, txLongitudeDeg), txHeightM,
                                QPointF(rxLatitudeDeg, rxLongitudeDeg), rxHeightM, distKm, numPts, isLOSProfilePtr);
                    ...
                    if (propEnv == CConst::urbanPropEnv) {
                        // Winner2 C2: urban
                        pathLoss = Winner2_C2urban(
                            1000 * distKm, rxHeightM, txHeightM, frequency, sigma, pathLossModelStr, pathLossCDF, probLOS, winner2LOSValue);
                    } else if (propEnv == CConst::suburbanPropEnv) {
                        // Winner2 C1: suburban
                        pathLoss = Winner2_C1suburban(
                            1000 * distKm, rxHeightM, txHeightM, frequency, sigma, pathLossModelStr, pathLossCDF, probLOS, winner2LOSValue);
                ...
                if (itmFSPLFlag) {
                    ...
                    pathLoss = UlsMeasurementAnalysis::runPointToPoint(_terrainDataModel, false, QPointF(txLatitudeDeg, txLongitudeDeg), txHeightM,
                        QPointF(rxLatitudeDeg, rxLongitudeDeg), rxHeightM, distKm, _itmEpsDielect, _itmSgmConductivity, surfaceRefractivity,
                        frequencyMHz, radioClimate, _itmPolarization, u, _reliabilityITM, numPts, NULL, ITMProfilePtr);
        ...
        if (fsplDistKm * 1000 < 30.0) {
            ...
            if (win2DistKm * 1000 <= 50.0) {
                ...
                if (((!reqTx) || (txHeightSource == CConst::lidarHeightSource)) && ((!reqRx) || (rxHeightSource == CConst::lidarHeightSource))) {
                    int numPts = std::min(((int)floor(distKm * 1000 / _itmMinSpacing)) + 1, _itmMaxNumPts);
                    bool losFlag = UlsMeasurementAnalysis::isLOS(_terrainDataModel, QPointF(txLatitudeDeg, txLongitudeDeg), txHeightM,
                        QPointF(rxLatitudeDeg, rxLongitudeDeg), rxHeightM, distKm, numPts, isLOSProfilePtr, isLOSSurfaceFracPtr);
                    if (losFlag) {
...
/******************************************************************************************/
double AfcManager::Winner2_C1suburban_LOS(
...
/**** frequency = Frequency (Hz)                                                       ****/
/******************************************************************************************/
double AfcManager::Winner2_C1suburban_NLOS(
    double distance, double hBS, double /* hMS */, double frequency, double zval, double &sigma, double &pathLossCDF) const
{
    double retval;
...
/**** frequency = Frequency (Hz)                                                       ****/
/******************************************************************************************/
double AfcManager::Winner2_C1suburban(double distance, double hBS, double hMS, double frequency, double &sigma, std::string &pathLossModelStr,
...
/**** frequency = Frequency (Hz)                                                       ****/
/******************************************************************************************/
double AfcManager::Winner2_C2urban_LOS(
    double distance, double hBS, double hMS, double frequency, double zval, double &sigma, double &pathLossCDF) const
{
    double retval;
...
/**** frequency = Frequency (Hz)                                                       ****/
/******************************************************************************************/
double AfcManager::Winner2_C2urban_NLOS(
    double distance, double hBS, double /* hMS */, double frequency, double zval, double &sigma, double &pathLossCDF) const
{
    double retval;
...
/**** frequency = Frequency (Hz)                                                       ****/
/******************************************************************************************/
double AfcManager::Winner2_C2urban(double distance, double hBS, double hMS, double frequency, double &sigma, std::string &pathLossModelStr,
    double &pathLossCDF, double &probLOS, int losValue) const
{
    double retval = quietNaN;
...
/**** frequency = Frequency (Hz)                                                       ****/
/******************************************************************************************/
double AfcManager::Winner2_D1rural_LOS(
    double distance, double hBS, double hMS, double frequency, double zval, double &sigma, double &pathLossCDF) const
{
    double retval;
...
/**** frequency = Frequency (Hz)                                                       ****/
/******************************************************************************************/
double AfcManager::Winner2_D1rural_NLOS(
    double distance, double hBS, double hMS, double frequency, double zval, double &sigma, double &pathLossCDF) const
{
    double retval;
...
/**** frequency = Frequency (Hz)                                                       ****/
/******************************************************************************************/
double AfcManager::Winner2_D1rural(double distance, double hBS, double hMS, double frequency, double &sigma, std::string &pathLossModelStr,
    double &pathLossCDF, double &probLOS, int losValue) const
{
    double retval = quietNaN;
...
void AfcManager::runPointAnalysis()
{
    ...
    for (scanPtIdx = 0; scanPtIdx < (int)scanPointList.size(); scanPtIdx++) {
        ...
        MultibandRasterClass::HeightResult lidarHeightResult;
        _terrainDataModel->getTerrainHeight(
            scanPt.second, scanPt.first, rlanTerrainHeight[scanPtIdx], bldgHeight, lidarHeightResult, rlanHeightSource[scanPtIdx]);
    ...
    if (fkml) {
        ...
        fkml->writeTextElement("coordinates",
            QString::asprintf("%.10f,%.10f,%.2f", rlanCenterPtGeo.longitudeDeg, rlanCenterPtGeo.latitudeDeg, rlanCenterPtGeo.heightKm * 1000.0));
        ...
        for (ptIdx = 0; ptIdx < (int)ptList.size(); ptIdx++) {
            ...
            QString side_coords;
            side_coords.append(QString::asprintf(
        ...
        if (_rlanAntenna) {
            ...
            for (cvgPhiIdx = 0; cvgPhiIdx < numCvgPoints; ++cvgPhiIdx) {
                ...
                Vector3 cvgIntPosn = centerPosn +
                    (rlanAntennaArrowLength / 1000.0) * (zvec * cos(cvgTheta) + (xvec * cos(cvgPhi) + yvec * sin(cvgPhi)) * sin(cvgTheta));
                    ...
                    for (chanIdx = 0; chanIdx < (int)_channelList.size(); ++chanIdx) {
                        ...
                        for (int freqSegIdx = 0; freqSegIdx < channel->segList.size(); ++freqSegIdx) {
                            if (std::get<2>(channel->segList[freqSegIdx]) != BLACK) {
                                ...
                                bool hasOverlap = computeSpectralOverlapLoss((double *)NULL, chanStartFreq, chanStopFreq, dr->getStartFreq(),
                                    dr->getStopFreq(), false, CConst::psdSpectralAlgorithm);
                                if (hasOverlap) {
                                    channel->segList[freqSegIdx] = std::make_tuple(
                                        -std::numeric_limits<double>::infinity(), -std::numeric_limits<double>::infinity(), BLACK);
                                }
                            }
                        }
                    }
    ...
    std::cout << "End Processing Denied Regions " << tstr << " Elapsed time = " << (tEndDR - tStartDR) << " sec = " << elapsedTimeDRDay << " days "
              << elapsedTimeDRHour << " hours " << elapsedTimeDRMin << " min " << elapsedTimeDRSec << " sec." << std::endl
              << std::flush;
        ...
        if (true) {
            ...
            for (int segIdx = segStart; segIdx < numPR + 1; ++segIdx) {
                for (int divIdx = 0; divIdx < numDiversity; ++divIdx) {
                    Vector3 ulsRxPos = (segIdx == numPR ? (divIdx == 0 ? uls->getRxPosition() : uls->getDiversityPosition()) :
                    ...
                    if (distKmSquared < maxRadiusKmSquared) {
                        ...
                        double ulsRxHeightAGL = (segIdx == numPR ?
                                (divIdx == 0 ? uls->getRxHeightAboveTerrain() : uls->getDiversityHeightAboveTerrain()) :
                                uls->getPR(segIdx).heightAboveTerrainRx);
                        double ulsRxHeightAMSL = (segIdx == numPR ? (divIdx == 0 ? uls->getRxHeightAMSL() : uls->getDiversityHeightAMSL()) :
                                                                    uls->getPR(segIdx).heightAMSLRx);
                        ...
                        if ((!contains2D) && (fabs(closestLatLon.first - ulsRxLatLon.first) < 2.0 / _scanres_points_per_degree) &&
                            ...
                            for (scanPtIdx = 0; (scanPtIdx < (int)scanPointList.size()) && (!contains2D); scanPtIdx++) {
                                LatLon scanPt = scanPointList[scanPtIdx];
                                if ((fabs(scanPt.second - adjFSRxLongitude) < 1.0e-10) && (fabs(scanPt.first - adjFSRxLatitude) < 1.0e-10)) {
                                    contains2D = true;
                                }
                            }
                        }
                        ...
                        if (distKmSquared <= exclusionDistKmSquared) {
                            LOGGER_INFO(logger) << "FSID = " << uls->getID() << (segIdx == numPR ? " RX" : " PR " + std::to_string(segIdx))
                            ...
                            for (chanIdx = 0; chanIdx < (int)_channelList.size(); ++chanIdx) {
                                ...
                                for (int freqSegIdx = 0; freqSegIdx < channel->segList.size(); ++freqSegIdx) {
                                    ...
                                    if ((chanColor != BLACK)) {
                                        double chanStartFreq = channel->freqMHzList[freqSegIdx] * 1.0e6;
                                        double chanStopFreq = channel->freqMHzList[freqSegIdx + 1] * 1.0e6;
                                        bool hasOverlap = computeSpectralOverlapLoss((double *)NULL, chanStartFreq, chanStopFreq, uls->getStartFreq(),
                                            uls->getStopFreq(), useACI, CConst::psdSpectralAlgorithm);
                            ...
                            Vector3 ulsAntennaPointing = (segIdx == numPR ?
                                    (divIdx == 0 ? uls->getAntennaPointing() : uls->getDiversityAntennaPointing()) :
                                    uls->getPR(segIdx).pointing);

                            double minAngleOffBoresightDeg = 0.0;
                            if (contains2D) {
                                double minAOBLon, minAOBLat, minAOBHeghtAMSL;
                                minAngleOffBoresightDeg = _rlanRegion->calcMinAOB(
                                    ulsRxLatLon, ulsAntennaPointing, ulsRxHeightAMSL, minAOBLon, minAOBLat, minAOBHeghtAMSL);
                            ...
                            for (scanPtIdx = 0; scanPtIdx < (int)scanPointList.size(); scanPtIdx++) {
                                ...
                                for (rlanHtIdx = 0; rlanHtIdx < numRlanHt[scanPtIdx]; ++rlanHtIdx) {
                                    ...
                                    for (chanIdx = 0; chanIdx < (int)_channelList.size(); ++chanIdx) {
                                        ...
                                        // First set state =  0.  When state =  0, FSPL is used  in place of ITM.  In state = 0, go  through all freq
                                        // segments and if  state=0 does not  limit EIRP those  segments are  done. Identity list of segments  for
                                        // which state =  0 (FSPL in place  of ITM) does  limit EIRP and  full Path loss  calculation is  required.
                                        // Next  set state = 1.  When state = 1,  the full path  loss model is  used. For the  range of  frequencies
                                        // where  Path loss  calculation is  required, calculate path  loss at min and  max frequencies,  then use
                                        // linear  interpoplation  for intermediate  frequencies.
                                        bool contFlag = true;
                                        int state = 0;
                                        int freqSegIdx = 0;
                                        int itmSegIdx = -1;
                                        ...
                                        while (contFlag) {
                                            ...
                                            if ((chanColor != BLACK) && (chanColor != RED)) {
                                                ...
                                                bool useACI = (channelType == INQUIRED_FREQUENCY ? false : _aciFlag);
                                                CConst::SpectralAlgorithmEnum spectralAlgorithm = (channelType == INQUIRED_FREQUENCY ?
                                                        CConst::psdSpectralAlgorithm :
                                                        _channelResponseAlgorithm);
                                                // LOGGER_INFO(logger) << "COMPUTING SPECTRAL
                                                // OVERLAP FOR FSID = " << uls->getID();
                                                double spectralOverlapLossDB;
                                                bool hasOverlap = computeSpectralOverlapLoss(&spectralOverlapLossDB, chanStartFreqMHz * 1.0e6,
                                                    chanStopFreqMHz * 1.0e6, uls->getStartFreq(), uls->getStopFreq(), useACI, spectralAlgorithm);
                                                if (hasOverlap) {
                                                    ...
                                                    for (int bandEdgeIdx = 0; bandEdgeIdx < numBandEdge; ++bandEdgeIdx) {
                                                        ...
                                                        if (state == 0) {
                                                            ...
                                                            computePathLoss(contains2D ? CConst::FSPLPathLossModel : _pathLossModel, true,
                                                                rlanPropEnv[scanPtIdx], fsPropEnv, rlanNlcdLandCat[scanPtIdx], nlcdLandCatRx, distKm,
                                                                fsplDistKm, win2DistKm, evalFreqHz, rlanCoord.longitudeDeg, rlanCoord.latitudeDeg,
                                                                rlanHtAboveTerrain, elevationAngleTxDeg, ulsRxLongitude, ulsRxLatitude,
                                                                ulsRxHeightAGL, elevationAngleRxDeg, pathLoss, pathClutterTxDB, pathClutterRxDB,
                                                                pathLossModelStr, pathLossCDF, pathClutterTxModelStr, pathClutterTxCDF,
                                                                pathClutterRxModelStr, pathClutterRxCDF, &txClutterStr, &rxClutterStr,
                                                                &(uls->ITMHeightProfile), &(uls->isLOSHeightProfile), &(uls->isLOSSurfaceFrac)
#if DEBUG_AFC
                                                                                                                          ,
                                                                uls->ITMHeightType
#endif
                                                            );
                                                            buildingPenetrationDB = computeBuildingPenetration(_buildingType, elevationAngleTxDeg,
                                                                evalFreqHz, buildingPenetrationModelStr, buildingPenetrationCDF);

                                                            if (contains2D) {
                                                                angleOffBoresightDeg = minAngleOffBoresightDeg;
                                                            } else {
                                                                angleOffBoresightDeg = acos(ulsAntennaPointing.dot(
                                                                                           -(lineOfSightVectorKm.normalized()))) *
                                                                    180.0 / M_PI;
                                                            }
                                                            if (segIdx == numPR) {
                                                                rxGainDB = uls->computeRxGain(angleOffBoresightDeg, elevationAngleRxDeg, evalFreqHz,
                                                                    rxAntennaSubModelStr, divIdx);
                                                            } else {
                                                                discriminationGain = uls->getPR(segIdx).computeDiscriminationGain(
                                                                    angleOffBoresightDeg, elevationAngleRxDeg, evalFreqHz, reflectorD0, reflectorD1);
                                                            ...
                                                            if (segIdx == numPR) {
                                                                if (_nearFieldAdjFlag && (distKm * 1000.0 < uls->getRxNearFieldDistLimit()) &&
                                                                    (angleOffBoresightDeg < 90.0)) {
                                                                    bool unii5Flag = computeSpectralOverlapLoss((double *)NULL, uls->getStartFreq(),
                                                                        uls->getStopFreq(), 5925.0e6, 6425.0e6, false, CConst::psdSpectralAlgorithm);
                                                                    double Fc;
                                                                    if (unii5Flag) {
                                                                        Fc = 6175.0e6;
                                                                    } else {
                                                            ...
                                                            rxPowerDBW_0PL[bandEdgeIdx] = (maxEIRPdBm - 30.0) + rlanDiscriminationGainDB -
                                                                _bodyLossDB - buildingPenetrationDB - pathClutterTxDB - pathClutterRxDB + rxGainDB +
                                                                nearFieldOffsetDB - spectralOverlapLossDB - _polarizationLossDB -
                                                            ...
                                                            pathLoss = (itmStartPathLoss * (itmStopFreqMHz - evalFreqMHz) +
                                                                           itmStopPathLoss * (evalFreqMHz - itmStartFreqMHz)) /
                                                        ...
                                                        if (excthrGc && (((state == 0) && (_printSkippedLinksFlag)) || (state == 1)) &&
                                                            ...
                                                            if (ulsSegmentDistance != -1.0) {
                                                                const Vector3 ulsTxPos = (segIdx ? uls->getPR(segIdx - 1).positionTx :
                                                                                                   uls->getTxPosition());
                                                            ...
                                                            if (segIdx == numPR) {
                                                                ...
                                                                if (ulsRxAntennaType == CConst::LUTAntennaType) {
                                                                    ...
                                                                    rxAntennaTypeStr = std::string(CConst::strULSAntennaTypeList->type_to_str(
                                                                                           ulsRxAntennaType)) +
                                                                        excThrParam[bandEdgeIdx].rxAntennaSubModelStr;
                                                                }
                                                            } else {
                                                            ...
                                                            excthrGc->fsAgl = divIdx == 0 ? uls->getRxHeightAboveTerrain() :
                                                            ...
                                                            excthrGc->antCategory = CConst::strAntennaCategoryList->type_to_str(
                                                                segIdx == numPR ? uls->getRxAntennaCategory() : uls->getPR(segIdx).antCategory);
                                                    ...
                                                    if (state == 0) {
                                                        ...
                                                        if (contains2D) {
                                                            ...
                                                            ((numBandEdge == 2) &&
                                                                ((eirpLimit_dBm[1] - 1) < std::get<1>(channel->segList[freqSegIdx])))) {
                                                    ...
                                                    if (!skip) {
                                                        ...
                                                        if (channelType == ChannelType::INQUIRED_CHANNEL) {
                                                            if ((std::get<2>(channel->segList[freqSegIdx]) != RED) &&
                                                                (eirpLimit_dBm[0] < std::get<0>(channel->segList[freqSegIdx]))) {
                                                            ...
                                                            if (std::get<2>(channel->segList[freqSegIdx]) != RED) {
                                                                bool redFlag = true;
                                                                for (int bandEdgeIdx = 0; bandEdgeIdx < numBandEdge; ++bandEdgeIdx) {
                                                                    double bandEdgeVal = ((bandEdgeIdx == 0) ?
                                                    ...
                                                    if ((numItmSeg > 1) || ((numItmSeg > 0) && (channelType == ChannelType::INQUIRED_FREQUENCY))) {
                                                        if (channelType == ChannelType::INQUIRED_FREQUENCY) {
                                                            ...
                                                            itmStartFreqMHz = (channel->freqMHzList[itmSegList[0]] +
                                                                ...
                                                                2.0;
                                                            itmStopFreqMHz = (channel->freqMHzList[itmSegList[numItmSeg]] +
                                                                                 channel->freqMHzList[itmSegList[numItmSeg] + 1]) /
                                                                2.0;
                                                        }

                                                        computePathLoss(contains2D ? CConst::FSPLPathLossModel : _pathLossModel, false,
                                                            rlanPropEnv[scanPtIdx], fsPropEnv, rlanNlcdLandCat[scanPtIdx], nlcdLandCatRx, distKm,
                                                            fsplDistKm, win2DistKm, itmStartFreqMHz * 1.0e6, rlanCoord.longitudeDeg,
                                                            rlanCoord.latitudeDeg, rlanHtAboveTerrain, elevationAngleTxDeg, ulsRxLongitude,
                                                            ulsRxLatitude, ulsRxHeightAGL, elevationAngleRxDeg, itmStartPathLoss, pathClutterTxDB,
                                                            pathClutterRxDB, pathLossModelStr, pathLossCDF, pathClutterTxModelStr, pathClutterTxCDF,
                                                            pathClutterRxModelStr, pathClutterRxCDF, &txClutterStr, &rxClutterStr,
                                                            &(uls->ITMHeightProfile), &(uls->isLOSHeightProfile), &(uls->isLOSSurfaceFrac)
#if DEBUG_AFC
                                                                                                                      ,
                                                        ...
                                                        computePathLoss(contains2D ? CConst::FSPLPathLossModel : _pathLossModel, false,
                                                            rlanPropEnv[scanPtIdx], fsPropEnv, rlanNlcdLandCat[scanPtIdx], nlcdLandCatRx, distKm,
                                                            fsplDistKm, win2DistKm, itmStopFreqMHz * 1.0e6, rlanCoord.longitudeDeg,
                                                            rlanCoord.latitudeDeg, rlanHtAboveTerrain, elevationAngleTxDeg, ulsRxLongitude,
                                                            ulsRxLatitude, ulsRxHeightAGL, elevationAngleRxDeg, itmStopPathLoss, pathClutterTxDB,
                                                            pathClutterRxDB, pathLossModelStr, pathLossCDF, pathClutterTxModelStr, pathClutterTxCDF,
                                                            pathClutterRxModelStr, pathClutterRxCDF, &txClutterStr, &rxClutterStr,
                                                            &(uls->ITMHeightProfile), &(uls->isLOSHeightProfile), &(uls->isLOSSurfaceFrac)
#if DEBUG_AFC
                                                        ...
                                                        computePathLoss(contains2D ? CConst::FSPLPathLossModel : _pathLossModel, false,
                                                            rlanPropEnv[scanPtIdx], fsPropEnv, rlanNlcdLandCat[scanPtIdx], nlcdLandCatRx, distKm,
                                                            fsplDistKm, win2DistKm, (itmStartFreqMHz + itmStopFreqMHz) * 0.5e6,
                                                            rlanCoord.longitudeDeg, rlanCoord.latitudeDeg, rlanHtAboveTerrain, elevationAngleTxDeg,
                                                            ulsRxLongitude, ulsRxLatitude, ulsRxHeightAGL, elevationAngleRxDeg, itmStartPathLoss,
                                                            pathClutterTxDB, pathClutterRxDB, pathLossModelStr, pathLossCDF, pathClutterTxModelStr,
                                                            pathClutterTxCDF, pathClutterRxModelStr, pathClutterRxCDF, &txClutterStr, &rxClutterStr,
                                                            &(uls->ITMHeightProfile), &(uls->isLOSHeightProfile), &(uls->isLOSSurfaceFrac)
#if DEBUG_AFC
                                                                                                                      ,
                                                            uls->ITMHeightType
                                    ...
                                    if (traceFlag && (!contains2D)) {
                                        if (uls->ITMHeightProfile) {
                                            ...
                                            for (int ptIdx = 0; ptIdx < N; ptIdx++) {
                                                Vector3 losPathPosn = (((double)(N - 1 - ptIdx)) / (N - 1)) * rlanPosn +
                        ...
                        LOGGER_DEBUG(logger) << "FSID: " << uls->getID() << " DIV_IDX = " << divIdx << " SEG_IDX = " << segIdx
                ...
                for (int segIdx = 0; segIdx < numPR + 1; ++segIdx) {
                    ...
                    for (cvgPhiIdx = 0; cvgPhiIdx < numCvgPoints; ++cvgPhiIdx) {
                        ...
                        Vector3 cvgIntPosn = ulsRxPosn +
                            linkDistKm * (zvec * cos(cvgTheta) + (xvec * cos(cvgPhi) + yvec * sin(cvgPhi)) * sin(cvgTheta));
    ...
    std::cout << "End Processing ULS RX's " << tstr << " Elapsed time = " << (tEndULS - tStartULS) << " sec = " << elapsedTimeDay << " days "
              << elapsedTimeHour << " hours " << elapsedTimeMin << " min " << elapsedTimeSec << " sec." << std::endl
              << std::flush;
...
std::vector<ULSClass *> AfcManager::getSortedUls()
{
    ...
    Vector3 apEcef = EcefModel::fromGeodetic(
        GeodeticCoord::fromLatLon(_rlanRegion->getCenterLatitude(), _rlanRegion->getCenterLongitude(), _rlanRegion->getCenterHeightAMSL() / 1000.));
...
void AfcManager::runScanAnalysis()
{
    ...
    if (_rlanRegion->getMinHeightAGL() < _minRlanHeightAboveTerrain) {
        throw std::runtime_error(ErrStream() << std::string("ERROR: Point Analysis: Invalid RLAN parameter settings.") << std::endl
                                             << std::string("RLAN Min Height above terrain = ") << _rlanRegion->getMinHeightAGL() << std::endl
                                             << std::string("RLAN must be more than ") << _minRlanHeightAboveTerrain << " meters above terrain"
                                             << std::endl);
    }
    ...
    for (scanPtIdx = 0; scanPtIdx < (int)scanPointList.size(); scanPtIdx++) {
        ...
        for (htIdx = 0; htIdx <= 2 * NHt; ++htIdx) {
            rlanCoordList[htIdx] = GeodeticCoord::fromLatLon(
                scanPt.first, scanPt.second, (height0 + (NHt ? (htIdx - NHt) * heightUncertainty / NHt : 0.0)) / 1000.0);
        ...
        for (rlanPosnIdx = 0; rlanPosnIdx < numRlanPosn; ++rlanPosnIdx) {
            ...
            for (ulsIdx = 0; ulsIdx < _ulsList->getSize(); ++ulsIdx) {
                ...
                double slat = sin((lat2Rad - lat1Rad) / 2);
                double slon = sin((lon2Rad - lon1Rad) / 2);
                double groundDistanceKm = 2 * CConst::averageEarthRadius * asin(sqrt(slat * slat + cos(lat1Rad) * cos(lat2Rad) * slon * slon)) *
                    1.0e-3;
                    ...
                    for (auto &channel : _channelList) {
                        for (int freqSegIdx = 0; freqSegIdx < channel.segList.size(); ++freqSegIdx) {
                            if (std::get<2>(channel.segList[freqSegIdx]) != BLACK) {
                                ...
                                CConst::SpectralAlgorithmEnum spectralAlgorithm = (channel.type == INQUIRED_FREQUENCY ? CConst::psdSpectralAlgorithm :
                                                                                                                        _channelResponseAlgorithm);
                                // LOGGER_INFO(logger) << "COMPUTING
                                // SPECTRAL OVERLAP FOR FSID = " <<
                                // uls->getID();
                                double spectralOverlapLossDB;
                                bool hasOverlap = computeSpectralOverlapLoss(&spectralOverlapLossDB, chanStartFreq, chanStopFreq, uls->getStartFreq(),
                                ...
                                if (hasOverlap) {
                                    ...
                                    double buildingPenetrationDB = computeBuildingPenetration(
                                        _buildingType, elevationAngleTxDeg, chanCenterFreq, buildingPenetrationModelStr, buildingPenetrationCDF);
                                    ...
                                    computePathLoss(_pathLossModel, false, rlanPropEnv, fsPropEnv, nlcdLandCatTx, nlcdLandCatRx, distKm, fsplDistKm,
                                        win2DistKm, chanCenterFreq, rlanCoord.longitudeDeg, rlanCoord.latitudeDeg, rlanHtAboveTerrain,
                                        elevationAngleTxDeg, uls->getRxLongitudeDeg(), uls->getRxLatitudeDeg(), uls->getRxHeightAboveTerrain(),
                                        elevationAngleRxDeg, pathLoss, pathClutterTxDB, pathClutterRxDB, pathLossModelStr, pathLossCDF,
                                        pathClutterTxModelStr, pathClutterTxCDF, pathClutterRxModelStr, pathClutterRxCDF, &txClutterStr,
                                        &rxClutterStr, &(uls->ITMHeightProfile), &(uls->isLOSHeightProfile), &(uls->isLOSSurfaceFrac)
#if DEBUG_AFC
                                                                                                                 ,
                                        uls->ITMHeightType
#endif
                                    ...
                                    std::string rxAntennaSubModelStr;
                                    double angleOffBoresightDeg = acos(uls->getAntennaPointing().dot(-(lineOfSightVectorKm.normalized()))) * 180.0 /
                                        M_PI;
                                    double rxGainDB = uls->computeRxGain(
                                        angleOffBoresightDeg, elevationAngleRxDeg, chanCenterFreq, rxAntennaSubModelStr, 0);

                                    double rxPowerDBW = (_maxEIRP_dBm - 30.0) - _bodyLossDB - buildingPenetrationDB - pathLoss - pathClutterTxDB -
                                        pathClutterRxDB + rxGainDB - spectralOverlapLossDB - _polarizationLossDB - uls->getRxAntennaFeederLossDB();
                                    ...
                                    if (eirpLimit_dBm <
                                        std::min(std::get<0>(channel.segList[freqSegIdx]), std::get<1>(channel.segList[freqSegIdx]))) {
                                        std::get<0>(channel.segList[freqSegIdx]) = eirpLimit_dBm;
                                        std::get<1>(channel.segList[freqSegIdx]) = eirpLimit_dBm;
                                    }
                                }
            ...
            fprintf(fscan, "%d,%d,%.6f,%.6f,%.1f,%.1f,%s,%s", scanPtIdx, rlanPosnIdx, rlanCoord.longitudeDeg, rlanCoord.latitudeDeg,
                rlanHtAboveTerrain, rlanTerrainHeight, _terrainDataModel->getSourceName(rlanHeightSource).c_str(),
                CConst::strPropEnvList->type_to_str(rlanPropEnv));

            for (bwIdx = 0; bwIdx < numBW; ++bwIdx) {
                fprintf(fscan, ",%d,%d,%d,%d", numBlack[bwIdx], numRed[bwIdx], numYellow[bwIdx], numGreen[bwIdx]);
            }
            fprintf(fscan, "\n");
            fflush(fscan);
        }
...
void AfcManager::runExclusionZoneAnalysis()
{
    ...
    bool hasOverlap = computeSpectralOverlapLoss(
        &spectralOverlapLossDB, chanStartFreq, chanStopFreq, uls->getStartFreq(), uls->getStopFreq(), useACI, spectralAlgorithm);
    double chanCenterFreq = (chanStartFreq + chanStopFreq) / 2;
    ...
    /******************************************************************************/
    double pathLossDB = (_exclusionZoneRLANEIRPDBm - 30.0) - _bodyLossDB + uls->getRxGain() - spectralOverlapLossDB - _polarizationLossDB -
        uls->getRxAntennaFeederLossDB() - uls->getNoiseLevelDBW() - _IoverN_threshold_dB;
    ...
    for (exclPtIdx = 0; exclPtIdx < numContourPoints; exclPtIdx++) {
        ...
        double d0 = initialD0;
        do {
            margin0 = computeIToNMargin(
                d0, cc, ss, uls, chanCenterFreq, bandwidth, chanStartFreq, chanStopFreq, spectralOverlapLossDB, ulsRxPropEnv, distKm0, "", nullptr);
        ...
        do {
            d1 = d0 * 0.95;
            margin1 = computeIToNMargin(
                d1, cc, ss, uls, chanCenterFreq, bandwidth, chanStartFreq, chanStopFreq, spectralOverlapLossDB, ulsRxPropEnv, distKm1, "", nullptr);
        ...
        if (!minRadiusFlag) {
            ...
            while (d0 - d1 > 1.0e-6) {
                double dm = (d1 + d0) / 2;
                double marginM = computeIToNMargin(dm, cc, ss, uls, chanCenterFreq, bandwidth, chanStartFreq, chanStopFreq, spectralOverlapLossDB,
                    ulsRxPropEnv, distKmM, "", nullptr);
                if (marginM < 0.0) {
                    d1 = dm;
                    margin1 = marginM;
                    distKm1 = distKmM;
        ...
        margin1 = computeIToNMargin(d1, cc, ss, uls, chanCenterFreq, bandwidth, chanStartFreq, chanStopFreq, spectralOverlapLossDB, ulsRxPropEnv,
            distKm1, "Above Thr", &excthrGc);
        margin0 = computeIToNMargin(d0, cc, ss, uls, chanCenterFreq, bandwidth, chanStartFreq, chanStopFreq, spectralOverlapLossDB, ulsRxPropEnv,
            distKm0, "Below Thr", &excthrGc);
...
/******************************************************************************************/
double AfcManager::computeIToNMargin(double d, double cc, double ss, ULSClass *uls, double chanCenterFreq, double bandwidth, double chanStartFreq,
    double chanStopFreq, double spectralOverlapLossDB, char ulsRxPropEnv, double &distKmM, std::string comment, ExThrGzipCsv *excthrGc)
{
    ...
    if (rlanHeight - heightUncertainty - rlanTerrainHeight < _minRlanHeightAboveTerrain) {
        throw std::runtime_error(
            ErrStream() << std::string("ERROR: ItoN: Invalid RLAN parameter settings.") << std::endl
    ...
    for (rlanPosnIdx = 0; rlanPosnIdx < numRlanPosn; ++rlanPosnIdx) {
        ...
        computePathLoss(
            _pathLossModel, false, (rlanPropEnv == CConst::unknownPropEnv ? CConst::barrenPropEnv : rlanPropEnv), fsPropEnv, nlcdLandCatTx,
            nlcdLandCatRx, distKm, fsplDistKm, win2DistKm, chanCenterFreq, rlanCoord.longitudeDeg, rlanCoord.latitudeDeg, rlanHtAboveTerrain,
            elevationAngleTxDeg, uls->getRxLongitudeDeg(), uls->getRxLatitudeDeg(), uls->getRxHeightAboveTerrain(), elevationAngleRxDeg, pathLoss,
            pathClutterTxDB, pathClutterRxDB, pathLossModelStr, pathLossCDF, pathClutterTxModelStr, pathClutterTxCDF, pathClutterRxModelStr,
            pathClutterRxCDF, &txClutterStr, &rxClutterStr, &(uls->ITMHeightProfile), &(uls->isLOSHeightProfile), &(uls->isLOSSurfaceFrac)
#if DEBUG_AFC
                                                                                                                      ,
            uls->ITMHeightType
#endif
        ...
        double rxPowerDBW = (_exclusionZoneRLANEIRPDBm - 30.0) - _bodyLossDB - buildingPenetrationDB - pathLoss - pathClutterTxDB - pathClutterRxDB +
            rxGainDB - spectralOverlapLossDB - _polarizationLossDB - uls->getRxAntennaFeederLossDB();
...
void AfcManager::writeKML()
{
    ...
    fkml->writeTextElement("altitudeMode", "absolute");
    fkml->writeTextElement("coordinates",
        QString::asprintf("%12.10f,%12.10f,%5.3f", uls->getRxLongitudeDeg(), uls->getRxLatitudeDeg(),
...
void AfcManager::runHeatmapAnalysis()
{
    ...
    if (_channelList.size() != 1) {
        throw std::runtime_error(
            ErrStream() << "ERROR: Number of channels = " << _channelList.size() << " require exactly 1 channel for heatmap analysis");
    }
    ...
    _heatmapNumPtsLon = ceil(
        (_heatmapMaxLon - _heatmapMinLon) * M_PI / 180.0 * CConst::earthRadius * cos(minAbsLat * M_PI / 180.0) / _heatmapRLANSpacing);
    ...
    for (lonIdx = 0; lonIdx < _heatmapNumPtsLon; ++lonIdx) {
        ...
        for (latIdx = 0; latIdx < _heatmapNumPtsLat; ++latIdx) {
            double rlanLat = (_heatmapMinLat * (2 * _heatmapNumPtsLat - 2 * latIdx - 1) + _heatmapMaxLat * (2 * latIdx + 1)) /
                (2 * _heatmapNumPtsLat);
            ...
            channel->segList[freqSegIdx] = std::make_tuple(std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity(), GREEN);
                                ...
                                if (hasOverlap) {
                                    ...
                                    if (segIdx == numPR) {
                                        rxGainDB = uls->computeRxGain(
                                            angleOffBoresightDeg, elevationAngleRxDeg, chanCenterFreq, rxAntennaSubModelStr, divIdx);
                                    } else {
                                        discriminationGain = uls->getPR(segIdx).computeDiscriminationGain(
                                            angleOffBoresightDeg, elevationAngleRxDeg, chanCenterFreq, reflectorD0, reflectorD1);
                                        rxGainDB = uls->getPR(segIdx).effectiveGain + discriminationGain;
                                    }
                                    ...
                                    if (segIdx == numPR) {
                                        if (_nearFieldAdjFlag && (distKm * 1000.0 < uls->getRxNearFieldDistLimit()) &&
                                            (angleOffBoresightDeg < 90.0)) {
                                            bool unii5Flag = computeSpectralOverlapLoss((double *)NULL, uls->getStartFreq(), uls->getStopFreq(),
                                    ...
                                    rxPowerDBW = (rlanEIRP_dBm - 30.0) + rlanDiscriminationGainDB - _bodyLossDB - buildingPenetrationDB - pathLoss -
                                        pathClutterTxDB - pathClutterRxDB + rxGainDB + nearFieldOffsetDB - spectralOverlapLossDB -
                                    ...
                                    if (excthrGc && (std::isnan(rxPowerDBW) || (I2NDB > _visibilityThreshold) || (distKm * 1000 < _closeInDist))) {
                                        ...
                                        std::string bldgTypeStr = (_fixedBuildingLossFlag        ? "I"
                                                ...
                                                _buildingType == CConst::noBuildingType          ? "O"
                                                                                                   ...
                                                                                                   "O"
                                                                                                   "R" :
                                                _buildingType == CConst::traditionalBuildingType ? "T"
                                                                                                   "R"
                                                                                                   "A"
                                                                                                   "D"
                                                                                                   "I"
            ...
            if (maxIToNDB == -std::numeric_limits<double>::infinity()) {
                numInvalid++;
                if (numInvalid <= 100) {
                    errStr << "At position LON = " << rlanLon << " LAT = " << rlanLat << " there are no FS receivers within " << (_maxRadius / 1000)
            ...
            std::cout << " [" << numProc << " / " << totNumProc << "] "
                      << " Elapsed Time = " << std::setprecision(6) << std::chrono::duration_cast<std::chrono::duration<double>>(t2 - t1).count()
                      << std::endl
                      << std::flush;
    ...
    if (numInvalid) {
        errStr << "There were a total of " << numInvalid << " RLAN locations for which there are no FS receivers within " << (_maxRadius / 1000)
               << " Km that have nonzero spectral overlap" << std::endl;
        LOGGER_WARN(logger) << errStr.str();
        statusMessageList.push_back(errStr.str());
        errStr.str("");
        errStr.clear();
    ...
    LOGGER_INFO(logger) << "End Processing Heatmap " << tstr << " Elapsed time = " << (tEndHeatmap - tStartHeatmap) << " sec = " << elapsedTimeDay
                        << " days " << elapsedTimeHour << " hours " << elapsedTimeMin << " min " << elapsedTimeSec << " sec.";
    ...
    if (inputGc) {
        ...
        if (_analysisType == "ExclusionZoneAnalysis") {
            ...
            if (_inquiredChannels.size() != 1) {
                throw std::runtime_error(ErrStream() << "ERROR: Number of channels = " << _inquiredChannels.size()
...
CConst::PropEnvEnum AfcManager::computePropEnv(double lonDeg, double latDeg, CConst::NLCDLandCatEnum &nlcdLandCat, bool errorFlag) const
{
    ...
    if ((propEnv == CConst::unknownPropEnv) && (errorFlag)) {
        throw std::runtime_error(ErrStream() << "ERROR: RLAN Location LAT = " << latDeg << " LON = " << lonDeg
...
void AfcManager::createChannelList()
{
    ...
    for (auto &freqRange : _inquiredFrequencyRangesMHz) {
        ...
        if (inquiredStopFreqMHz <= inquiredStartFreqMHz) {
            LOGGER_DEBUG(logger) << "Inquired Freq Range INVALID: [" << inquiredStartFreqMHz << ", " << inquiredStopFreqMHz
    ...
    for (int chanIdx = 0; chanIdx < (int)_channelList.size(); ++chanIdx) {
        ...
        if (channel->type == INQUIRED_FREQUENCY) {
            ...
            for (segIdx = 0; segIdx < channel->segList.size(); ++segIdx) {
                ...
                std::cout << "SEG " << segIdx << ": " << channel->freqMHzList[segIdx] << " - " << channel->freqMHzList[segIdx + 1] << " " << colorStr
                          << std::endl;
            }
#endif
        }
...
void AfcManager::runTestITM(std::string inputFile)
{
    ...
    #if 1
    extern void point_to_point(double elev[], double tht_m, double rht_m, double eps_dielect, double sgm_conductivity, double eno_ns_surfref,
        double frq_mhz, int radio_climate, int pol, double conf, double rel, double &dbloss, std::string &strmode, int &errnum);
    #endif
    ...
    while (fgetline(fin, line, false)) {
        ...
        switch (lineType) {
                ...
                for (fIdx = 0; fIdx < (int)fieldIdxList.size(); fIdx++) {
                    if (*fieldIdxList.at(fIdx) == -1) {
                        errStr << "ERROR: Invalid Winner2 Test Input file "
                                  "\""
                               << inputFile << "\" label line missing \"" << fieldLabelList.at(fIdx) << "\"" << std::endl;
                        throw std::runtime_error(errStr.str());
                    }
                }
            ...
            case dataLineType:
                ...
                if (strval.empty()) {
                    errStr << "ERROR: Invalid ITM Test Input file \"" << inputFile << "\" line " << linenum << " missing Terrain Height (m)"
                           << std::endl;
                    throw std::runtime_error(errStr.str());
                }
    ...
    for (bool reverseFlag : {false}) {
        for (int rlanHeightIdx = 0; rlanHeightIdx < numRlanHeight; ++rlanHeightIdx) {
            double rlanHeightAGL = (rlanHeightAGLStart * (numRlanHeight - 1 - rlanHeightIdx) + rlanHeightAGLStop * rlanHeightIdx) /
                (numRlanHeight - 1);
            ...
            point_to_point(heightProfile, txHeightAGL, rxHeightAGL, eps_dielect, sgm_conductivity, surfaceRefractivity, frequencyMHz, radioClimate,
                pol, conf, rel, pathLoss, strmode, errnum);
    ...
    while (fgetline(fin, line, false)) {
        ...
        switch (lineType) {
            ...
            case dataLineType:
                ...
                if (!validFlag) {
                    errStr << "ERROR: Invalid Winner2 Test Input file \"" << inputFile << "\" line " << linenum << " INVALID REGION = " << strval
                           << std::endl;
                    throw std::runtime_error(errStr.str());
                }
                /**************************************************************************/
                ...
                if (strval.empty()) {
                    errStr << "ERROR: Invalid Winner2 Test Input file \"" << inputFile << "\" line " << linenum << " missing Distance (m)"
                           << std::endl;
                    throw std::runtime_error(errStr.str());
                }
                ...
                if (strval.empty()) {
                    errStr << "ERROR: Invalid Winner2 Test Input file \"" << inputFile << "\" line " << linenum << " missing Frequency (GHz)"
                           << std::endl;
                    throw std::runtime_error(errStr.str());
                }
                ...
                if (propEnv == CConst::urbanPropEnv) {
                    ...
                    plCombined = Winner2_C1suburban(
                        distance, hb, hm, frequency, sigmaCombined, pathLossModelStr, pathLossCDF, probLOS, winner2LOSValue);
...
void AfcManager::runAnalyzeNLCD()
{
    ...
    for (lonRegionIdx = 0; lonRegionIdx < numRegionLon; lonRegionIdx++) {
        ...
        for (latRegionIdx = 0; latRegionIdx < numRegionLat; latRegionIdx++) {
            ...
            std::string pngFile = std::string("/tmp/image") + "_" + std::to_string(lonRegionIdx) + "_" + std::to_string(latRegionIdx) + ".png";
