...
class ResponseInfo
{
	...
	public:
		...
		ResponseInfo(CConst::ResponseCodeEnum responseCode) : _responseCode(responseCode)
		{
			switch (responseCode) {
				case CConst::generalFailureResponseCode:
					_shortDescription = "General Failure";
					break;
				case CConst::successResponseCode:
					_shortDescription = "Success";
					break;
				case CConst::versionNotSupportedResponseCode:
					_shortDescription = "Version Not Supported";
					break;
				case CConst::deviceDisallowedResponseCode:
					_shortDescription = "Device Disallowed";
					break;
				case CConst::missingParamResponseCode:
					_shortDescription = "Missing Param";
					break;
				case CConst::invalidValueResponseCode:
					_shortDescription = "Invalid Value";
					break;
				case CConst::unexpectedParamResponseCode:
					_shortDescription = "Unexpected Param";
					break;
				case CConst::unsupportedSpectrumResponseCode:
					_shortDescription = "Unsupported Spectrum";
					break;
				default:
					CORE_DUMP;
					break;
		...
		ResponseInfo(CConst::ResponseCodeEnum responseCode,
			     const std::ostringstream &shortDescription) :
		...
		/** responseCode value */
		CConst::ResponseCodeEnum responseCode() const
		{
		...
		bool success() const
		{
			return _responseCode == CConst::successResponseCode;
		}

		/** True id supplementalInfo object should be created */
		bool hasSupplementalInfoFlag() const
		{
			return std::set<CConst::ResponseCodeEnum>(
...
/*static*/ bool AfcManager::_inFiniteRange(double value, double minVal, double maxVal)
{
	return std::isfinite(value) && value >= minVal && value <= maxVal;
}
...
void AfcManager::initializeDatabases()
{
	...
	if (_analysisType == "AP-AFC" || _analysisType == "ScanAnalysis" ||
		...
		// Clamp so cos() stays positive; beyond this the bounding-box already
		// spans the continent and further expansion adds no geometric benefit.
		maxAbsLat = std::min(maxAbsLat, 89.0);
		...
		if (_pathLossModel == CConst::FCC6GHzReportAndOrderPathLossModel) {
			...
			maxAbsLatBldg = std::min(maxAbsLatBldg, 89.0);
		...
		// Guard against an FSID that yields no FS rows: indexing (*_ulsList)[0]
		// on an empty ListClass reads past a zero-size allocation and
		// dereferences garbage → SIGSEGV. Reject with a structured error.
		if (_ulsList->getSize() == 0) {
			throw std::runtime_error(ErrStream() << "ExclusionZoneAnalysis: no FS "
								"record found for FSID "
							     << _exclusionZoneFSID);
		} else if (_ulsList->getSize() > 1) {
			throw std::runtime_error(ErrStream() << "ExclusionZoneAnalysis: multiple "
								"FS records found for FSID "
							     << _exclusionZoneFSID);
		}
...
void AfcManager::importGUIjson(const std::string &inputJSONpath)
{
	...
	if (_guiJsonVersion == "1.4") {
		...
		_responseInfo.reset(
			new ResponseInfo(CConst::versionNotSupportedResponseCode,
					 std::ostringstream()
						 << "VERSION NOT SUPPORTED: GUI JSON FILE \""
						 << inputJSONpath
...
void AfcManager::importGUIjsonVersion1_4(const QJsonObject &jsonObj)
{
	...
	if ((_analysisType == "AP-AFC") || (_analysisType == "ScanAnalysis") ||
		...
		if (!requiredParams.contains("availableSpectrumInquiryRequests")) {
			...
			if (requestArray.size() != 1) {
				_responseInfo.reset(new ResponseInfo(
					...
					std::ostringstream()
						<< "GENERAL FAILURE: afc-engine only processes a "
						   "single request, "
		...
		if (!optionalParams.contains("vendorExtensions")) {
			...
			// Cap vendorExtensions to prevent per-element filesystem I/O amplification.
			if (vendorExtensionArray.size() > 16) {
				LOGGER_WARN(logger) << "vendorExtensions array size "
						    << vendorExtensionArray.size()
						    << " exceeds limit of 16; truncating";
				vendorExtensionArray = QJsonArray();
			}
		}
		...
		/**********************************************************************/
		if (certificationIdArray.size() > 8) {
		...
		if (n != 1) {
			_responseInfo.reset(new ResponseInfo(
				CConst::generalFailureResponseCode,
				std::ostringstream() << "GENERAL FAILURE: location object must "
							"contain exactly one instance of ellipse, "
							"linearPolygon, or radialPolygon, total of "
						     << n << " instances found"));
			return;
		}
		...
		if (hasLinearPolygonFlag) {
			...
			for (auto &key : linearPolygonObj.keys()) {
				if (linearPolygonObj[key].isNull()) {
					continue;
				}
				int rIdx = requiredParams.indexOf(key);
				if (rIdx != -1) {
		...
		if (hasRadialPolygonFlag) {
			...
			for (auto &key : radialPolygonObj.keys()) {
				if (radialPolygonObj[key].isNull()) {
					continue;
				}
				int rIdx = requiredParams.indexOf(key);
				if (rIdx != -1) {
		...
		if (hasOuterBoundaryPointArrayFlag) {
			if (outerBoundaryPointArray.size() > 15) {
				_invalidParams << "outerBoundary";
				_responseInfo.reset(
					new ResponseInfo(CConst::invalidValueResponseCode));
				return;
			}
		...
		if (hasOuterBoundaryVectorArrayFlag) {
			if (outerBoundaryVectorArray.size() > 15) {
				_invalidParams << "outerBoundary";
				_responseInfo.reset(
					new ResponseInfo(CConst::invalidValueResponseCode));
				return;
		...
		/**********************************************************************/
		if (inquiredFrequencyRangeArray.size() > 256) {
			_invalidParams << "inquiredFrequencyRange";
			_responseInfo.reset(new ResponseInfo(CConst::invalidValueResponseCode));
			return;
		}
		...
		/**********************************************************************/
		if (inquiredChannelsArray.size() > 256) {
			_invalidParams << "inquiredChannels";
			_responseInfo.reset(new ResponseInfo(CConst::invalidValueResponseCode));
			return;
		}
		...
		if (hasIndoorDeploymentFlag) {
			...
			switch (indoorDeploymentVal) {
				case 0: /* Unknown */
					...
					break;
				case 1: /* Indoor */
					/* enforce server-derived _certifiedIndoor
					 * unconditionally; afc_server already encodes the
					 * per-ruleset indoor decision in RUNTIME_OPT_CERT_ID,
					 * so do not key on AP-supplied _rulesetId here. */
					if (_certifiedIndoor) {
						_rlanType = RLANType::RLAN_INDOOR;
					} else {
						LOGGER_INFO(logger)
		...
		// _inFiniteRange includes the isfinite check so NaN/Inf values are
		// caught before the range comparison — NaN would silently pass a
		// plain < 0.0 guard (NaN comparisons always return false).
		if (elevationObj["verticalUncertainty"].isNull() ||
		    !elevationObj["verticalUncertainty"].isDouble() ||
		    !_inFiniteRange(verticalUncertainty, 0.0, _maxVerticalUncertainty)) {
			LOGGER_WARN(logger)
				<< "GENERAL FAILURE: verticalUncertainty = " << verticalUncertainty
				<< " is not finite or outside [0, " << _maxVerticalUncertainty
				<< "] m";
			_invalidParams << "verticalUncertainty";
		}
		double centerHeight = elevationObj["height"].toDouble();
		// Reject non-finite or out-of-range antenna heights.
		// Range: -500 m (below sea level) to 100 000 m (well above any AP).
		// Extreme values cause NaN/Inf in ECEF geometry and fail-open path-loss.
		if (elevationObj["height"].isNull() || !elevationObj["height"].isDouble() ||
		    !std::isfinite(centerHeight) || centerHeight < -500.0 ||
		    centerHeight > 100000.0) {
			LOGGER_WARN(logger)
				<< "GENERAL FAILURE: elevation.height = " << centerHeight
				<< " is not finite or outside [-500, 100000] m";
			_invalidParams << "height";
		}
		...
		if (hasEllipseFlag) {
			...
			// _inFiniteRange guards include isfinite — NaN lat/lon would silently
			// pass a plain range comparison and propagate into ECEF geometry.
			if (!_inFiniteRange(centerLatitude, -90.0, 90.0)) {
				_invalidParams << "latitude";
			}
			// Reject polar latitudes (|lat| >= 89.9°): planar small-angle
			// geometry degenerates near the poles (cos(lat) → 0), so these
			// values produce geometrically invalid results.
			if (_inFiniteRange(centerLatitude, -90.0, 90.0) &&
			    ((centerLatitude >= 89.9) || (centerLatitude <= -89.9))) {
				_invalidParams << "latitude";
			}
			if (!_inFiniteRange(centerLongitude, -180.0, 180.0)) {
				_invalidParams << "longitude";
			}

			// A zero axis means "point location / no uncertainty" per the spec.
			// Clamp to a small positive epsilon so downstream computations do not
			// divide by zero or produce Inf/NaN.
			const double AXIS_EPSILON_M = 1e-3;
			if (minorAxis == 0.0) {
				minorAxis = AXIS_EPSILON_M;
			}
			if (majorAxis == 0.0) {
				majorAxis = AXIS_EPSILON_M;
			}

			// Non-finite axes must be rejected explicitly: a NaN minorAxis/
			// majorAxis makes every plain relational comparison below false,
			// silently passing validation (matching the isfinite convention
			// already applied to latitude/longitude/height/verticalUncertainty
			// above).
			if (!std::isfinite(minorAxis) || !std::isfinite(majorAxis)) {
				LOGGER_WARN(logger)
					<< "GENERAL FAILURE: minorAxis = " << minorAxis
					<< ", majorAxis = " << majorAxis << " is not finite";
			...
			// _inFiniteRange includes the isfinite check so a NaN orientation
			// cannot silently pass the plain range guard below.
			if (!_inFiniteRange(orientation, 0.0, 180.0)) {
			...
			for (auto outerBoundaryPointVal : outerBoundaryPointArray) {
				...
				if (!_inFiniteRange(latitude, -90.0, 90.0)) {
					_invalidParams << "latitude";
				}
				// Reject polar vertex latitudes: planar geometry degenerates near
				// poles.
				if ((latitude >= 89.9) || (latitude <= -89.9)) {
					_invalidParams << "latitude";
				}
				if (!_inFiniteRange(longitude, -180.0, 180.0)) {
					_invalidParams << "longitude";
				}
			...
			for (auto outerBoundaryVectorVal : outerBoundaryVectorArray) {
				...
				if (!_inFiniteRange(length,
						    0.0,
						    _maxHorizontalUncertaintyDistance)) {
					_invalidParams << "length";
				}
				if (!_inFiniteRange(angle, 0.0, 360.0)) {
			...
			// Validate center lat/lon ranges for radialPolygon.
			// Use _inFiniteRange so NaN inputs (from e.g. 1e400 in JSON)
			// are caught: bare > / < comparisons return false for NaN.
			if (!_inFiniteRange(centerLatitude, -90.0, 90.0)) {
				_invalidParams << "centerLatitude";
			}
			// Reject polar latitudes for radialPolygon: planar geometry invalid near
			// poles.
			if ((centerLatitude >= 89.9) || (centerLatitude <= -89.9)) {
				_invalidParams << "centerLatitude";
			}
			if (!_inFiniteRange(centerLongitude, -180.0, 180.0)) {
				_invalidParams << "centerLongitude";
			}

			double maxHorizDistSq = 0.0;
			for (int i = 0; i < (int)_rlanRadialPolygon.size(); i++) {
				double angle_i = _rlanRadialPolygon[i].first; // .first = angle
				double length_i = _rlanRadialPolygon[i].second; // .second = length
				// Enforce per-vertex upper bound independent of pair-wise check
				if (length_i > _maxHorizontalUncertaintyDistance) {
					LOGGER_WARN(logger)
						<< "GENERAL FAILURE: Radial polygon vertex length "
						<< length_i << " m exceeds max value of "
						<< _maxHorizontalUncertaintyDistance;
					_invalidParams << "outerBoundary";
				}
				double x_i = length_i * sin(angle_i * M_PI / 180.0);
				double y_i = length_i * cos(angle_i * M_PI / 180.0);
				for (int j = i + 1; j < (int)_rlanRadialPolygon.size() + 1; j++) {
					double jj = j % _rlanRadialPolygon.size();
					double angle_j =
						_rlanRadialPolygon[jj].first; // .first = angle
					double length_j =
						_rlanRadialPolygon[jj].second; // .second = length
					double x_j = length_j * sin(angle_j * M_PI / 180.0);
					double y_j = length_j * cos(angle_j * M_PI / 180.0);
		...
		// Cap inquiredChannels array (and each nested channelCfi sub-array) to
		// bound CPU/memory amplification, matching the inquiredFrequencyRange
		// guard below. Without this, N duplicate operating-class
		// entries multiply into ~59N ChannelStruct objects in
		// createChannelList().
		const int maxInquiredChannels = 256;
		const int maxChannelCfi = 512;
		if (inquiredChannelsArray.size() > maxInquiredChannels) {
			_responseInfo.reset(new ResponseInfo(CConst::unexpectedParamResponseCode,
							     std::ostringstream()
								     << "inquiredChannels "
									"array exceeds maximum "
									"allowed size of "
								     << maxInquiredChannels));
			return;
		}
		...
		for (auto inquiredChannelsVal : inquiredChannelsArray) {
			...
			if (inquiredChannelsObj.contains("channelCfi")) {
				auto cfiArray = inquiredChannelsObj["channelCfi"].toArray();
				if (cfiArray.size() > maxChannelCfi) {
					_responseInfo.reset(new ResponseInfo(
						CConst::unexpectedParamResponseCode,
						std::ostringstream()
							<< "channelCfi array exceeds maximum "
							   "allowed size of "
							<< maxChannelCfi));
					return;
				}
				for (const QJsonValue &chanIdx : cfiArray)
		...
		// Cap inquiredFrequencyRange array to bound processing time
		const int maxFreqRanges = 256;
		if (inquiredFrequencyRangeArray.size() > maxFreqRanges) {
			_responseInfo.reset(new ResponseInfo(CConst::unexpectedParamResponseCode,
							     std::ostringstream()
		...
		for (auto vendorExtensionVal : vendorExtensionArray) {
			...
			if (parametersObj.contains("type") &&
				...
				if (type == "rlanAntenna") {
					if (hasRLANAntenna) {
						_responseInfo.reset(new ResponseInfo(
							CConst::generalFailureResponseCode,
							"GENERAL FAILURE: multiple RLAN antennas "
							"specified"));
						return;
					}
					...
					if (!_inFiniteRange(_rlanAzimuthPointing, 0.0, 360.0)) {
						_responseInfo.reset(new ResponseInfo(
							CConst::generalFailureResponseCode,
							std::ostringstream()
								<< "GENERAL FAILURE: "
					...
					if (!_inFiniteRange(_rlanElevationPointing, -90.0, 90.0)) {
						_responseInfo.reset(new ResponseInfo(
							CConst::generalFailureResponseCode,
							std::ostringstream()
								<< "GENERAL FAILURE: "
					...
					if (numAntennaAOB < 2 || numAntennaAOB > 3601) {
						_responseInfo.reset(new ResponseInfo(
							CConst::generalFailureResponseCode,
							std::ostringstream()
					...
					// Clamp discrimination gain to [−6, 0] dB to prevent
					// self-attested antenna patterns from defeating FCC I/N
					// limits. The FCC standard-power AP omnidirectional
					// assumption bounds off-axis discrimination at no worse
					// than −6 dB.
					const double RLAN_DISC_GAIN_MIN_DB = -6.0;
					const double RLAN_DISC_GAIN_MAX_DB = 0.0;
					for (int aobIdx = 0; aobIdx < numAntennaAOB; ++aobIdx) {
						std::get<0>(pt) = (((double)aobIdx) /
								   (numAntennaAOB - 1)) *
								  M_PI;
						double rawGain = gainArray.at(aobIdx).toDouble();
						if (rawGain < RLAN_DISC_GAIN_MIN_DB)
							rawGain = RLAN_DISC_GAIN_MIN_DB;
						if (rawGain > RLAN_DISC_GAIN_MAX_DB)
							rawGain = RLAN_DISC_GAIN_MAX_DB;
						std::get<1>(pt) = rawGain;
					...
					if (_heatmapMaxLat <= _heatmapMinLat ||
					    _heatmapMaxLon <= _heatmapMinLon ||
					    _heatmapMinLat < -90.0 || _heatmapMaxLat > 90.0 ||
					    _heatmapMinLon < -180.0 || _heatmapMaxLon > 180.0 ||
					    !(_heatmapRLANSpacing >= 1.0) ||
					    std::isnan(_heatmapMinLat) ||
					    std::isnan(_heatmapMaxLat) ||
					    std::isnan(_heatmapMinLon) ||
					    std::isnan(_heatmapMaxLon)) {
						_responseInfo.reset(new ResponseInfo(
							CConst::generalFailureResponseCode,
							std::ostringstream()
								<< "GENERAL FAILURE: Invalid "
								   "heatmap bounds or spacing"));
						return;
					}
					...
					if (indoorFlag) {
						...
						if (!optionalParams.contains("RLANIndoorHeightUncer"
							...
							_responseInfo.reset(new ResponseInfo(
								CConst::generalFailureResponseCode,
								"GENERAL FAILURE: heatmap "
								"RLANIndoorHeightUncertainty not "
								"specified"));
							return;
						}
					...
					if (outdoorFlag) {
						...
						if (!optionalParams.contains("RLANOutdoorHeight")) {
							...
							_responseInfo.reset(new ResponseInfo(
								CConst::generalFailureResponseCode,
								"GENERAL FAILURE: heatmap "
								"RLANOutdoorHeight not specified"));
							return;
						}
						if (!optionalParams.contains("RLANOutdoorHeightUnce"
							...
							_responseInfo.reset(new ResponseInfo(
								CConst::generalFailureResponseCode,
								"GENERAL FAILURE: heatmap "
								"RLANOutdoorHeightUncertainty not "
								"specified"));
							return;
						}
					...
					if (fsidTypeStr == "All") {
						...
						if (!optionalParams.contains("fsId")) {
							...
							_responseInfo.reset(new ResponseInfo(
								CConst::generalFailureResponseCode,
								"GENERAL FAILURE: heatmap fsId not "
								"specified"));
							return;
						}
					} else {
					...
					// Honour heatmap vendorExtension only when the
					// engine was already launched as HeatmapAnalysis by the
					// operator; reject the attempt to escalate an AP-AFC
					// request to the expensive heatmap compute path.
					if (_analysisType != "HeatmapAnalysis") {
						_responseInfo.reset(new ResponseInfo(
							CConst::unexpectedParamResponseCode,
							"vendorExtension type 'heatmap' not "
							"permitted for AP-AFC requests"));
						return;
					}
			...
			if (extensionId == "openAfc.overrideAfcConfig") {
				// Removed: per-request override of the operator FS database.
				// The FS/RAS database selection must originate from the
				// operator config only (importConfigAFCjson).
				LOGGER_WARN(logger)
					<< "openAfc.overrideAfcConfig vendorExtension is "
					<< "no longer supported; ignoring.";
			}
		}
		...
		if (_inquiredChannels.size() + _inquiredFrequencyRangesMHz.size() == 0) {
			_responseInfo.reset(new ResponseInfo(CConst::generalFailureResponseCode,
							     "GENERAL FAILURE: must specify either "
							     "inquiredChannels or "
							     "inquiredFrequencies"));
			return;
		}
		...
		_exclusionZoneRLANEIRPDBm = jsonObj.contains("EIRP") ? jsonObj["EIRP"].toDouble() :
								       0.0;
		...
		double heightUncertainty = jsonObj.contains("heightUncertainty") ?
						   jsonObj["heightUncertainty"].toDouble() :
						   0.0;
		// Store height in _rlanLLA; lat/lon are not used for ExclusionZone (derived from FS
		// location)
		_rlanLLA = std::make_tuple(quietNaN, quietNaN, height);
		_rlanUncerts_m = std::make_tuple(quietNaN, quietNaN, heightUncertainty);

		QString htType = jsonObj.contains("heightType") ? jsonObj["heightType"].toString() :
								  "AGL";
		if (htType == "AMSL") {
			_rlanHeightType = CConst::AMSLHeightType;
		} else {
		...
		QString indoorOutdoor = jsonObj.contains("indoorOutdoor") ?
						jsonObj["indoorOutdoor"].toString() :
						"Outdoor";
		...
		// Set up a frequency range from centerFrequency +/- bandwidth/2 for channel
		// computation
		double centerFreqMHz = jsonObj.contains("centerFrequency") ?
					       jsonObj["centerFrequency"].toDouble() :
					       0.0;
		double bwMHz = jsonObj.contains("bandwidth") ? jsonObj["bandwidth"].toDouble() :
							       0.0;
		if (centerFreqMHz <= 0.0 || bwMHz <= 0.0) {
			_missingParams << "centerFrequency"
				       << "bandwidth";
			_responseInfo.reset(new ResponseInfo(CConst::missingParamResponseCode));
			return;
		}
...
void AfcManager::importConfigAFCjson(const std::string &inputJSONpath, const std::string &tempDir)
{
	...
	if (jsonObj.contains("fsAnalysisListFile") &&
		...
		QString fsAnalysisName = jsonObj["fsAnalysisListFile"].toString();
		if (fsAnalysisName.contains('/') || fsAnalysisName.contains('\\') ||
		    fsAnalysisName.contains("..")) {
			throw std::runtime_error(
				ErrStream()
				<< "AfcManager::importConfigAFCjson(): fsAnalysisListFile "
				   "must be a bare filename (no path separators or '..'): "
				<< fsAnalysisName.toStdString());
		}
	...
	// Range-validate the ITM profile parameters: minSpacing divides a
	// distance (zero/negative/tiny values yield a negative or multi-billion
	// point count) and maxPoints bounds the elevation-profile allocation in
	// computeElevationVector(). Negated in-range test so NaN also rejects.
	if (!((_itmMinSpacing >= 1.0) && (_itmMinSpacing <= 1000.0))) {
		throw std::runtime_error("AfcManager::importConfigAFCjson(): Invalid "
					 "ITMParameters.minSpacing specified, must "
					 "be in [1.0, 1000.0].");
	}
	if ((_itmMaxNumPts < 2) || (_itmMaxNumPts > 10000)) {
		throw std::runtime_error("AfcManager::importConfigAFCjson(): Invalid "
					 "ITMParameters.maxPoints specified, must "
					 "be in [2, 10000].");
	}
	// ***********************************
	...
	switch (_scanRegionMethod) {
		...
		case CConst::xyAlignRegionMajorMinorScanRegionMethod:
			...
			// Range-validate the scan spacing: tiny/negative values
			// explode the scan-point count. Negated in-range test so
			// NaN also rejects.
			if (!((_scanres_xy >= 0.1) && (_scanres_xy <= 1000.0))) {
				throw std::runtime_error(
					"AfcManager::importConfigAFCjson(): Invalid "
					"APUncertainty.horizontal specified, must "
					"be in [0.1, 1000.0].");
			}
			break;
		case CConst::latLonAlignGridScanRegionMethod:
			...
			// Range-validate: huge values explode the scan-point count.
			if ((_scanres_points_per_degree < 1) ||
			    (_scanres_points_per_degree > 36000)) {
				throw std::runtime_error(
					"AfcManager::importConfigAFCjson(): Invalid "
					"APUncertainty.points_per_degree specified, "
					"must be in [1, 36000].");
			}
			break;
		default:
	...
	// Range-validate the vertical scan spacing: tiny/negative values
	// explode the scan-point count. Negated in-range test so NaN also
	// rejects.
	if (!((_scanres_ht >= 0.1) && (_scanres_ht <= 100.0))) {
		throw std::runtime_error("AfcManager::importConfigAFCjson(): Invalid "
					 "APUncertainty.height specified, must "
					 "be in [0.1, 100.0].");
	}
...
OGRLayer *AfcManager::createGeoJSONLayer(const char *tmpPath, GDALDataset **dataSet)
{
	...
	if (!driver) {
		throw std::runtime_error("GDAL OGR driver 'GeoJSON' not available after "
					 "GDALAllRegister()");
	}
	...
	for (dbIdx = 0; dbIdx < (int)ulsDatabaseList.size(); ++dbIdx) {
		...
		if (_analysisType == "ExclusionZoneAnalysis") {
			...
			if (rows.size() == 0) {
				_responseInfo.reset(new ResponseInfo(
					CConst::generalFailureResponseCode,
					std::ostringstream() << "GENERAL FAILURE: invalid FSID "
								"specified for heatmap analysis: "
							     << _heatmapFSID));
			}
		} else {
		...
		for (UlsRecord row : rows) {
				...
				if (!ignoreFlag) {
					if (rxGain > 80.0) {
						if (fixAnomalousEntries) {
							...
							// Reject instead of warn-and-keep: an
							// anomalous rxGain drives an uncapped
							// exponential sweep in
							// ULSClass::computeBeamWidth
							reasonIgnored = "Ignored: rxGain > 80 dB";
							ignoreFlag = true;
							numIgnoreInvalid++;
							LOGGER_WARN(logger)
								<< "WARNING: ULS data for FSID = "
								<< fsid << ", rxGain = " << rxGain
								<< " is > 80.0, entry ignored";
						}
					}
				}
			}
			...
			if (!ignoreFlag) {
				...
				if (std::isnan(rxDlambda)) {
					// Missing rx antenna diameter in ULS data: estimate
					// D/lambda from gain (ITU-R F.699/F.1245) so NaN never
					// reaches the antenna gain calculation.
					rxDlambda = pow(10.0, (rxGain - 7.7) / 20.0);
				}
				...
				if (hasDiversity) {
					...
					if (std::isnan(diversityDlambda)) {
						diversityDlambda = pow(10.0,
								       (diversityGain - 7.7) /
									       20.0);
					}
				...
				for (prIdx = 0; prIdx < numPR; ++prIdx) {
					...
					if (row.prType[prIdx] == "Ant") {
						...
						// Unknown prType in ULS data: reject via exception
						// (converted to an error response), not process
						// abort.
						throw std::runtime_error("ULS data: unknown "
									 "passive repeater type "
									 "\"" +
									 row.prType[prIdx] + "\"");
					}
				}
...
void AfcManager::readDeniedRegionData(std::string filename)
{
	...
	int lat1Rect1FieldIdx = -1;
	int lat2Rect1FieldIdx = -1;
	int lon1Rect1FieldIdx = -1;
	int lon2Rect1FieldIdx = -1;

	int lat1Rect2FieldIdx = -1;
	int lat2Rect2FieldIdx = -1;
	int lon1Rect2FieldIdx = -1;
	int lon2Rect2FieldIdx = -1;

	int radiusFieldIdx = -1;
	int latCircleFieldIdx = -1;
	int lonCircleFieldIdx = -1;

	int heightAGLFieldIdx = -1;
...
void AfcManager::runPointAnalysis()
{
	...
	// Guard against oversized scan regions: reject rather than crash with
	// a stack-VLA overflow (SIGSEGV) when scanPointList is large.
	const std::size_t maxScanPoints = 50000;
	if (scanPointList.size() > maxScanPoints) {
		_responseInfo.reset(
			new ResponseInfo(CConst::generalFailureResponseCode,
					 std::ostringstream()
						 << "GENERAL FAILURE: scan region too large ("
						 << scanPointList.size() << " points, max "
						 << maxScanPoints << ")"));
		return;
	}

	const std::size_t nScan = scanPointList.size();
	const std::size_t nHt = static_cast<std::size_t>(2 * NHt + 1);

	// Guard the flat vector allocation: even with maxScanPoints enforced,
	// an extreme NHt (from operator-configured maxVerticalUncertainty /
	// scanres_ht) could produce a nScan*nHt product large enough to exhaust
	// heap.  5 M combined scan×height slots is well above any realistic
	// antenna-height sweep.
	const std::size_t maxScanAlloc = 5000000UL;
	if (nScan * nHt > maxScanAlloc) {
		_responseInfo.reset(new ResponseInfo(CConst::generalFailureResponseCode,
						     std::ostringstream()
							     << "GENERAL FAILURE: scan×height "
	...
	std::vector<Vector3> rlanPosnListFlat(nScan * nHt);
	std::vector<GeodeticCoord> rlanCoordListFlat(nScan * nHt);
	std::vector<int> numRlanHt(nScan);
	std::vector<double> rlanTerrainHeight(nScan);
	std::vector<CConst::HeightSourceEnum> rlanHeightSource(nScan);
	std::vector<CConst::PropEnvEnum> rlanPropEnv(nScan);
	std::vector<CConst::NLCDLandCatEnum> rlanNlcdLandCat(nScan);

	// Helpers to index the 2-D arrays stored in flat vectors.
	auto rlanPosnList = [&](std::size_t s, std::size_t h) -> Vector3 & {
		return rlanPosnListFlat[s * nHt + h];
	};
	auto rlanCoordList = [&](std::size_t s, std::size_t h) -> GeodeticCoord & {
		return rlanCoordListFlat[s * nHt + h];
	};
	...
	for (scanPtIdx = 0; scanPtIdx < (int)scanPointList.size(); scanPtIdx++) {
		...
		for (htIdx = 0; (htIdx <= 2 * NHt) && (!lowHeightFlag); ++htIdx) {
			...
			if (useFlag) {
				rlanCoordList(scanPtIdx,
					      htIdx) = GeodeticCoord::fromLatLon(scanPt.first,
										 scanPt.second,
										 heightAMSL /
											 1000.0);
	...
	/**************************************************************************************/
	/* Fail closed when no RLAN scan position survived the height-column build (e.g.      */
	/* scanPointBelowGroundMethod "discard" with an entirely below-ground uncertainty     */
	/* region): the channel lists were pre-initialized to max EIRP / GREEN, so            */
	/* proceeding with zero analyzed heights would return an unanalyzed full-power        */
	/* grant.  This check is intentionally independent of                                 */
	/* _reportErrorRlanHeightLowFlag and of RlanRegionClass::getMaxHeightAGL() (whose     */
	/* return value is clamped up to _minRlanHeightAboveTerrain).                         */
	/**************************************************************************************/
	int totalNumRlanHt = 0;
	for (scanPtIdx = 0; scanPtIdx < (int)scanPointList.size(); scanPtIdx++) {
		totalNumRlanHt += numRlanHt[scanPtIdx];
	}
	if (totalNumRlanHt == 0) {
		_responseInfo.reset(new ResponseInfo(
			CConst::invalidValueResponseCode,
			std::ostringstream()
				...
				<< "No valid RLAN scan position: every candidate height is "
				<< "below the minimum height above terrain of "
				<< _minRlanHeightAboveTerrain << " m"));
		_invalidParams << "heightType"
			       << "height"
			       << "verticalUncertainty";
		return;
	...
	if ((_rlanRegion->getMaxHeightAGL() < _minRlanHeightAboveTerrain) &&
		...
		_responseInfo.reset(new ResponseInfo(
			...
			std::ostringstream()
				<< "ERROR: Point Analysis: Invalid RLAN parameter settings.\n"
				<< "RLAN uncertainty region has a max AGL height of "
				<< _rlanRegion->getMaxHeightAGL() << ", which is < "
				<< _minRlanHeightAboveTerrain));
		_invalidParams << "heightType"
			       << "height"
			       << "verticalUncertainty";
										...
										while (contFlag) {
											...
											if ((chanColor !=
												...
												if (hasOverlap) {
													...
													if (!skip) {
														// Guard: a NaN per-FS EIRP limit fails
														// every '<' comparison below, silently
														// dropping this FS constraint (fail-
														// open). Treat NaN as full deny (-inf)
														// so a non-finite path loss can never
														// relax the limit.
														for (int bandEdgeIdx =
															     0;
														     bandEdgeIdx <
											...
											if (state ==
												...
												if (freqSegIdx ==
													...
													if ((numItmSeg >
														...
														itmSegIdx =
															0;
														if (itmPathLossModelStrChk !=
														    itmPathLossModelStr) {
															LOGGER_WARN(
																logger)
																<< "WARNING: Different path loss models for interp: = "
																<< itmPathLossModelStr
																<< " and "
																<< itmPathLossModelStrChk;
															itmPathLossModelStr =
...
void AfcManager::runScanAnalysis()
{
	...
	FILE *fscan;
	std::string rawSerial = _serialNumber.toStdString();
	std::string safeSerial;
	for (char c : rawSerial) {
		if (std::isalnum((unsigned char)c) || c == '_' || c == '-') {
			safeSerial.push_back(c);
		}
		if (safeSerial.size() >= 64) {
			break;
		}
	}
	if (safeSerial.empty()) {
	...
	std::string scanFileName = QDir(QString::fromStdString(_tempDir))
					   .filePath(QString::fromStdString(safeSerial + "_scan."
											 "csv"))
					   .toStdString();
	...
	double heightUncertainty = _rlanRegion->getHeightUncertainty();
	int NHt = (int)ceil(heightUncertainty / _scanres_ht);
	std::vector<Vector3> rlanPosnList(2 * NHt + 1);
	std::vector<GeodeticCoord> rlanCoordList(2 * NHt + 1);

	int bwIdx;
	std::vector<int> numBlack(numBW, 0);
	std::vector<int> numGreen(numBW, 0);
	std::vector<int> numYellow(numBW, 0);
	std::vector<int> numRed(numBW, 0);
	...
	// Guard against oversized scan regions: runScanAnalysis may use a finer
	// scan resolution than the main AFC path; cap the count to avoid heap
	// exhaustion from a large polygon at 1-arcsec (≈30 m) resolution.
	{
		const std::size_t maxScanAnalysisPoints = 50000;
		if (scanPointList.size() > maxScanAnalysisPoints) {
			_responseInfo.reset(
				new ResponseInfo(CConst::generalFailureResponseCode,
						 std::ostringstream()
							 << "GENERAL FAILURE: scan analysis region "
							    "too large ("
							 << scanPointList.size() << " points, max "
							 << maxScanAnalysisPoints << ")"));
			fclose(fscan);
			return;
		}
	}
	...
	/* Create KML file                                                                    */
	/**************************************************************************************/
	std::string kmzFileName = QDir(QString::fromStdString(_tempDir))
					  .filePath(QString::fromStdString(safeSerial + "_scan."
											"kmz"))
					  .toStdString();
...
void AfcManager::runHeatmapAnalysis()
{
	...
	if (!(_heatmapNumPtsLon > 0) || !(_heatmapNumPtsLat > 0) || !(_heatmapNumPtsLon <= 1000) ||
	    !(_heatmapNumPtsLat <= 1000) ||
	    (static_cast<long long>(_heatmapNumPtsLon) * _heatmapNumPtsLat > 1000000LL)) {
		LOGGER_ERROR(logger)
			<< "Heatmap dimensions too large or invalid. LON: " << _heatmapNumPtsLon
			<< ", LAT: " << _heatmapNumPtsLat;

		_responseInfo.reset(new ResponseInfo(CConst::generalFailureResponseCode,
						     "GENERAL FAILURE: Invalid heatmap "
						     "dimensions"));
		return;
	}
	...
	if ((!_heatmapIToNDB) || (!_heatmapIsIndoor)) {
		throw std::runtime_error(ErrStream() << "ERROR: Unable to allocate heatmap arrays");
	}
	...
	for (lonIdx = 0; lonIdx < _heatmapNumPtsLon; ++lonIdx) {
		...
		if ((!_heatmapIToNDB[lonIdx]) || (!_heatmapIsIndoor[lonIdx])) {
			throw std::runtime_error(ErrStream()
						 << "ERROR: Unable to allocate heatmap arrays");
		}
	}
	/**************************************************************************************/
	...
	/**************************************************************************************/
	// Use per-request tempDir instead of world-predictable /tmp to avoid symlink races
	const std::string kmlTmpPath =
		QDir(QString::fromStdString(_tempDir)).filePath("heatmap_doc.kml").toStdString();
	FILE *fkml = (FILE *)NULL;
	if (!(fkml = fopen(kmlTmpPath.c_str(), "wb"))) {
	...
	if (fkml) {
		...
		for (lonRegionIdx = 0; lonRegionIdx < numRegionLon; lonRegionIdx++) {
			...
			for (latRegionIdx = 0; latRegionIdx < numRegionLat; latRegionIdx++) {
				...
				/**************************************************************************************/
				FILE *fppm;
				// Use tempDir-relative path
				const std::string ppmTmpPath =
					QDir(QString::fromStdString(_tempDir))
						.filePath("heatmap_image.ppm")
						.toStdString();
				if (!(fppm = fopen(ppmTmpPath.c_str(), "wb"))) {
					throw std::runtime_error("ERROR");
				}
				fprintf(fppm, "P3\n");
				fprintf(fppm,
				...
				std::string pngFile =
					QDir(QString::fromStdString(_tempDir))
						.filePath("heatmap_image_" +
							  QString::fromStdString(
								  std::to_string(lonRegionIdx)) +
							  "_" +
							  QString::fromStdString(
								  std::to_string(latRegionIdx)) +
							  ".png")
						.toStdString();

				std::string command = "convert " + ppmTmpPath + " " + pngFile;
	...
	/**************************************************************************************/
	if (fkml) {
		std::string command = "zip -j " + _kmlFile + " " + kmlTmpPath;
		for (lonRegionIdx = 0; lonRegionIdx < numRegionLon; lonRegionIdx++) {
			for (latRegionIdx = 0; latRegionIdx < numRegionLat; latRegionIdx++) {
				command += " " +
					   QDir(QString::fromStdString(_tempDir))
...
void AfcManager::setConstInputs(const std::string &tempDir)
{
	...
	_tempDir = tempDir; // store for safe per-request temp-file creation
...
void AfcManager::createChannelList()
{
	...
	for (auto &freqRange : _inquiredFrequencyRangesMHz) {
		...
		if (inquiredStopFreqMHz <= inquiredStartFreqMHz) {
			_responseInfo.reset(
				new ResponseInfo(CConst::unsupportedSpectrumResponseCode,
						 std::ostringstream()
							 << "Inquired Freq Range INVALID: ["
							 << inquiredStartFreqMHz << ", "
							 << inquiredStopFreqMHz
							 << "] stop freq must be > start freq"));
			return;
		}
	...
	for (auto &channelPair : _inquiredChannels) {
		...
		if (totalNumChan == 0) {
			_responseInfo.reset(new ResponseInfo(CConst::missingParamResponseCode,
							     "Missing valid Inquired channel and "
							     "frequency "));
			return;
		}
	}
...
void AfcManager::runAnalyzeNLCD()
{
	...
	/**************************************************************************************/
	if (1) {
		// Use per-request _tempDir to avoid races on shared /tmp
		const std::string nlcdKmlPath = QDir(QString::fromStdString(_tempDir))
							.filePath("nlcd_doc.kml")
							.toStdString();
		if (!(fkml = fopen(nlcdKmlPath.c_str(), "wb"))) {
			errStr << std::string("ERROR: Unable to open kmlFile \"") + nlcdKmlPath +
					  std::string("\"\n");
			throw std::runtime_error(errStr.str());
		}
	} else {
	...
	for (lonRegionIdx = 0; lonRegionIdx < numRegionLon; lonRegionIdx++) {
		...
		for (latRegionIdx = 0; latRegionIdx < numRegionLat; latRegionIdx++) {
			...
			/**************************************************************************************/
			FILE *fppm;
			// Use tempDir-relative paths to avoid races on shared /tmp
			const std::string ppmTmpPath = QDir(QString::fromStdString(_tempDir))
							       .filePath("heatmap_image.ppm")
							       .toStdString();
			if (!(fppm = fopen(ppmTmpPath.c_str(), "wb"))) {
				throw std::runtime_error("ERROR");
			...
			std::string pngFile = QDir(QString::fromStdString(_tempDir))
						      .filePath(
							      "nlcd_image_" +
			...
			std::string command = "convert " + ppmTmpPath + " -transparent white " +
					      pngFile;
			std::cout << "COMMAND: " << command << std::endl;
			system(command.c_str());
