...
namespace
{
	...
	LOGGER_DEFINE_GLOBAL(logger, "GdalDataModel")
}; // end namespace
...
GdalDataModel::GdalDataModel(const std::string &dataSourcePath, const std::string &heightFieldName) {
	...
	if (_ptrDataSource->GetLayerCount() != 1)
		...
		LOGGER_INFO(logger) << "Successfully opened OGR data source at " << dataSourcePath;
	...
	if (heightFieldIdx == -1) {
		...
		LOGGER_INFO(logger) << "Height Field Index set to  " << heightFieldIdx;
	}
...
double GdalDataModel::getMaxBuildingHeightAtPoint(double latDeg, double lonDeg) const {
	double maxHeight_m = quietNaN; // Instantiate as NaN in case no building data exists at input lat/lon
...
std::map<int64_t,double> GdalDataModel::getBuildingsAtPoint(double lat, double lon) const { // If in a building return a polygon, if not then return NULL
