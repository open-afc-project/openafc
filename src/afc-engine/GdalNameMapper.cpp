...
GdalNameMapperPattern::NamePart::NamePart(GdalNameMapperPattern::Src src_, GdalNameMapperPattern::Op op_, std::string str_) :
...
GdalNameMapperPattern::GdalNameMapperPattern(const std::string &pattern, const std::string &directory)
{
    ...
    while (boost::regex_search(start, end, m, elemRegex)) {
        ...
        if ((elemType == "latHem") || (elemType == "lonHem")) {
            ...
            _nameParts.emplace_back((elemType.substr(0, 3) == "lat") ? Src::Lat : Src::Lon, Op::Hemi, elemFormat);
...
void GdalNameMapperPattern::appendLatLon(Src src, Op op, const std::string &elemFormat, const std::string errPrefix)
{
...
std::string GdalNameMapperPattern::nameFor(double latDeg, double lonDeg)
{
    ...
    for (const NamePart &np : _nameParts) {
        ...
        switch (np.op) {
            ...
            case Op::DegCeil1:
            case Op::DegCeil:
                snprintf(buf, sizeof(buf), np.str.c_str(), (int)fabs(ceil((np.src == Src::Lat) ? latDeg : lonDeg)));
                ret += buf;
                break;
            case Op::DegFloor:
            case Op::DegFloor1:
                snprintf(buf, sizeof(buf), np.str.c_str(), (int)fabs(floor((np.src == Src::Lat) ? latDeg : lonDeg)));
                ret += buf;
                break;
        }
    }
    if (!ret.empty() && !_directory.empty()) {
        ...
        for (boost::filesystem::directory_iterator di(_directory); di != boost::filesystem::directory_iterator(); ++di) {
            std::string filename = di->path().filename().string();
            if (boost::filesystem::is_regular_file(di->path()) && (fnmatch(ret.c_str(), filename.c_str(), 0) != FNM_NOMATCH) &&
...
std::unique_ptr<GdalNameMapperBase> GdalNameMapperPattern::make_unique(const std::string &pattern, const std::string &directory)
{
...
GdalNameMapperDirect::GdalNameMapperDirect(const std::string &fnmatchPattern, const std::string &directory) : _fnmatchPattern(fnmatchPattern)
{
    GDALAllRegister();
    std::ostringstream errStr;
    for (boost::filesystem::directory_iterator di(directory); di != boost::filesystem::directory_iterator(); ++di) {
        std::string filename = di->path().filename().native();
        if ((!boost::filesystem::is_regular_file(di->path())) || (fnmatch(fnmatchPattern.c_str(), filename.c_str(), 0) == FNM_NOMATCH)) {
            continue;
        }
        GDALDataset *gdalDataSet = nullptr;
        try {
            gdalDataSet = static_cast<GDALDataset *>(GDALOpen(di->path().native().c_str(), GA_ReadOnly));
            _files.push_back(std::make_tuple(GdalTransform(gdalDataSet, filename).makeBoundRect(), filename));
...
std::unique_ptr<GdalNameMapperBase> GdalNameMapperDirect::make_unique(const std::string &fnmatchPattern, const std::string &directory)
{
    return std::unique_ptr<GdalNameMapperBase>(new GdalNameMapperDirect(fnmatchPattern, directory));
}
