...
const StrTypeClass MultibandRasterClass::strHeightResultList[] = {{MultibandRasterClass::OUTSIDE_REGION, "OUTSIDE_REGION"},
    {MultibandRasterClass::NO_DATA, "NO_DATA"}, {MultibandRasterClass::NO_BUILDING, "NO_BUILDING"}, {MultibandRasterClass::BUILDING, "BUILDING"},
...
void MultibandRasterClass::getHeight(
    const double &latDeg, const double &lonDeg, double &terrainHeight, double &bldgHeight, HeightResult &heightResult, bool directGdalMode) const
{
    ...
    if (_cgLidar.covers(latDeg, lonDeg)) {
        ...
        } else if ((_format == CConst::fromRasterLidarFormat) && (bldgHeightF <= (terrainHeightF + 1))) {
