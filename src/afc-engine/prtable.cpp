        ...
        switch (lineType) {
            case labelLineType: {
                ...
                if (sizeStrList.size() != 2) {
                    errStr << std::string("ERROR: Passive Repeater Table File ") << tableFile << ":" << linenum << " Ivalid table size "
                           << fieldList[0] << std::endl;
                    throw std::runtime_error(errStr.str());
                }
...
double PRTABLEClass::computePRTABLE(double Q, double oneOverKs)
{
    ...
    if (kIdx0 == numOneOverKs - 1) {
        kIdx0 = numOneOverKs - 2;
    }
    ...
    double tableVal = F00 * (qIdx0 + 1 - qIdxDbl) * (kIdx0 + 1 - kIdxDbl) + F01 * (qIdx0 + 1 - qIdxDbl) * (kIdxDbl - kIdx0) +
        F10 * (qIdxDbl - qIdx0) * (kIdx0 + 1 - kIdxDbl) + F11 * (qIdxDbl - qIdx0) * (kIdxDbl - kIdx0);
