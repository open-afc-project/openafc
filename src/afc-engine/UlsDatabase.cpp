			     ...
			     QSqlQuery &q)
{
	...
	while (rasQueryRes.next()) {
		...
		std::string exclusionZoneStr =
			rasQueryRes.value(ras_exclusionZoneIdx).toString().toStdString();
	...
	if (numAntennaDB) {
		...
		// Fail closed on incomplete DB-derived interpolation tables: a duplicated
		// aob_idx consumes one row and leaves another slot at its quietNaN
		// initializer, which would reach LinInterpClass evaluation (process abort)
		// at inquiry time. Reject the load instead, matching the engine's
		// exception-based rejection idiom.
		for (int aobIdx = 0; aobIdx < numAntennaAOB; ++aobIdx) {
			if (!std::isfinite(antennaAOBList[aobIdx])) {
				throw std::runtime_error(
					ErrStream()
					<< "UlsDatabase.cpp: antaob table incomplete: aob_idx "
					<< aobIdx << " missing or duplicated (expected "
					<< numAntennaAOB << " distinct entries)");
			}
			if (aobIdx > 0 && !(antennaAOBList[aobIdx] > antennaAOBList[aobIdx - 1])) {
				throw std::runtime_error(ErrStream()
							 << "UlsDatabase.cpp: antaob aob_deg "
							    "values not strictly "
							 << "increasing at aob_idx " << aobIdx);
			}
		}
	}
	/**************************************************************************************/
						...
						std::string antennaName)
{
	...
	for (int aobIdx = 0; aobIdx < numAntennaAOB; ++aobIdx) {
		if (!std::isfinite(std::get<1>(sampledData[aobIdx]))) {
			LOGGER_ERROR(logger)
				<< "UlsDatabase: antgain table incomplete for antenna "
				<< antennaName << ": aobIdx " << aobIdx << " is non-finite";
			return nullptr;
		}
	}
