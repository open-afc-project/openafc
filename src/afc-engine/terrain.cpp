...
TerrainClass::TerrainClass(std::string lidarDir, std::string cdsmDir, std::string srtmDir, std::string depDir, std::string globeDir,
    double terrainMinLat, double terrainMinLon, double terrainMaxLat, double terrainMaxLon, double terrainMinLatBldg, double terrainMinLonBldg,
    ...
    maxLidarRegionLoad(maxLidarRegionLoadVal), gdalDirectMode(false)
{
    ...
    cgSrtm.reset(new CachedGdal<int16_t>(srtmDir, "srtm",
        GdalNameMapperPattern::make_unique("{latHem:NS}{latDegFloor:02}{lonHem:EW}{"
                                           "lonDegFloor:03}.hgt")));
    ...
    cgGlobe.reset(new CachedGdal<int16_t>(globeDir, "globe", GdalNameMapperDirect::make_unique("*.bil", globeDir)));
    cgGlobe->setNoData(0);
    ...
    MultibandRasterClass::HeightResult &lidarHeightResult, CConst::HeightSourceEnum &heightSource, bool cdsmFlag) const
{
    ...
    } else if ((longitudeDeg >= minLidarLongitude) && (longitudeDeg <= maxLidarLongitude) && (latitudeDeg >= minLidarLatitude) &&
    ...
    if (lidarRegionIdx != -1) {
        ...
        lidarRegionList[lidarRegionIdx].multibandRaster->getHeight(
            latitudeDeg, longitudeDeg, terrainHeight, bldgHeight, lidarHeightResult, gdalDirectMode);
        ...
        switch (lidarHeightResult) {
            case MultibandRasterClass::OUTSIDE_REGION:
                ...
                // checked.
                throw std::logic_error("point outside region defined by rectangle 'bounds' for "
                                       "lat: " +
                    std::to_string(latitudeDeg) + ", lon: " + std::to_string(longitudeDeg) + " in lidarRegionIdx: " + std::to_string(lidarRegionIdx));
                break;
...
void TerrainClass::readLidarData(double terrainMinLat, double terrainMinLon, double terrainMaxLat, double terrainMaxLon)
{
    ...
    for (lidarRegionIdx = 0; (lidarRegionIdx < ((int)lidarRegionList.size())); ++lidarRegionIdx) {
        ...
        if (!((terrainMaxLon < lidarRegion.minLonDeg) || (terrainMinLon > lidarRegion.maxLonDeg) || (terrainMaxLat < lidarRegion.minLatDeg) ||
                        ...
                        if (*fieldIdxList.at(fIdx) == -1) {
                            if (fieldLabelList.at(fIdx) == "FORMAT") {
                                ...
                                errStr << "ERROR: Invalid Lidar "
                                          "Info file \""
                                       << infoFile << "\" label line missing \"" << fieldLabelList.at(fIdx) << "\"" << std::endl;
                                throw std::runtime_error(errStr.str());
                            }
                        }
                    }
...
void TerrainClass::printStats()
{
    ...
    LOGGER_INFO(logger) << "NUM_LIDAR = " << numLidar << "  (" << (double)(totalNumTerrain ? numLidar * 100.0 / totalNumTerrain : 0.0) << " %)";
    LOGGER_INFO(logger) << "NUM_CDSM = " << numCDSM << "  (" << (double)(totalNumTerrain ? numCDSM * 100.0 / totalNumTerrain : 0.0) << " %)";
    LOGGER_INFO(logger) << "NUM_DEP = " << numDEP << "  (" << (double)(totalNumTerrain ? numDEP * 100.0 / totalNumTerrain : 0.0) << " %)";
    LOGGER_INFO(logger) << "NUM_SRTM = " << numSRTM << "  (" << (double)(totalNumTerrain ? numSRTM * 100.0 / totalNumTerrain : 0.0) << " %)";
    LOGGER_INFO(logger) << "NUM_GLOBAL = " << numGlobal << "  (" << (double)(totalNumTerrain ? numGlobal * 100.0 / totalNumTerrain : 0.0) << " %)";
