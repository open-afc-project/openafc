...
QVector<QPointF> computeApproximateGreatCircleLine(const QPointF &from, const QPointF &to, int numpts, double *tdist)
{
    ...
    for (int i = 1; i < numpts; ++i) {
        ...
        latlons[i] += QPointF(delx * frac, dely * frac);
    }
...
QVector<QPointF> computeGreatCircleLine(const QPointF &from, const QPointF &to, int numpts, double *tdist)
{
    ...
    //  We will be interpolating along greatCircleDistance and converting distances along the
    //  great circle into lat/lons.
    QVector<QPointF> latlons;
    ...
    for (int i = 2; i < numpts; ++i, ++it) {
        ...
        // greatCircleDistance, tgcDist);
        double tgc = tgcDist / earthRad;

        // printf("END POINT: %.8g, %.8g, %.8g, %.8g\n", tLatRad, tLonRad, targetAz * M_PI /
        // 180.0, tgc);
        ...
        if (targetAz > 180.0)
            zrLon = fromY - fabs(rdLon) * 180 / M_PI;
        else
            zrLon = fromY + fabs(rdLon) * 180 / M_PI;
...
QVector<QPointF> computePartialGreatCircleLine(const QPointF &from, const QPointF &to, int numptsTotal, int numptsPartial, double *tdist)
{
    ...
    for (int i = 0; i < numptsPartial; ++i) {
        ...
        if (remainingPoints < thisStepCount)
...
double *computeElevationVector(
    const TerrainClass *terrain, bool includeBldg, bool cdsmFlag, const QPointF &from, const QPointF &to, int numpts, double *cdsmFracPtr)
{
    double tdist;
...
double greatCircleDistance(double lat1, double lon1, double lat2, double lon2)
{
    ...
    double centerAngle = std::acos(std::sin(lat1rad) * std::sin(lat2rad) + std::cos(lat1rad) * std::cos(lat2rad) * std::cos(deltalon));
...
double runPointToPoint(const TerrainClass *terrain, bool includeBldg, QPointF transLocLatLon, double transHt, QPointF receiveLocLatLon,
    double receiveHt, double lineOfSightDistanceKm, double eps_dielect, double sgm_conductivity, double eno_ns_surfref, double frq_mhz,
    int radio_climate, int pol, double conf, double rel, int numpts, char *prefix, double **heightProfilePtr)
{
    if (!(*heightProfilePtr)) {
    ...
    point_to_point(*heightProfilePtr, transHt, receiveHt, eps_dielect, sgm_conductivity, eno_ns_surfref, frq_mhz, radio_climate, pol, conf, rel, rv,
        strmode, errnum);
...
bool isLOS(const TerrainClass *terrain, QPointF transLocLatLon, double transHt, QPointF receiveLocLatLon, double receiveHt,
    double lineOfSightDistanceKm, int numpts, double **heightProfilePtr, double *cdsmFracPtr)
{
    if (!(*heightProfilePtr)) {
