...
Logging::Filter::NameError::NameError(const std::string &msg) : logic_error(msg)
{
}

Logging::Filter::Filter() : leastLevel(Logging::LOG_DEBUG)
{
}
...
Logging::SyslogConfig::SyslogConfig() : facility(sinks::syslog::user)
{
}
#endif
...
Logging::Config::Config() : useStdOut(false), useStdErr(true)
{
#if !defined(BOOST_LOG_WITHOUT_SYSLOG)
    useSyslog = false;
#endif
...
void Logging::initialize(const Logging::Config &config)
{
    ...
    if (config.useSyslog && config.syslogConfig) {
        ...
        auto backend = boost::make_shared<backend_t>(
            keywords::facility = config.syslogConfig->facility, keywords::ident = config.syslogConfig->identity);
    ...
    if (config.useWinlog && config.winlogConfig) {
        ...
        auto backend = boost::make_shared<backend_t>(keywords::log_source = config.winlogConfig->identity);
        {
