...
SqlSelect::SqlSelect() : _rowLimit(-1)
{
}

SqlSelect::SqlSelect(const QSqlDatabase &db, const QString &table) : _db(db), _table(table), _rowLimit(-1)
{
}

SqlSelect::SqlSelect(const QSqlDatabase &db, const SqlTable &table) : _db(db), _table(table.expression()), _rowLimit(-1)
{
}
...
SqlSelect &SqlSelect::whereBetween(const QString &colVal, const QVariant &minInclusive, const QVariant &maxInclusive)
{
    const QSqlDriver *drv = _db.driver();
    _whereExprs
        << QString("((%1 >= %2) AND (%1 <= %3))").arg(colVal).arg(SqlHelpers::quoted(drv, minInclusive)).arg(SqlHelpers::quoted(drv, maxInclusive));
    return *this;
}
...
QString SqlSelect::query() const
{
    ...
    {
        queryStr += QString(" %1 JOIN %2 ON (%3)").arg(joinVal.typeClause, joinVal.whatClause, joinVal.onClause);
    }
