...
SqlTransaction::SqlTransaction(QSqlDatabase &db) : _db(&db)
{
	const bool success = _db->transaction();
	if (SqlHelpers::doDebug()) {
		LOGGER_DEBUG(logger)
			<< "Started on " << _db->connectionName() << " success=" << success;
	}
	if (!success) {
...
SqlTransaction::~SqlTransaction()
{
	...
	if (SqlHelpers::doDebug()) {
		LOGGER_DEBUG(logger)
			<< "Rollback on " << _db->connectionName() << " success=" << success;
	}
...
void SqlTransaction::commit()
{
	...
	if (SqlHelpers::doDebug()) {
		LOGGER_DEBUG(logger)
			<< "Commit on " << _db->connectionName() << " success=" << success;
	}
	if (!success) {
