...
SqlDelete::SqlDelete(const QSqlDatabase &db, const QString &tableName) : _db(db), _table(tableName), _tableExpr(tableName)
{
}
...
{
    _whereExprs << QString("(%1 %2 %3)").arg(col).arg(op).arg(SqlHelpers::quoted(_db.driver(), value));
    return *this;
}
