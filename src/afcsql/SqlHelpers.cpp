...
void SqlHelpers::execPrepared(const QSqlDatabase &db, QSqlQuery &qObj)
{
    ...
    if (doDebug()) {
        LOGGER_DEBUG(logger) << "execPrepared RUN " << qObj.lastQuery() << " WITH (" << boundList(qObj).join(", ") << ")";
    ...
    if (doDebug()) {
        ...
        LOGGER_DEBUG(logger) << "execPrepared TIME " << elapsed << " SIZE " << qObj.size() << " ERR " << qObj.lastError();
    }
    if (!success) {
        const QSqlError sqlErr = qObj.lastError();
        checkMysql(db, sqlErr);
        throw SqlError(QString("Failed exec for \"%1\" with values (%2)").arg(qObj.lastQuery()).arg(boundList(qObj).join(",")), sqlErr);
    }
}
