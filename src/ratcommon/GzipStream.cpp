...
GzipStream::GzipStream(QIODevice *dev, QObject *parentVal) : QIODevice(parentVal), _dev(nullptr), _pos(0)
{
    setSourceDevice(dev);
}
...
bool GzipStream::openStream(OpenMode rwmode)
{
    ...
    switch (rwmode) {
        ...
        case WriteOnly:
            ...
            status = ::deflateInit2(&_stream, _params.compressLevel, Z_DEFLATED, _params.windowBits, _params.memLevel, Z_DEFAULT_STRATEGY);
            if (status != Z_OK) {
                setError();
                return false;
            }
