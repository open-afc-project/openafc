...
class FieldMachine
{
    public:
        ...
        FieldMachine() : keepQuotes(false), state(StartField)
        {
        }
            ...
            switch (state) {
                ...
                case UnquotedField:
                    if (chr == sep) {
                        ...
                        field.append(chr);
                        // EOL text is kept in unquoted field until full
                        // match
...
CsvReader::CsvReader(const QString &fileName) : _ownFile(true), _nominalColumnCount(-1)
{
    _defaultOpts();
...
CsvReader::CsvReader(QIODevice &device) : _ownFile(false), _nominalColumnCount(-1)
{
    _defaultOpts();
...
QStringList CsvReader::readRow()
{
    ...
    if (_nominalColumnCount > 0)
        fields.reserve(_nominalColumnCount);
    while (true) {
#if defined(CSV_USE_TEXT_STREAM)
        _str >> chr;
        switch (_str.status()) {
            ...
            case QTextStream::ReadPastEnd:
                ...
                if (process.state == FieldMachine::QuotedField) {
                    throw FileError(QString("End of file within quoted field"));
                }
            ...
            default:
                throw FileError(QString("Failed to read line from file: %1").arg(_str.device()->errorString()));
        }
#else
        if (_dev->atEnd()) {
            // Insert trailing EOL if necessary
            ...
            if (_dev->getChar(&c)) {
                ...
                throw FileError(QString("Failed to read line from file: %1").arg(_dev->errorString()));
            }
        }
#endif
