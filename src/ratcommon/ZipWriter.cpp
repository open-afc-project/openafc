...
ZipWriter::ZipWriter(const QString &fileName, WriterMode mode, int compress) : _impl(new Private)
{
...
std::unique_ptr<QIODevice> ZipWriter::openFile(const QString &intFileName, const QDateTime &modTime)
{
    ...
    int status = zipOpenNewFileInZip64(
        _impl->file, intFileName.toUtf8().data(), &fileInfo, nullptr, 0, nullptr, 0, nullptr, Z_DEFLATED, _impl->compression, 1);
    if (status != ZIP_OK) {
...
ZipWriter::ContentFile::ContentFile(ZipWriter &parentVal) : _parent(&parentVal)
{
