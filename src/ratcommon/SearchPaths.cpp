...
QString SearchPaths::forWriting(const QString &prefix, const QString &fileName)
{
    ...
    LOGGER_WARN(logger) << "No forWriting path found under \"" << prefix << "\" with name \"" << fileName << "\"";
    return QString();
}
...
QString SearchPaths::forReading(const QString &prefix, const QString &fileName, bool required)
{
    ...
    if (required) {
        throw std::runtime_error(QString("No path found for \"%1\" with name \"%2\"").arg(prefix, fileName).toStdString());
    }

    LOGGER_WARN(logger) << "No forReading path found for \"" << prefix << "\" with name \"" << fileName << "\"";
    return QString();
}
