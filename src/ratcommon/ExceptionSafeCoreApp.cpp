...
ExceptionSafeCoreApp::ExceptionSafeCoreApp(int &argc, char **argv) : QCoreApplication(argc, argv)
{
}
...
void ExceptionSafeCoreApp::logError(const QObject *obj, const QMetaObject *mObj, const QEvent *eventVal, const QString &msg) const
{
    ...
    LOGGER_ERROR(logger) << QString("Failed sending event type %1 to %2(%3): %4").arg(eType).arg(targetClass, targetObj).arg(msg);
}
