...
GzipCsv::GzipCsv(const std::string &filename)
{
    ...
    if (!_gzipWriter->open(QIODevice::WriteOnly)) {
        throw std::runtime_error(QString("Gzip \"%1\" failed to open").arg(qfilename).toStdString());
    }
...
GzipCsv::ColBase::ColBase(GzipCsv *container, const std::string &name) : _name(QString::fromStdString(name))
{
...
GzipCsv::ColDouble::ColDouble(GzipCsv *container, const std::string &name, const std::string &format) : ColBase(container, name), _format(format)
{
}
...
GzipCsv::ColBool::ColBool(GzipCsv *container, const std::string &name, const std::vector<std::string> &tf) : ColBase(container, name)
{
    assert(tf.size() == 2);
    for (const auto &s : tf) {
...
GzipCsv::ColEnum::ColEnum(GzipCsv *container, const std::string &name, const std::map<int, std::string> &items, const std::string &defName) :
