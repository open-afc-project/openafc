...
CsvWriter::CsvWriter(const QString &fileName)
  : _ownFile(true), _colI(0){
    _defaultOpts();

    // Destructor is not called if constructor throws
    QScopedPointer<QIODevice> file(new QFile(fileName));
    if(!file->open(QIODevice::WriteOnly)){
        throw FileError(QString("Failed to open \"%1\" for writing: %2").arg(fileName, file->errorString()));
    }
    _str.setDevice(file.take());
    _str.setCodec("UTF-8");
}

CsvWriter::CsvWriter(QIODevice &device)
  : _ownFile(false), _colI(0){
    _defaultOpts();
