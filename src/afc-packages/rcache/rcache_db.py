    ...
    def __init__(self, rcache_db_dsn: Optional[str] = None,
        ...
        sa.Table(
            ...
            sa.Column("last_update", sa.DateTime(), nullable=False,
                      index=True),
            # When the current req_cfg_digest holder first claimed this PK
            # slot. Unlike last_update it is preserved across
            # digest-preserving rewrites (notably the precomputation
            # refresh), so RcacheDbAsync.update_cache's age arm can bound a
            # spoofed-identity squat
        ...
        with FailOnError(fail_on_error):
            try:
                ...
                try:
                    ...
                    # create_all() creates missing tables but not missing
                    # columns, and the runtime metadata is reflected from
                    # the live database - so 'created_at' (squat epoch, see
                    # RcacheDbAsync.update_cache) must be added to databases
                    # that predate it. Idempotent (IF NOT EXISTS)
                    with engine.begin() as conn:
                        conn.execute(sa.text(
                            f"ALTER TABLE {self.AP_TABLE_NAME} "
                            f"ADD COLUMN IF NOT EXISTS created_at "
