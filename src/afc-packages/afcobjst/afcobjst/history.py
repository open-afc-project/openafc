...
def _derive_objst_bearer_token(key):
    ...
    HMAC-SHA256(key, 'afc-objst-bearer-v1') - mirrors filestorage.py; the
    raw provisioned secret must never transit the wire because the RMQ
...
def _load_objst_api_key():
    """ Return the expected hist bearer token (derived from
...
def get(path):
    ...
    try:
        ...
        with objst.open(lpath) as hobj:
            ...
            data = hobj.read()
            # Stored history artifacts are writable by any holder of the
            # shared objst bearer key, so they must never render inline in
            # an authenticated origin: force a download with a fixed
            # non-executable MIME type instead of letting Flask sniff
            # text/html from an attacker-chosen file extension.
