...
# Size caps for objstore-sourced bodies: objstorage is writable by any
# holder of the shared bearer key, so bodies must be bounded before
# buffering (fst read max_bytes) — same invariant as ratapi.py/ratafc.py
# MAX_RESPONSE_BODY_BYTES and afctask.py MAX_STATUS_JSON_BYTES.
MAX_REQUEST_CONFIG_BYTES = 64 * 1024 * 1024
# Crash-report re-reads feed a best-effort ALS log entry; bound them
# tightly (matches ratafc.py MAX_ERROR_TXT_BYTES).
...
def _verify_req_cfg_digest(
    ...
    # differences are eliminated.  The comparison must be TYPE-strict:
    # Python dict equality coerces bool/int ({"x": 1} == {"x": True}) while
    # the engine's QJsonValue treats bool and number as distinct types, so
    # `==` on the parsed trees would authorise an engine input that computes
    # on different effective values than the digest-bound original (same
    # parser-differential class as SUB-0138-93).  Canonical re-serialisation
    # keeps bool/int distinct ("true" vs "1"), and allow_nan=False raises
    # ValueError on Infinity/NaN forms QJson rejects (caught below).
    if request_str and request_str != original_request_str:
        try:
            ...
            orig_canonical = json.dumps(
                orig_parsed, sort_keys=True, allow_nan=False)
            req_canonical = json.dumps(
                req_parsed, sort_keys=True, allow_nan=False)
            if orig_canonical != req_canonical:
                LOGGER.warning(
...
def run(prot, host, port, request_type, task_id, hash_val,
    ...
    # bind request_type, so non-"AP-AFC" is rejected.
    # ratafc.py only ever dispatches "AP-AFC" on this path.
    ...
    try:
        ...
        if use_tasks:
            ...
            with dataif.open(os.path.join("/responses", hash_val,
                ...
                _req_bytes = hfile.read(max_bytes=MAX_REQUEST_CONFIG_BYTES)
            ...
            with dataif.open(config_path) as hfile:
                _cfg_bytes = hfile.read(max_bytes=MAX_REQUEST_CONFIG_BYTES)
            ...
            # check an unverified message could republish a
            # digest-valid tuple with
            # a different task_id and place objstore
            ...
            # before the Celery message is published; status.json validation
            # requires the worker's own Bearer credential).
            if _tasks_hash_ok:
                _dispatch_stat = None
                try:
                    with dataif.open(
                        ...
                        _dispatch_stat = json.loads(hfile.read(
                            max_bytes=afctask.MAX_STATUS_JSON_BYTES))
            ...
            # Abort BEFORE launching the engine when the digest/task
            # binding failed: previously the engine still ran and only the
            # objstore writes were suppressed, so unverified input could
            # burn the full soft_time_limit per task.
            if not _tasks_hash_ok:
                LOGGER.error(
                    "use_tasks digest/task-binding check failed "
                    "(hash_val=%s); aborting before engine launch instead "
                    "of running afc-engine on unverified input.", hash_val)
                log_file.close()
                err_file.close()
                return
        else:
            ...
            with dataif.open(config_path) as hfile:
                engine_config_bytes = hfile.read(
                    max_bytes=MAX_REQUEST_CONFIG_BYTES)
            with open(analysis_config_path, "wb") as outfile:
            ...
            # Enforce the request/config/runtime_opts binding BEFORE
            # launching afc-engine: any objstore bearer-key holder can
            # overwrite /afc_config/{digest}/afc_config.json between
            # afc_server's write and this read, and verifying only after
            # proc.wait() lets the substituted config burn the full
            # soft_time_limit per task. Same arguments as the post-run
            # check below.
            if not _verify_req_cfg_digest(
                    hash_val=hash_val,
                    original_request_str=original_request_str,
                    request_str=request_str,
                    config_str=engine_config_str,
                    runtime_opts=runtime_opts,
                    use_tasks=use_tasks):
                LOGGER.error(
                    "rmq digest mismatch (hash_val=%s, runtime_opts=%#x) "
                    "detected before engine launch; refusing to run "
                    "afc-engine on unverified config",
                ...
                get_rcache_client().rmq_send_response(
                    ...
                    request=original_request_str, response=None,
                    update_cache=False, task_id=str(task_id))
                return
        ...
        try:
            ...
            try:
                if use_tasks:
                    if os.path.isfile(analysis_request_path):
                        with open(analysis_request_path, "r", encoding="utf-8", errors="replace") as f:
                            request_str = f.read(MAX_CRASH_REPORT_BYTES)
                    else:
                        with dataif.open(os.path.join("/responses", hash_val,
                                                      "analysisRequest.json")) \
                                as hfile:
                            request_str = hfile.read(
                                max_bytes=MAX_CRASH_REPORT_BYTES).decode(
                                    "utf-8", errors="replace")
                    if os.path.isfile(analysis_config_path):
                        with open(analysis_config_path, "r", encoding="utf-8", errors="replace") as f:
                            config_str = f.read(MAX_CRASH_REPORT_BYTES)
                    else:
                        with dataif.open(config_path) as hfile:
                            config_str = hfile.read(
                                max_bytes=MAX_CRASH_REPORT_BYTES).decode(
                                    "utf-8", errors="replace")
