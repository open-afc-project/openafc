...
class DsnInfo:
    ...
    def __init__(self, dsn: str, password_file: Optional[str] = None,
        ...
        if password_file:
            ...
            # instead of silently minting a role with the default
            # credential.
...
def ensure_dsn(dsn: str, password_file: Optional[str] = None,
    ...
    if not local:
        ...
        # credentials. Anchor the transport-security decision here:
        # cleartext credential transport is opt-in
        # (AFC_DB_CREATOR_ALLOW_INSECURE_HTTP defaults to false), so a
        # plain http:// URL is refused unless the operator explicitly
        # accepts cleartext on this seam.
        if urllib.parse.urlparse(db_creator_url).scheme != "https":
            error_if(
                not to_bool(os.environ.get(
                    "AFC_DB_CREATOR_ALLOW_INSECURE_HTTP", "false")),
                f"DB Creator REST API URL '{DB_CREATOR_URL_ENV}'="
                f"'{db_creator_url}' does not use https:// and "
                f"AFC_DB_CREATOR_ALLOW_INSECURE_HTTP is not set to true; "
                f"refusing to POST a database-owner password/internal "
                f"token in cleartext")
    ...
    try:
        ...
        error_if(creator_dsn_info.password_defaulted,
                 ...
                 f"is set; refusing to fall back to the default "
                 f"password")
        ...
        try:
            ...
            # str(SQLAlchemyError) appends the failed statement text
            # ('[SQL: ...]'), which for CREATE USER embeds the plaintext
            # password rendered by sql.Literal.  Log only the driver-level
            # error and redact any literal occurrence of the password.
            msg = str(getattr(ex, "orig", None) or ex)
            if desired_dsn_info.password:
                msg = msg.replace(desired_dsn_info.password, "[REDACTED]")
            error(f"Error creating user '{desired_dsn_info.user}': {msg}")
        ...
        # template1 no longer grants CREATE ON SCHEMA public to the
        # application read/write role (template-poisoning hardening in
        # bulk_postgres/initdb.d/01_create_bulk_roles.sh and
        # bulk_postgres/postgis-upgrade.sh), so grant it per application
        # database at creation time to the role the database was created
        # for.
        if not db_exists:
            try:
                _creator_parts = urllib.parse.urlparse(creator_dsn_info.dsn)
                ...
                try:
                    with _db_engine.connect() as conn:
                        ...
                        conn.execute(sa.text("COMMIT"))
                        query_schema_grant = sql.SQL(
                            "GRANT CREATE ON SCHEMA public TO {}").format(
                            sql.Identifier(desired_dsn_info.user))
                        conn.execute(sa.text(
                            query_schema_grant.as_string(raw_conn)))
                finally:
                    _db_engine.dispose()
            except sa.exc.SQLAlchemyError as ex:
                error(f"Failed to grant CREATE ON SCHEMA public on "
                      f"'{desired_dsn_info.db}' to "
                      f"'{desired_dsn_info.user}': {ex}")
