...
# The server reads users from the rendered definitions.json, not from the
# environment — drop the credentials so beam.smp and its children (and any
# erl_crash.dump Environment section) do not inherit them.
unset BROKER_PWD RCACHE_RMQ_PWD
