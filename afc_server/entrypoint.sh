...
case "$AFC_DEVEL_ENV" in
  "devel")
    ...
    echo "AFC_SERVER_RATDB_DSN = ${AFC_SERVER_RATDB_DSN}" | sed 's#://[^@]*@#://***@#'
    echo "AFC_SERVER_RATDB_PASSWORD_FILE = ${AFC_SERVER_RATDB_PASSWORD_FILE}"
    echo "RCACHE_RMQ_DSN = ${RCACHE_RMQ_DSN}" | sed 's#://[^@]*@#://***@#'
    ...
    echo "AFC_ALS_TOPIC_NAME = ${AFC_ALS_TOPIC_NAME}"
    echo "AFC_JSON_TOPIC_PREFIX = ${AFC_JSON_TOPIC_PREFIX}"
