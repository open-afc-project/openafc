...
class AfcServerCompute:
    ...
    def __init__(self, rmq_dsn: str, rmq_password_file: Optional[str],
        ...
        # Celery task ID of the outstanding dispatch per digest.  Responses
        # whose task_id does not match are rejected (replay defense).
    ...
    async def process_request(
        ...
        try:
            ...
            if future_holders is None:
                ...
                self._request_task_ids[req_cfg_digest] = task_id
                celery_sender_task = \
                    asyncio.create_task(
                        asyncio.to_thread(
            ...
            if not future_holders:
                ...
                self._request_task_ids.pop(req_cfg_digest, None)
                ...
                async with connection:
                    ...
                    async with queue.iterator(no_ack=True) as queue_iter:
                        async for msg in queue_iter:
                            # Per-message errors (e.g. malformed signature
                            # material raising out of verification) must
                            # reject the single message, not exit the
                            # channel loop: leaving `async with connection`
                            # deletes the exclusive response queue and
                            # drops every in-flight AP response. Reserve
                            # the outer reconnect loop for genuine
                            ...
                            try:
                                ...
                                if not rcache_models.verify_rmq_resp_hmac(
                                    ...
                                    LOGGER.error(
                                        ...
                                        rrk.req_cfg_digest)
                                    continue
                                # Freshness/replay defense: only accept the
                                # response produced for the dispatch that is
                                # actually outstanding for this digest.  A
                                # validly signed triple captured before a data
                                # update (rcache invalidation) carries the old
                                # task_id and is rejected here.
                                if rrk.task_id is None or \
                                        self._request_task_ids.get(
                                            rrk.req_cfg_digest) != rrk.task_id:
                                    LOGGER.error(
                                        "Rejected RMQ response for digest %s: "
                                        "task_id %r does not match the "
                                        "outstanding dispatch (possible replay "
                                        "of a stale signed response)",
                                        rrk.req_cfg_digest, rrk.task_id)
                                    continue
                                future_holders = \
                                ...
                                LOGGER.error(
                                    f"Error processing RMQ response message "
                                    f"(message rejected): {ex}")
                                continue
