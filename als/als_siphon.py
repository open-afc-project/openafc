    ...
    def write_log(self, topic: str, records: List["LogsDatabase.Record"]) \
        ...
        try:
            if topic not in self.metadata.tables:
                ...
                # of creating one persistent table per requested
                # topic name.
        ...
        except (sa.exc.SQLAlchemyError, ValueError, TypeError) as ex:
            # ValueError/TypeError included: psycopg2 raises a plain
            # ValueError (not an SQLAlchemyError) on NUL bytes in bound
            # string parameters and TypeError on unadaptable bound types
            # (see TableUpdaterBase.update_db)
...
class AlsMessage:
    ...
    def __init__(self, raw_msg: bytes) -> None:
        ...
        if (self.runtime_opt is not None) and \
                (not isinstance(self.runtime_opt, int) or
                 not (0 <= self.runtime_opt < (1 << 31))):
            raise AlsProtocolError(
                f"Invalid 'runtimeOpt' value: {self.runtime_opt!r}",
                code_line=LineNumber.exc(), data=msg_dict)
...
class AlsMessageBundle:
    ...
    def update(self, message: AlsMessage, position: KafkaPosition) -> None:
        ...
        if self._assembled:
            return
        try:
            # Set the server attribution only from the message that first
            # establishes the bundle - rewriting it on every message would
            # let a forged AFC Config overwrite the audited afcServer
            # attribution with an arbitrary claimed value
            if not self._afc_server:
                self._afc_server = message.afc_server
            if message.msg_type == AlsMessage.MsgType.Request:
                if self._request_msg is not None:
                    try:
                        duplicate_msg = jd(json.loads(message.json_str))
                    except (json.JSONDecodeError, TypeError):
                        duplicate_msg = None
                    if (duplicate_msg != self._request_msg) or \
                            ((message.mtls_dn or "") != self._mtls_dn) or \
                            (message.ap_ip != self._ap_ip) or \
                            (message.runtime_opt != self._runtime_opt):
                        raise JsonFormatError(
                            "Conflicting duplicate AFC Request for bundle "
                            f"key {self._message_key!r} (claimed afcServer "
                            f"'{message.afc_server}') - possible spoofed "
                            "producer; first-arrived request kept",
                ...
                if self._response_msg is not None:
                    # Shared topic-write access lets any producer pre-empt
                    # the genuine afc_server response (audit forgery), so a
                    # second Response must not be dropped silently.  An
                    # identical retry (normal at-least-once producer
                    # behavior) stays silent; a CONFLICTING duplicate is
                    # surfaced as JsonFormatError so the caller records a
                    # decode_error row.  The first-arrived response is kept
                    # and the Kafka position was already recorded above, so
                    # the offset is still committed either way.
                    try:
                        duplicate_msg = json.loads(message.json_str)
                    except json.JSONDecodeError:
                        duplicate_msg = None
                    if duplicate_msg != self._response_msg:
                        raise JsonFormatError(
                            "Conflicting duplicate AFC Response for bundle "
                            f"key {self._message_key!r} (claimed afcServer "
                            f"'{message.afc_server}') - possible spoofed "
                            "producer; first-arrived response kept",
                            code_line=LineNumber.current(),
                            data=message.raw_msg)
                    return
                try:
                    response_msg = jd(json.loads(message.json_str))
                except json.JSONDecodeError:
                    raise JsonFormatError(
                ...
                if not isinstance(
                        response_msg.get(
                            "availableSpectrumInquiryResponses"), list):
                    raise JsonFormatError(
                        "AFC Response message lacks list-valued "
                ...
                # Parse-validate the config text at ingest (result
                # discarded; the DB-write phase re-parses).  json.loads
                # raises RecursionError (a RuntimeError subclass) on deeply
                # nested input, and the DB-write phase's handler tuples do
                # not contain it, so an unparseable config deferred to that
                # phase would unwind main_loop and crash-loop the siphon on
                # Kafka redelivery.  Raising JsonFormatError here routes
                # the poison message to decode_error and marks its offset
                # processed (same containment as Request/Response above).
                try:
                    json.loads(js(message.json_str))
                except (json.JSONDecodeError, RecursionError) as ex:
                    raise JsonFormatError(
                        f"Malformed AFC Config JSON: {ex!r}",
                        code_line=LineNumber.exc(),
                        data=message.json_str)
                config_info = \
                    self.AfcConfigInfo(
                ...
                # Mirror the Request/Response conflicting-duplicate policy:
                # shared topic-write access lets any producer silently
                # replace the audited config, so a DIFFERING AFC Config for
                # an already-populated slot is surfaced as JsonFormatError
                # (the caller records a decode_error row) and the
                # first-arrived config is kept; identical retries (normal
                # at-least-once producer behavior) stay silent.
                indexes = list(message.request_indexes) \
                    if message.request_indexes else [None]
                for i in indexes:
                    existing = self._configs.get(i)
                    if (existing is not None) and (existing != config_info):
                        raise JsonFormatError(
                            "Conflicting duplicate AFC Config for bundle "
                            f"key {self._message_key!r}, request index {i} "
                            f"(claimed afcServer '{message.afc_server}') - "
                            "possible spoofed producer; first-arrived "
                            "config kept",
                            code_line=LineNumber.current(),
                            data=message.raw_msg)
                for i in indexes:
    ...
    def take_apart(self) -> "AlsMessageBundle.StoreParts":
        ...
        jd(parts.rx_envelope)[
            "availableSpectrumInquiryRequests"] = []
        jd(parts.tx_envelope)[
            "availableSpectrumInquiryResponses"] = []
        ...
        for req_idx, request in enumerate(requests):
            ...
            if response is None:
                parts.orphan_requests.append(request)
                continue
            ...
            if config_info is None:
                ...
                parts.orphan_requests.append(request)
                parts.orphan_responses.append(response)
                continue
            expire_time_str: Optional[str] = \
                response.get("availabilityExpireTime")
            if expire_time_str is not None:
                # js() raises TypeError on non-string values from the
                # attacker-suppliable AFC Response JSON; update_db converts
                # it to JsonFormatError (poison-bundle path) instead of an
                # uncaught AttributeError crash-looping the siphon
                expire_time_str = js(expire_time_str).replace("Z", "+00:00")
            ...
            response["requestId"] = ""
            parts.request_responses[req_id] = \
        ...
        for orphan in response_map.values():
            parts.orphan_responses.append(orphan)
        self._store_parts = parts
        return self._store_parts
    ...
    def _check_config_indexes(self, message: AlsMessage) -> None:
        ...
        bad_indexes = \
            [i for i in self._configs if (i is not None) and not (0 <= i < rc)]
        if bad_indexes:
...
class AfcConfigLookup(LookupBase[uuid.UUID, str]):
    ...
    def _rows_from_value(self, value: str, month_idx: int) \
        ...
        try:
            ...
            # RecursionError (deeply nested config) is mapped to
            # JsonFormatError so both the batch and per-bundle DB-write
            # handlers route the poison bundle to decode_error instead of
            # letting it unwind main_loop (which crash-loops the siphon on
            # Kafka redelivery, halting the regulatory audit pipeline).
            raise JsonFormatError(f"Malformed AFC Config JSON: {ex!r}",
    ...
    def update_db(self,
        ...
        try:
            ...
            self._update_foreign_targets(row_infos=row_infos,
                                         month_idx=month_idx)
        except (LookupError, TypeError, ValueError, AttributeError) as ex:
            # AttributeError included so any type-confusion while decomposing
            # an attacker-suppliable bundle (e.g. str method called on a
            # non-string JSON field) degrades to the dropped-poison-bundle
            # path instead of unwinding main_loop into a redelivery crash loop
            raise JsonFormatError(
        ...
        except (sa.exc.SQLAlchemyError, ValueError) as ex:
            # ValueError included because psycopg2 raises a plain ValueError
            # (not an SQLAlchemyError) on NUL bytes in bound string
            # parameters; it must take the same DbFormatError poison-bundle
            # path (matching LookupBase.update_db above)
...
class DecodeErrorTableWriter(AlsTableBase):
    ...
    def write_decode_error(
        ...
        # This is the error-writer of last resort for unparseable input
        # messages: psycopg2 raises ValueError on a NUL (0x00) byte in a
        # bound string parameter, which is not a psycopg2.Error and would
        # escape uncaught, causing an uncommitted offset loop. Sanitize NUL before bind.
        ...
        try:
            ...
            # immediately, before the corresponding Kafka offset is
            # committed.  Without this the autobegun transaction is rolled
            # back at exit and every decode_error record is silently lost.
            self._conn.commit()
        except Exception as exc:
            # Log and drop: losing one decode_error row is preferable to
            # letting the malformed-message handler itself crash-loop the
            # ALS pipeline.  Roll back so the connection stays usable.
            logging.error(f"Error writing decode_error table: {repr(exc)}")
            try:
                self._conn.rollback()
            except Exception:
                pass
    ...
    def fetch_assembled(
        ...
        for bundle in self._bundle_queue:
            ...
            if (max_requests is not None) and \
                ...
                continue
            ret.append(bundle)
            num_requests += bundle.request_count()
...
class KafkaClient:
    ...
    def __init__(self, args: Any, subscribe_als: bool, subscribe_log: bool,
        ...
        # Fail-closed override: only explicit affirmative consent disables
        # the guard (same contract as kafka-entrypoint.sh) -- a truthiness
        # check would accept "false"/"0"/"no" as consent.
        error_if(
            effective_proto == "PLAINTEXT"
            and os.environ.get(
                "ALS_SIPHON_ALLOW_PLAINTEXT_KAFKA", "").lower()
            not in ("1", "true", "yes"),
    ...
    def _read_als_kafka_messages(
        ...
        for kafka_message in kafka_messages:
            ...
            try:
                ...
                # Outer guard: the error-writer consumes raw input
                # data; a failure inside it must neither escape main_loop
                # nor prevent the offset being marked processed
                try:
                    self._decode_error_writer.write_decode_error(
                        msg=ex.msg, line=ex.code_line, data=ex.data)
                except Exception as write_ex:
                    logging.error(
                        f"Error writing decode_error row: {repr(write_ex)}")
    ...
    def _process_log_kafka_messages(
        ...
        for kafka_message in kafka_messages:
            ...
            try:
                ...
                records.append(
                    LogsDatabase.Record(
                        # js() raises TypeError (caught below) on non-string
                        # 'source'; NUL bytes are escaped because psycopg2
                        # raises a plain ValueError on NUL in bound string
                        # parameters (see DecodeErrorTableWriter)
                        source=js(log_message["source"]).replace(
                            "\u0000", "\\x00"),
                ...
                # Mirrors _read_als_kafka_messages' broad guard: json.loads
                # raises RecursionError (a RuntimeError subclass, NOT in the
                # tuple above) on deeply nested input. No single message's
                # content may prevent offset progress, so route the raw
                # message to decode_error and mark the offset processed.
                self._metrics.siphon_log_malformed(topic).inc()
                try:
                    self._decode_error_writer.write_decode_error(
                        msg=f"Can't decode log message: {repr(ex)}",
                        line=None, data=kafka_message.value)
                except Exception as write_ex:
                    logging.error(
                        f"Error writing decode_error row: {repr(write_ex)}")
                self._kafka_positions.mark_processed(
        ...
        if records and (self._ldb is not None):
            ...
            try:
                ...
                # ValueError/TypeError included: psycopg2 raises a plain
                # ValueError on NUL bytes in bound string parameters (not an
                # SQLAlchemyError). Route the failed batch to decode_error
                # and advance the offsets so a single poison message cannot
                # crash-loop or permanently wedge the siphon's offset commit.
                logging.error(
                    f"DB write failed for topic {topic!r}, "
                    f"routing batch to decode_error: {ex}")
                try:
                    self._decode_error_writer.write_decode_error(
                        msg=f"Log DB write failed for topic {topic!r}: {ex}",
                        line=None, data=str(records))
                except Exception as write_ex:
                    logging.error(
                        f"Error writing decode_error row: {repr(write_ex)}")
                for kafka_message in kafka_messages:
    ...
    def _write_als_messages(self) -> bool:
        ...
        try:
            ...
            self._metrics.siphon_afc_msg_in_progress().set(
                self._als_bundles.get_incomplete_count())
            transaction = None
        except (JsonFormatError, DbFormatError, RecursionError) as ex:
            ...
            # RecursionError is included (mirroring the JSON-log path's broad
            # guard) so any residual deeply-nested parse on this path degrades
            # to the per-bundle replay instead of unwinding main_loop.
            if transaction is not None:
                transaction.rollback()
                transaction = None
            # Lookup caches are populated inside the transaction that just
            # rolled back, so they must be invalidated on EVERY failure kind
            # (DbFormatError included), else cached keys reference rows the
            # rollback deleted and subsequent inserts FK-violate
        ...
        for key, bundle in data_dict.items():
            ...
            try:
                ...
                if transaction is not None:
                    transaction.rollback()
                    transaction = None
                # Invalidate lookup caches populated inside the rolled-back
                # per-bundle transaction (same reason as the batch path)
                self._lookups.reread()
                self._metrics.siphon_als_malformed().inc()
