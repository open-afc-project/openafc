...
# that already exists in this cluster. bulk_rw keeps CREATE explicitly in
# application databases only: it is the documented read/write role and
# application services create their tables/partitions with it.  template1
# is deliberately excluded from the grant: CREATE there would let a
# bulk_rw credential holder seed decoy/search-path objects that CREATE
# DATABASE copies into every new database (CVE-2018-1058 staging).
for _db in template1 $(psql --username "$POSTGRES_USER" --no-password -Atc \
    ...
    if [ "$_db" != "template1" ] && [ "$_db" != "postgres" ]; then
        psql --username "$POSTGRES_USER" --no-password --dbname "$_db" \
