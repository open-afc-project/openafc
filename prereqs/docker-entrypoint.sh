...
if [ "$AFC_PROD_ENV" == "final" ]; then
	...
	# apache
	echo "APACHE"
	if [[ ! -z "$AFC_OBJST_HIST_HOST" ]]; then
		sed -i "/<\/VirtualHost>/i  ProxyPassReverse \/dbg http:\/\/$AFC_OBJST_HIST_HOST:4999\/dbg\n  ProxyPass \/dbg http:\/\/$AFC_OBJST_HIST_HOST:4999\/dbg\n  ProxyPreserveHost On\n  ProxyRequests Off" $(ls /etc/httpd/conf.d/*-notls.conf)
		sed -i "/<\/VirtualHost>/i  ProxyPassReverse \/dbg http:\/\/$AFC_OBJST_HIST_HOST:4999\/dbgs\n  ProxyPass \/dbg http:\/\/$AFC_OBJST_HIST_HOST:4999\/dbgs\n  ProxyPreserveHost On\n  ProxyRequests Off" $(ls /etc/httpd/conf.d/*-tls.conf)
	fi
