...
def email_if_needed(state_db: StateDb, settings: Any) -> None:
    ...
    for reg in sorted({cast(str, r) for r in reg_data_changes.keys() if r is not None} |
                      regions):
...
def main(argv: List[str]) -> None:
    ...
    # Make inert alarm configuration visible: with ULS_ALARM_EMAIL_TO set
    # but an incomplete SMTP endpoint, send_email_smtp() silently
    # early-returns; with a non-positive alarm interval, email_if_needed()
    # never sends the alarm (expired() returns False for max_age_hr <= 0).
    if settings.email_to:
        if not (bool(settings.smtp_server) and bool(settings.smtp_username)):
            logging.warning(
                "ULS_ALARM_EMAIL_TO is set but SMTP settings are incomplete "
                "(both ULS_SMTP_SERVER and ULS_SMTP_USERNAME are required) "
                "- alarm/beacon emails will NOT be sent")
        if not (settings.alarm_email_interval_hr is not None and
                settings.alarm_email_interval_hr > 0):
            logging.warning(
                "ULS_ALARM_EMAIL_TO is set but ULS_ALARM_ALARM_INTERVAL_HR "
                "is unset or not positive - alarm emails will NOT be sent")
