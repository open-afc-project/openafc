...
#
# Usage:
#   ./update_uls_manifest.sh --db-dir DIR --manifest FILE --current NAME [OPTIONS]
#
# Required:
#   --db-dir   DIR     Signer-local staging directory containing the
#                      FS_*.sqlite3 files produced by the trusted rebuild.
#                      Must NOT be writable by other parties (e.g. the shared
#                      NFS volume): every production-named file in it is
#                      attested into the signed manifest.
#   --manifest FILE    Local path to write manifest.json
#   --current  NAME    Basename of the database the pipeline just built;
#                      recorded as the manifest's single activatable
#                      '_current' entry.
#
# Optional:
...
#
# Cron example (run every 4 hours after the trusted rebuild, publish to
# production; LAST_BUILT is the rebuild pipeline's own record of the
# basename it just produced):
#   0 */4 * * *  /opt/afc/brcm-afc/uls/update_uls_manifest.sh \
#       --db-dir  /opt/afc/uls-staging/ \
#       --manifest /opt/afc/uls-staging/uls-hash-manifest.json \
#       --current "$(cat /opt/afc/uls-staging/LAST_BUILT)" \
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            sed -n 's/^# //p; s/^#$//p' "$0" | head -60
            exit 0 ;;
        *) echo "Unknown option: $1" >&2; exit 1 ;;
    esac
done

if [[ -z "${DB_DIR}" || -z "${MANIFEST}" || -z "${CURRENT}" ]]; then
    echo "ERROR: --db-dir, --manifest and --current are required." >&2
