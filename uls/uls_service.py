...
class Settings(BaseSettings):
    ...
    hash_manifest_hmac_key: Optional[str] = \
        pydantic.Field(
            ...
            "DEPRECATED delivery: prefer ULS_HASH_MANIFEST_HMAC_KEY_FILE "
            "(plain env vars leak via 'docker inspect' and /proc/<pid>/environ).",
            json_schema_extra={
                "no": "No manifest authentication (insecure)",
                "convert": lambda v: "<redacted>" if v else None})
    hash_manifest_hmac_key_file: Optional[str] = \
        pydantic.Field(
            None, validation_alias="ULS_HASH_MANIFEST_HMAC_KEY_FILE",
            description="Name of file (Docker secret) holding the "
            "HMAC-SHA256 manifest key. Preferred over "
            "ULS_HASH_MANIFEST_HMAC_KEY. An empty file behaves as unset.",
            json_schema_extra={"no": "None"})
...
class DbDiff:
    ...
    def __init__(self, prev_filename: str, new_filename: str,
        ...
        except (pydantic.ValidationError, LookupError, TypeError,
                ValueError) as ex:
            # Malformed invalidation data (e.g. a poisoned previous DB)
            # must degrade to a failed cycle, not kill the process
            logging.error(f"Invalidation data malformed: {ex}")
            self.diff_tiles = []
            self.diff_beams = []
            return
        self.valid = True
...
class UlsFileChecker:
    ...
    def _check_hash(self, new_filename: str,
        ...
        # share as the databases it attests, so independent HMAC authentication
        # is required to attest integrity (including the _iat replay binding).
        ...
        # Parse only AFTER the manifest bytes have been authenticated
        # (parse-before-verify let unauthenticated bytes reach the JSON
        # parser). Catch RecursionError too, so a hostile deeply-nested
        # document takes the fail-closed rejection path instead of
        # terminating the service.
        if manifest_raw_bytes is not None:
            try:
                manifest = json.loads(manifest_raw_bytes.decode("utf-8"))
            except (json.JSONDecodeError, ValueError,
                    RecursionError) as ex:
                return (True, f"Cannot parse hash manifest: {ex}")
        ...
        # mark in the service state database — NOT on the attested share,
        # whose writers are exactly the adversary this gate defends against.
        if self._hash_manifest_hmac_key:
            try:
            ...
            # The high-water mark lives in the service state database: a
            # dot-file beside the databases sits on the very share the
            # manifest gate distrusts, so a share-writer could delete it
            # and reset the replay defence. A missing/unreadable mark is
            # fail-closed — the operator must baseline it
            # (StateDb.write_manifest_iat) rather than the check silently
            # restarting from -1.
            if self._state_db is None:
                return (True,
                        "Manifest '_iat' replay defence requires the "
                        "service state database, but no StateDb was "
                        "provided — refusing manifest")
            last_iat = self._state_db.read_manifest_iat()
            if last_iat is None:
                return (True,
                        "No manifest '_iat' high-water mark recorded in "
                        "the state database — operator must baseline the "
                        "replay defence (StateDb.write_manifest_iat) "
                        "before manifests are accepted")
            if manifest_iat < last_iat:
                return (True,
                        f"Hash manifest _iat={manifest_iat} older than last "
                        f"accepted {last_iat} — replay rejected")
            if manifest_iat > last_iat:
                self._state_db.write_manifest_iat(manifest_iat)

        # Rollback-replay defence: the manifest accumulates an entry for
        # every DB ever attested and '_iat' binds only manifest freshness,
        # so a perfectly fresh signed manifest still attests every
        # historical database. Only the manifest's designated '_current'
        # entry may be activated.
        current_key = manifest.get("_current")
        if not isinstance(current_key, str) or not current_key:
            return (True,
                    "Hash manifest lacks a '_current' designation — "
                    "refusing manifest that attests a set of databases "
                    "rather than a single current one (rollback-replay "
                    "defence; regenerate with generate_hash_manifest.py)")
        if manifest_key != current_key:
            return (True,
                    f"'{manifest_key}' is not the manifest's designated "
                    f"current database ('{current_key}') — rollback "
                    f"replay rejected")
...
def main(argv: List[str]) -> None:
    ...
    try:
        ...
        # Prefer file (Docker secret) delivery of the manifest HMAC key over
        # the plain environment variable (exposed via 'docker inspect' and
        # /proc/<pid>/environ). An empty secret file behaves as unset.
        hash_manifest_hmac_key = settings.hash_manifest_hmac_key
        if settings.hash_manifest_hmac_key_file:
            try:
                with open(settings.hash_manifest_hmac_key_file,
                          encoding="utf-8") as fh:
                    file_key = fh.read().strip()
            except OSError as exc:
                logging.critical(
                    "FATAL: cannot read ULS_HASH_MANIFEST_HMAC_KEY_FILE "
                    "%s: %s", settings.hash_manifest_hmac_key_file, exc)
                sys.exit(1)
            if file_key:
                hash_manifest_hmac_key = file_key
        ...
        uls_file_checker = \
            UlsFileChecker(
                ...
                hash_manifest_hmac_key=hash_manifest_hmac_key,
        ...
        # Read fd pinned to the staged database inode: all verification
        # reads go through this fd so a share-writer cannot swap the
        # staging path between the hash gate and activation (TOCTOU).
        staged_fd: Optional[int] = None
        ...
        while True:
            ...
            try:
                ...
                # AR-28 hardening: create the staging destination with
                # O_CREAT|O_EXCL|O_NOFOLLOW instead of shutil.copy2, which
                # follows a pre-planted symlink at this predictable staging
                # name and would let a share-writer redirect the attested
                # bytes to a path they control.  A pre-planted entry now
                # fails the staging copy closed instead of being followed.
                if os.path.lexists(temp_uls_file_name):
                    logging.warning(f"Removing stale staging file "
                                    f"'{temp_uls_file_name}'")
                    os.unlink(temp_uls_file_name)
                dst_fd = os.open(
                    temp_uls_file_name,
                    os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_NOFOLLOW,
                    os.stat(new_uls_file).st_mode & 0o777)
                with open(new_uls_file, "rb") as src_f, \
                        os.fdopen(dst_fd, "wb") as dst_f:
                    shutil.copyfileobj(src_f, dst_f)
                # Pin the staged inode. Every hash below is computed by
                # re-opening /proc/self/fd/<staged_fd> (the exact inode just
                # written), so a share-writer's unlink+recreate of the
                # predictable staging name can no longer change the bytes
                # being verified; the rename gate below then requires the
                # path to still name this same inode before activation.
                staged_fd = os.open(temp_uls_file_name,
                                    os.O_RDONLY | os.O_NOFOLLOW)
                staged_fd_path = f"/proc/self/fd/{staged_fd}"
                ...
                if updated_regions or settings.force:
                    ...
                    # Hash through the pinned fd (same inode the manifest
                    # gate verified and the FSID embed wrote in place), not
                    # the swappable staging path: a swap during the embed
                    # subprocess can no longer become the digest baseline.
                    with open(staged_fd_path, "rb") as f:
                        for chunk in iter(lambda: f.read(65536), b""):
                    ...
                    if uls_file_checker.valid(
                            ...
                            # The hash gate must open the exact file
                            # object being activated: the pinned staged
                            # inode, immune to a path swap during the
                            # long-running _check_afc content check.
                            hash_filename=staged_fd_path) or \
                            (settings.force and
                             not uls_file_checker.manifest_configured):
                        ...
                        # Activation continuity gate: the staging path must
                        # still name the exact inode every hash above was
                        # computed over. A share-writer's unlink+recreate
                        # (or symlink plant) between verification and rename
                        # fails the swap closed instead of being activated.
                        fd_stat = os.fstat(staged_fd)
                        path_stat = os.lstat(temp_uls_file_name)
                        if (path_stat.st_ino, path_stat.st_dev) != \
                                (fd_stat.st_ino, fd_stat.st_dev):
                            raise ProcessingException(
                                "Staged ULS database was replaced after "
                                "verification (st_ino/st_dev mismatch); "
                                "aborting ULS swap")
                        ...
                        if rcache and (db_diff is None):
                            # Cold start (no previous DB, e.g. the symlink
                            # was deleted on the shared volume): there is no
                            # diff to scope the invalidation, but the rcache
                            # may still hold grants derived from a previous
                            # database.  Cold-start activation must not
                            # silently skip invalidation - fail closed with
                            # a complete invalidation before the swap.
                            logging.info(
                                "Cold-start activation with rcache enabled: "
                                "requesting complete rcache invalidation")
                            als_record.invalidation = ["complete"]
                            try:
                                rcache.rcache_invalidate()
                            except Exception as rcache_ex:  # noqa: BLE001
                                raise ProcessingException(
                                    f"rcache complete invalidation failed; "
                                    f"aborting DB swap to prevent stale "
                                    f"entries: {rcache_ex}") from rcache_ex
                        ...
                        # does not trigger the on_conflict WHERE state != Invalid
                        ...
                        if settings.force:
                            ...
                            save_recent_download(set(new_uls_identity.keys()),
                                                 settings.num_saves, settings.temp_dir, settings.save_dir)
                        else:
                            # Saves based on new data
                            save_recent_download(updated_regions, settings.num_saves,
                                                 settings.temp_dir, settings.save_dir)
                    ...
                    sa.exc.SQLAlchemyError, pydantic.ValidationError) as ex:
                err_msg = str(ex)
