...
if [ -n "${RCACHE_URL:-}" ]; then
    ...
    curl -s -o /dev/null -K - <<CURLCFG || true
url = "${RCACHE_URL}/invalidation_state/true"
request = "POST"
...
CURLCFG
    ...
    for _ in $(seq 1 10); do
        _valid=$(curl -s -K - <<CURLCFG 2>/dev/null | grep -o '"num_valid_entries":[0-9]*' | cut -d: -f2 || echo "0"
url = "${RCACHE_URL}/status"
header = "Authorization: Bearer ${RCACHE_API_KEY}"
CURLCFG
)
