...
# Batch-caller token for Internal-route precompute jobs (uls_downloader).
# Grants the Internal route only; unlike AFC_DISPATCHER_TOKEN it never
# authorises mTLS attestation headers. Mounted into uls_downloader and the
# verifiers (rat_server, msghnd, afcserver) — NOT into dispatcher.
gen_or_rotate AFC_PRECOMPUTE_TOKEN "$ROTATE_TOKEN"
