...
class ColumnName(Enum):
    ...
    VERTICAL_UNCERTAINTY = (
        'Unlicensed device transmit antenna vertical location uncertainty in meters. '
        'Must be a positive integer (no decimal point).')
...
def generate_request_json(csv_dict, file_index, cert_id):
    ...
    location_dict['elevation'] = dict(
        ...
        heightType='AGL',
        verticalUncertainty=return_int_or_float(
            csv_dict[ColumnName.VERTICAL_UNCERTAINTY.value]),
    )
    ...
    if location_type == LocationType.ELLIPSE.value:
        center_dict = dict(
            longitude=return_int_or_float(
                csv_dict[ColumnName.LONTITUDE.value]),
            latitude=return_int_or_float(csv_dict[ColumnName.LATITUDE.value]),
        )
        location_dict['ellipse'] = dict(
            center=center_dict,
            majorAxis=return_int_or_float(
                csv_dict[ColumnName.MAJORAXIS.value]),
            minorAxis=return_int_or_float(
                csv_dict[ColumnName.MINORAXIS.value]),
            orientation=return_int_or_float(
                csv_dict[ColumnName.ORIENTATION.value]),
        )
    elif location_type == LocationType.POINT_RADIUS.value:
        center_dict = dict(
            longitude=return_int_or_float(
                csv_dict[ColumnName.POINT_LONTITUDE.value]),
            latitude=return_int_or_float(
                csv_dict[ColumnName.POINT_LATITUDE.value]),
        )
        location_dict['ellipse'] = dict(
            center=center_dict,
            majorAxis=return_int_or_float(
                csv_dict[ColumnName.POINT_RADIUS_AXIS.value]),
            minorAxis=return_int_or_float(
                csv_dict[ColumnName.POINT_RADIUS_AXIS.value]),
            orientation=0
        )
    elif location_type == LocationType.LINEAR_POLYGON.value:
        ob_entry_list = get_outerboundary_list(
            csv_dict[ColumnName.LINEAR_POLYGON_VERTICES.value])
        ob_list = []
        for i in ob_entry_list:
            ob_list.append(
                dict(longitude=return_int_or_float(
                    i[0]), latitude=return_int_or_float(i[1]))
            )
        ...
        center_dict = dict(
            longitude=return_int_or_float(
                csv_dict[ColumnName.CENTER_LONTITUDE.value]),
            latitude=return_int_or_float(
                csv_dict[ColumnName.CENTER_LATITUDE.value]),
        )
        ob_entry_list = get_outerboundary_list(
            csv_dict[ColumnName.VECTORS_FROM_CENTER.value])
        ob_list = []
        for i in ob_entry_list:
            ob_list.append(
                dict(length=return_int_or_float(
                    i[0]), angle=return_int_or_float(i[1]))
            )
