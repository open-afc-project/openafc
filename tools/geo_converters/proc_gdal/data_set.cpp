...
/******************************************************************************************/
// Run "convert <srcFile> <dstFile>" without a shell so that shell metacharacters
// in template-supplied file names cannot be interpreted as commands.
static void execConvert(const std::string &srcFile, const std::string &dstFile)
{
	std::cout << "COMMAND: convert " << srcFile << " " << dstFile << std::endl;
	pid_t pid = fork();
	if (pid == 0) {
		execlp("convert", "convert", srcFile.c_str(), dstFile.c_str(), (char *)NULL);
		_exit(127);
	} else if (pid > 0) {
		int status;
		waitpid(pid, &status, 0);
	} else {
		std::cout << "ERROR: fork() failed running convert" << std::endl;
	}
}
/******************************************************************************************/
...
void DataSetClass::run()
{
	if (parameterTemplate.function == "combine3D2D") {
		...
		errStr << "ERROR: function set to unrecognized value: "
		       << parameterTemplate.function;
...
void DataSetClass::combine3D2D()
{
	...
	GdalDataModel *gdalDataModel3D = new GdalDataModel(parameterTemplate.srcFile3D,
							   parameterTemplate.heightFieldName3D);
	// gdalDataModel3D->printDebugInfo();

	GdalDataModel *gdalDataModel2D = new GdalDataModel(parameterTemplate.srcFile2D,
							   parameterTemplate.heightFieldName2D);
	...
	outputDS =
		poDriver->Create(parameterTemplate.outputFile.c_str(), 0, 0, 0, GDT_Unknown, NULL);
	if (outputDS == NULL) {
	...
	poLayer = outputDS->CreateLayer(parameterTemplate.outputLayer.c_str(),
					NULL,
					wkbPolygon,
					NULL);
	if (poLayer == NULL) {
	...
	if (poLayer->CreateField(&heightField) != OGRERR_NONE) {
		std::cout << "Creating " << parameterTemplate.outputHeightFieldName
			  << " field failed.\n";
		exit(1);
	}
	int heightFIeldIdx =
		poLayer->FindFieldIndex(parameterTemplate.outputHeightFieldName.c_str(), TRUE);
	/**************************************************************************************/
	...
	while ((ptrOFeature = std::unique_ptr<OGRFeature, GdalHelpers::FeatureDeleter>(
			layer3D->GetNextFeature())) != NULL) {
		...
		if (ptrOGeometry != NULL &&
		    ptrOGeometry->getGeometryType() == OGRwkbGeometryType::wkbPolygon) {
			double height = ptrOFeature->GetFieldAsDouble(
				gdalDataModel3D->heightFieldIdx);
	...
	while ((ptrOFeature2D = std::unique_ptr<OGRFeature, GdalHelpers::FeatureDeleter>(
			layer2D->GetNextFeature())) != NULL) {
		// GDAL maintains ownership of returned pointer, so we should not manage its memory
		...
		if (ptrOGeometry != NULL &&
			...
			if ((ptrOFeature = std::unique_ptr<OGRFeature, GdalHelpers::FeatureDeleter>(
				     layer3D->GetNextFeature())) == NULL) {
				double height = ptrOFeature2D->GetFieldAsDouble(
					gdalDataModel2D->heightFieldIdx);
...
void DataSetClass::fixRaster()
{
	...
	// Create a per-invocation private temp dir so the intermediate PPM file
	// is not placed under a predictable world-writable /tmp path that a
	// co-resident user could pre-stage as a symlink.
	char _ppmDirBuf[] = "/tmp/proc_gdal_XXXXXX";
	bool _ownTmpFile = parameterTemplate.tmpImageFile.empty();
	if (_ownTmpFile) {
		if (mkdtemp(_ppmDirBuf) == nullptr) {
			throw std::runtime_error("ERROR: cannot create temp directory");
		}
	}
	std::string _ppmTmpDir(_ownTmpFile ? _ppmDirBuf : "");
	std::string tmpImageFile = (_ownTmpFile ? (_ppmTmpDir + "/image.ppm") :
						  parameterTemplate.tmpImageFile);
	...
	GDALDataset *srcDS = (GDALDataset *)GDALOpen(parameterTemplate.srcFileRaster.c_str(),
						     GA_ReadOnly);
	...
	GDALDataset *dstDS;
	dstDS = poDriver->CreateCopy(parameterTemplate.outputFile.c_str(),
				     srcDS,
				     FALSE,
				     options,
				     GDALTermProgress,
	...
	// Read srcDS-derived sizes BEFORE freeing it: GDALClose() destroys the
	// dataset, and dereferencing it afterwards is a use-after-free. dstDS is
	// an exact copy of srcDS (CreateCopy above), so its raster size is
	// identical and safe to read after srcDS is closed.
		...
		if (hProj != nullptr) {
			...
			if (hLatLong) {
				...
				OGRSpatialReference *poLatLong =
					reinterpret_cast<OGRSpatialReference *>(hLatLong);
	...
	// Reject absurd declared dimensions before allocating: nXSize * 4
	// computed in int wraps for a crafted raster (nXSize > 2^29), undersizing
	// the buffer that RasterIO later fills with nXSize samples.
	if (nXSize <= 0 || nYSize <= 0 || nXSize > 0x10000000 || nYSize > 0x10000000) {
		errStr << "ERROR: raster dimensions out of range: " << nXSize << " x " << nYSize;
		throw std::runtime_error(errStr.str());
	}
	float *pafScanline = (float *)CPLMalloc(static_cast<size_t>(nXSize) *
						GDALGetDataTypeSizeBytes(GDT_Float32));

	if (fabs(parameterTemplate.nodataVal) > std::numeric_limits<float>::max()) {
		errStr << "ERROR: nodataVal set to illegal value: " << parameterTemplate.nodataVal
		       << ", max value for float is " << std::numeric_limits<float>::max();
		throw std::runtime_error(errStr.str());
	}
	...
	if (genImageFlag) {
		...
		if (!imageScanline) {
			throw std::runtime_error("ERROR: memory allocation failed for "
						 "imageScanline");
		}
	...
	std::cout << "Num NODATA values " << numNoData << " ("
		  << 100.0 * numNoData / (1.0 * nXSize * nYSize) << "%)" << std::endl;
	std::cout << "Num values below min clamp " << parameterTemplate.clampMin << ": "
		  << numClampMin << " (" << 100.0 * numClampMin / (1.0 * nXSize * nYSize) << "%)"
		  << std::endl;
	std::cout << "Num values above max clamp " << parameterTemplate.clampMax << ": "
		  << numClampMax << " (" << 100.0 * numClampMax / (1.0 * nXSize * nYSize) << "%)"
		  << std::endl;
	std::cout << "Num values with fabs() below minMag " << parameterTemplate.minMag << ": "
		  << numMinMag << " (" << 100.0 * numMinMag / (1.0 * nXSize * nYSize) << "%)"
		  << std::endl;
	std::cout << "Num VALID  values " << numValid << " ("
		  << 100.0 * numValid / (1.0 * nXSize * nYSize) << "%)" << std::endl;

	int numModified = numClampMin + numClampMax + numMinMag;
	std::cout << "Num values modified: " << numModified << " ("
		  << 100.0 * numModified / (1.0 * nXSize * nYSize) << "%)" << std::endl;

	int numData = numValid;
	std::cout << "Num DATA values: " << numData << " ("
		  << 100.0 * numData / (1.0 * nXSize * nYSize) << "%)" << std::endl;

	CPLFree(pafScanline);

	if (numValid) {
		double pdfMin, pdfMax, pdfMean, pdfStdDev;
		rasterBand->ComputeStatistics(0,
					      &pdfMin,
					      &pdfMax,
					      &pdfMean,
					      &pdfStdDev,
	...
	if (genImageFlag) {
		execConvert(tmpImageFile, parameterTemplate.imageFile);
	}
	if (_ownTmpFile) {
		unlink(tmpImageFile.c_str());
		rmdir(_ppmTmpDir.c_str());
	}
}
/******************************************************************************************/
...
void DataSetClass::fixVector()
{
	...
	GdalDataModel *gdalDataModel = new GdalDataModel(parameterTemplate.srcFileVector,
							 parameterTemplate.srcHeightFieldName);
	...
	if (layer->GetExtent(&oExt, TRUE) == OGRERR_NONE) {
		...
		throw std::runtime_error("ERROR: fixVector: layer->GetExtent() failed, cannot size "
					 "image grid");
	}
	...
	// Reject non-finite or absurd layer extents before grid sizing:
	// (int)floor(extent / res) is UB for |extent / res| > INT_MAX and the
	// image grid is malloc'ed from the result (mirrors the raster
	// dimension guard in fixRaster / mbRasterCvg).
	double dLonN0 = floor(minLon / res);
	double dLonN1 = ceil(maxLon / res);
	double dLatN0 = floor(minLat / res);
	double dLatN1 = ceil(maxLat / res);
	if (!std::isfinite(dLonN0) || !std::isfinite(dLonN1) || !std::isfinite(dLatN0) ||
	    !std::isfinite(dLatN1) || fabs(dLonN0) > (double)0x10000000 ||
	    fabs(dLonN1) > (double)0x10000000 || fabs(dLatN0) > (double)0x10000000 ||
	    fabs(dLatN1) > (double)0x10000000 || dLonN1 - dLonN0 <= 0.0 || dLatN1 - dLatN0 <= 0.0 ||
	    dLonN1 - dLonN0 > (double)0x10000000 || dLatN1 - dLatN0 > (double)0x10000000) {
		throw std::runtime_error("ERROR: fixVector: layer extent out of range, "
					 "cannot size image grid");
	}
	...
	if (!image) {
		throw std::runtime_error("ERROR: memory allocation failed for image");
	}
	for (imgXIdx = 0; imgXIdx < imageXSize; ++imgXIdx) {
		image[imgXIdx] = (int *)malloc(imageYSize * sizeof(int));
		if (!image[imgXIdx]) {
			throw std::runtime_error("ERROR: memory allocation failed for image row");
		}
	...
	while ((ptrOFeature = std::unique_ptr<OGRFeature, GdalHelpers::FeatureDeleter>(
		...
		if (ptrOGeometry != NULL) {
			...
			if (geometryType == OGRwkbGeometryType::wkbPolygon) {
				...
				std::cout << "WARNING: Unrecognized Geometry Type: "
					  << OGRGeometryTypeToName(geometryType) << std::endl;
			...
			if ((genImageFlag) && (useGeometry)) {
				...
				/* Feature envelopes are independent of the layer extent
				 * used to size image[][]; a crafted file can place them
				 * outside the grid or make them non-finite.  Clamp before
				 * indexing. */
				if (std::isfinite(env.MinX) && std::isfinite(env.MaxX) &&
				    std::isfinite(env.MinY) && std::isfinite(env.MaxY)) {
					double dx0 = floor(env.MinX / res) - (double)lonN0;
					double dx1 = floor(env.MaxX / res) - (double)lonN0;
					double dy0 = floor(env.MinY / res) - (double)latN0;
					double dy1 = floor(env.MaxY / res) - (double)latN0;

					if ((dx1 >= 0.0) && (dx0 <= (double)(imageXSize - 1)) &&
					    (dy1 >= 0.0) && (dy0 <= (double)(imageYSize - 1))) {
						int x0 = (dx0 < 0.0 ? 0 : (int)dx0);
						int x1 = (dx1 > (double)(imageXSize - 1) ?
								  imageXSize - 1 :
								  (int)dx1);
						int y0 = (dy0 < 0.0 ? 0 : (int)dy0);
						int y1 = (dy1 > (double)(imageYSize - 1) ?
								  imageYSize - 1 :
								  (int)dy1);

						for (imgXIdx = x0; imgXIdx <= x1; imgXIdx++) {
							for (imgYIdx = y0; imgYIdx <= y1;
							     imgYIdx++) {
...
void DataSetClass::vectorCvg()
{
	...
	if (layer->GetExtent(&oExt, TRUE) == OGRERR_NONE) {
		...
		throw std::runtime_error("ERROR: vectorCvg: layer->GetExtent() failed, cannot size "
					 "image grid");
	}
	...
	if (!std::isfinite(dLonN0) || !std::isfinite(dLonN1) || !std::isfinite(dLatN0) ||
	    ...
	    fabs(dLatN1) > (double)0x10000000 || dLonN1 - dLonN0 <= 0.0 || dLatN1 - dLatN0 <= 0.0 ||
	    dLonN1 - dLonN0 > (double)0x10000000 || dLatN1 - dLatN0 > (double)0x10000000) {
		throw std::runtime_error("ERROR: vectorCvg: layer extent out of range, "
					 "cannot size image grid");
	}
	...
	while ((ptrOFeature = std::unique_ptr<OGRFeature, GdalHelpers::FeatureDeleter>(
		...
		if (ptrOGeometry != NULL) {
			...
			if (geometryType == OGRwkbGeometryType::wkbPolygon) {
				...
				std::cout << "Contains features of type: "
					  << OGRGeometryTypeToName(geometryType) << std::endl;
			}
		}
	}
...
void DataSetClass::mbRasterCvg()
{
	...
	if (srcDS->GetGeoTransform(adfGeoTransform) == CE_None) {
		...
		throw std::runtime_error("ERROR in mbRasterCvg(), unable to determine origin/pixel "
					 "size");
	}

	double pixelSize = adfGeoTransform[1];
	if (pixelSize <= 0.0) {
		throw std::runtime_error("ERROR in mbRasterCvg(), X pixel size must be positive");
	}
	if (fabs(pixelSize + adfGeoTransform[5]) > 1.0e-8) {
		throw std::runtime_error("ERROR: X / Y pixel sizes not properly set");
	}
	...
	if (!imageScanline) {
		throw std::runtime_error("ERROR: memory allocation failed for imageScanline");
	}
	float *pafScanline = (float *)CPLMalloc(static_cast<size_t>(nXSize) *
						GDALGetDataTypeSizeBytes(GDT_Float32));

	// Create a per-invocation private temp directory (mode 0700) instead of
	// writing to predictable /tmp/image_<N>.ppm paths that a co-resident user
	// could pre-stage as a symlink to truncate an operator-writable file.
	char ppmDirTemplate[] = "/tmp/proc_gdal_XXXXXX";
	if (mkdtemp(ppmDirTemplate) == nullptr) {
		throw std::runtime_error("ERROR: cannot create temp directory");
	}
	std::string ppmDir(ppmDirTemplate);
	...
	for (rasterBandIdx = 1; rasterBandIdx <= numRasterBand; rasterBandIdx++) {
		...
		/* Create PPM File */
		/**************************************************************************************/
		std::string ppmFile = ppmDir + "/image_" + std::to_string(rasterBandIdx) + ".ppm";
		ppmFiles.push_back(ppmFile);
		FILE *fppm = (FILE *)NULL;
			...
			for (xIdx = 0; xIdx < nXSize; xIdx++) {
				...
				if (rasterBandIdx == 1) {
					if (hasNoData && (pafScanline[xIdx] == nodataValue)) {
						numNoData++;
						pafScanline[xIdx] =
							(float)parameterTemplate.nodataVal;
						colorIdx = 0;
					} else {
						numValid++;
						colorIdx = 1;
	...
	if (ppmFiles.size() >= 1) {
		execConvert(ppmFiles[0], parameterTemplate.imageFile);
	}

	if (ppmFiles.size() >= 2) {
		execConvert(ppmFiles[1], parameterTemplate.imageFile2);
	}

	// Clean up the per-invocation temp files and directory.
	for (const auto &f : ppmFiles) {
		unlink(f.c_str());
	}
	rmdir(ppmDir.c_str());
}
...
void DataSetClass::procBoundary()
{
	...
	while ((ptrOFeature = std::unique_ptr<OGRFeature, GdalHelpers::FeatureDeleter>(
		...
		if (ptrOGeometry != NULL) {
			...
			if (geometryType == OGRwkbGeometryType::wkbPolygon) {
				...
				useGeometryFlag = false;
				isMultiPolygonFlag = false;
				std::cout << "Ignore features of type: "
					  << OGRGeometryTypeToName(geometryType) << std::endl;
			}
			...
			if (useGeometryFlag) {
				do {
					...
					// std::cout << "NUM_POINTS = " << pRing->getNumPoints() <<
					// std::endl;
					if (numPoints > 2) {
						double prevLon = pRing->getX(numPoints - 1);
						double prevLat = pRing->getY(numPoints - 1);
						/* Vertex coordinates come straight from the
						 * file; a crafted file can make them
						 * non-finite or out of range.  Sanitize
						 * before the wrap loop and grid indexing
						 * (see fixVector / vectorCvg). */
						if (!(std::isfinite(prevLon) &&
						      std::isfinite(prevLat)) ||
						    (std::fabs(prevLon) > 720.0)) {
							prevLon = parameterTemplate.minLonWrap;
							prevLat = 0.0;
						}
						if (prevLat < -90.0) {
							prevLat = -90.0;
						} else if (prevLat > 90.0) {
							prevLat = 90.0;
						}
						...
						while (prevLon >=
						       parameterTemplate.minLonWrap + 360.0) {
							prevLon -= 360.0;
						}

						int nx0 = (int)floor(prevLon * samplesPerDeg) -
							  lonN0;
						int ny0 = (int)floor(prevLat * samplesPerDeg) -
							  latN0;
						for (int ptIdx = 0; ptIdx < pRing->getNumPoints();
						     ptIdx++) {
							double lon = pRing->getX(ptIdx);
							double lat = pRing->getY(ptIdx);
							/* Reject crafted non-finite /
							 * out-of-range vertices; clamp
							 * latitude (see fixVector). */
							if (!(std::isfinite(lon) &&
							      std::isfinite(lat)) ||
							    (std::fabs(lon) > 720.0)) {
								continue;
							...
							int nx1 = (int)floor(lon * samplesPerDeg) -
								  lonN0;
							int ny1 = (int)floor(lat * samplesPerDeg) -
								  latN0;

							// std::cout << "POINT " << ptIdx << ": " <<
							// lon << " " << lat << std::endl;

							image->processSegment(prevLon,
									      prevLat,
									      nx0,
									      ny0,
									      lon,
						...
						std::cout << "WARNING: Polygon has " << numPoints
							  << " virtices" << std::endl;
					}
	...
	// PolygonClass::writeMultiGeometry(polyList, std::string("/tmp/") + parameterTemplate.name
	// + "_dbg.kml", 1.0/samplesPerDeg, parameterTemplate.name + "_dbg");
	...
	for (int polyIdx = 0; polyIdx < polyList.size(); ++polyIdx) {
		...
		std::cout << "[" << polyIdx << "] POLYGON NUM_VIRTICES: " << poly->num_bdy_pt[0]
			  << " Deleted " << numDel << " Points from polygon" << std::endl;
		totalNumPts += poly->num_bdy_pt[0];
	}
	std::cout << "TOTAL_NUM_VIRTICES: " << totalNumPts << std::endl;

	PolygonClass::writeMultiGeometry(polyList,
					 parameterTemplate.kmlFile,
					 1.0 / samplesPerDeg,
