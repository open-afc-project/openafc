...
PolygonClass::PolygonClass(std::string kmlFilename, double resolution)
{
	...
	if (found == std::string::npos) {
		std::cout << "SVAL: " << sval << std::endl;
		errStr << "ERROR: unable to find <outerBoundaryIs> while reading file "
		       << kmlFilename;
		throw std::runtime_error(errStr.str());
	}
							    ...
							    double resolution)
{
	...
	do {
		...
		if (cont) {
			...
			/**********************************************************************************/
			/* Remove duplicate endpoint (in kml polygons are closed with last pt =
			 * first pt) */
			/**********************************************************************************/
			if ((clist.size() > 1) && (clist[0] == clist[clist.size() - 1])) {
				clist.pop_back();
...
/******************************************************************************************/
void PolygonClass::writeMultiGeometry(std::vector<PolygonClass *> &polygonList,
				      ...
				      std::string name)
{
	...
	if (!(fkml = fopen(kmlFilename.c_str(), "wb"))) {
		errStr << std::string("ERROR: Unable to open kmlFile \"") + kmlFilename +
				  std::string("\"\n");
		throw std::runtime_error(errStr.str());
	}
	...
	for (int polyIdx = 0; polyIdx < polygonList.size(); ++polyIdx) {
		...
		for (int ptIdx = 0; ptIdx <= polygon->num_bdy_pt[segIdx]; ++ptIdx) {
			int k = ptIdx;
			if (k == polygon->num_bdy_pt[segIdx]) {
				k = 0;
			}
	...
	for (i = 0; i <= n - 1; i++) {
		...
		if (!same_y) {
			...
			if (y2 == b) {
				...
				if (((y2 < b) && (same_y == 1)) || ((y2 > b) && (same_y == -1))) {
					if (x1 < a) {
						num_left++;
					} else {
						num_right++;
