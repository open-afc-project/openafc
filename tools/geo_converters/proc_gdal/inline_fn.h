...
inline void checkStr(const char *varname,
		     int idx1,
		     int idx2,
		     int linenum,
		     char *strname,
...
inline void getParamVal(int &ivar,
			const char *varname,
			int linenum,
			char *strname,
			char *strval,
			const char *filename)
{
	...
	if (chptr) {
		...
		errStr << "ERROR: Invalid file \"" << filename << "\":" << linenum << " variable \""
		       << varname << "\" not specified" << std::endl;
		throw std::runtime_error(errStr.str());
		return;
	}
}

inline void getParamVal(double &dvar,
			const char *varname,
			int linenum,
			char *strname,
			char *strval,
...
inline void getParamVal(char *&svar,
			const char *varname,
			int linenum,
			char *strname,
			char *strval,
