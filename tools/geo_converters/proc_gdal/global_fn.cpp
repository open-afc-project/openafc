...
/**** Modified to be able to read both DOS and UNIX files.                             ****/
/******************************************************************************************/
// Cap for the char-buffer fgetline variant (S0061-44 hardening, mirrored
// from src/afc-engine/global_fn.cpp).  At most FGETLINE_CHAR_MAX bytes are
// written to s (terminating NUL included); callers must supply a buffer of
// at least this size.
	...
	for (i = 0; i < (int)cmd.length(); i++) {
		...
		if (i == ((int)cmd.length()) - 1) {
			if (state == 1) {
				...
				s << "ERROR: Unable to splitOptions() for command \"" << cmd
...
int fileExists(const char *filename)
{
	...
	exists = (intStat ? 0 : (statInfo.st_mode & S_IFDIR) ? 2 : 1);
...
/****    } while(chptr);                                                               ****/
/******************************************************************************************/
const char *getCSVField(const char *str,
			const char *&startPtr,
			int &fieldLen,
			const bool rmWhiteSpaceFlag,
			const char fs)
{
	startPtr = str;
	...
	if (rmWhiteSpaceFlag) {
		while ((fieldLen) && isspace(*startPtr)) {
			startPtr++;
			fieldLen--;
		}
		while ((fieldLen) && isspace(*(startPtr + fieldLen - 1))) {
			fieldLen--;
		}
	}
...
int is_big_endian()
{
	...
	if ((ch_ptr[0] == 0x89) && (ch_ptr[1] == 0xab) && (ch_ptr[2] == 0xcd) &&
	    (ch_ptr[3] == 0xef)) {
		big_endian = 1;
	} else if ((ch_ptr[3] == 0x89) && (ch_ptr[2] == 0xab) && (ch_ptr[1] == 0xcd) &&
		   (ch_ptr[0] == 0xef)) {
		big_endian = 0;
	} else {
		big_endian = -1;
...
{
	// Run "rm -fr <filename>" without a shell: filename is data-derived
	// (e.g. LiDAR dataset paths) and double-quoting inside a system()
	// string does not stop shell metacharacters (`$()`, backticks, a
	// literal '"') from being interpreted -- passing it as a discrete
	// argv entry to rm via execvp avoids shell parsing entirely.
	pid_t pid = fork();
	if (pid == 0) {
		execlp("rm", "rm", "-fr", filename, (char *)NULL);
		_exit(127);
	} else if (pid > 0) {
		int status;
	...
	while (fgetline(fp, line)) {
		...
		if ((lnptr[0] != '#') && (lnptr[0] != '\n')) {
			if ((numPts) >= maxNumPts) {
				...
				chptr += sprintf(chptr,
						 "Number of points in file %s exceeds maxNumPts = "
						 "%d\n",
						 flname,
						 maxNumPts);
				printf("%s", errmsg);
...
int cvtStrToVal(char const *strptr, std::complex<double> &val)
{
	...
	for (i = 0; i <= 1; i++) {
		switch (i) {
			case 0:
				rval = strtod(chptr, &nptr);
				break;
			case 1:
				ival = strtod(chptr, &nptr);
				break;
		}
		if (nptr == chptr) {
			std::stringstream errorStr;
			errorStr << "ERROR in cvtStrToVal() : Unable to cvt to "
				    "std::complex<double> \""
				 << strptr << "\"";
