...
GdalDataModel::GdalDataModel(const std::string &dataSourcePath, const std::string &heightFieldName)
{
	...
	_ptrDataSource = (GDALDataset *)
		GDALOpenEx(dataSourcePath.c_str(), GDAL_OF_VECTOR, NULL, NULL, NULL);
	...
	if (_ptrDataSource->GetLayerCount() != 1) {
		throw std::runtime_error("GdalDataModel::GdalDataModel(): There may be undefined "
					 "behavior if data source contains more than 1 layer");
	}
	...
	// Create new spatial reference to transform current layer's coordinates into our preferred
	// coordinate system GDAL maintains ownership of returned pointer, so we should not manage
	// its memory
	testDestSpatialRef = _ptrLayer->GetSpatialRef();

	// Prepare for coordinate transform
	testCoordTransform = OGRCreateCoordinateTransformation(testSrcSpatialRef,
							       testDestSpatialRef);
	invCoordTransform = OGRCreateCoordinateTransformation(testDestSpatialRef,
							      testSrcSpatialRef);

	if (!heightFieldName.empty()) {
		std::unique_ptr<OGRFeature, GdalHelpers::FeatureDeleter> ptrOFeature;
		ptrOFeature = std::unique_ptr<OGRFeature, GdalHelpers::FeatureDeleter>(
			_ptrLayer->GetNextFeature());
		heightFieldIdx = ptrOFeature->GetFieldIndex(heightFieldName.c_str());

		if (heightFieldIdx == -1) {
			throw std::runtime_error(std::string("GdalDataModel::GdalDataModel(): "
							     "ERROR: data contains no height field "
							     "\"") +
						 heightFieldName + "\"");
		}
	} else {
...
/***********************************/
// Looks at 2.5D data (buildings with multiple heights/polygons) and returns the highest possible
// height for a given test point (ensures accuracy)
double GdalDataModel::getMaxBuildingHeightAtPoint(double latDeg, double lonDeg) const
{
	double maxHeight_m =
		std::numeric_limits<double>::quiet_NaN(); // Instantiate as NaN in case no building
							  // data exists at input lat/lon

	const auto buildingMap = getBuildingsAtPoint(latDeg, lonDeg);

	if (!buildingMap.empty()) {
		const auto maxBuildingPair_m =
			std::max_element(buildingMap.begin(),
					 buildingMap.end(),
...
{ // If in a building return a polygon, if not then return NULL
	...
	// Prepare for coordinate transform
	OGRCoordinateTransformation *tmpCoordTransform =
		OGRCreateCoordinateTransformation(tmpSrcSpatialRef, tmpDestSpatialRef);

	int transformSuccess;
	// Perform transform into WGS-84 lat/lon coordinate system
	int allTransformSuccess =
		tmpCoordTransform->Transform(1, &lon, &lat, nullptr, &transformSuccess);

	// Store input point projected into data source's spatial reference
	GdalHelpers::GeomUniquePtr<OGRPoint> testPoint(
		GdalHelpers::createGeometry<OGRPoint>()); // Create OGRPoint on heap
	testPoint->setX(lon);
	testPoint->setY(
		lat); // lat and lon have been transformed into the layer's spatial reference

	// Filter for features/geometries which geographically intersect with the test point
	if (testPoint.get()->IsValid()) {
		_ptrLayer->SetSpatialFilter(
			testPoint.get()); // FIXME: Does this shrink layer to only include features
					  // which intersected with a previous test point?
	} else {
		throw std::runtime_error("GdalDataModel::getBuildingsAtPoint(): OGRPoint testPoint "
					 "returned invalid.");
	}
		...
		if (ptrOGeometry != NULL &&
			...
			if (heightFieldIdx != -1) {
				returnMap.insert(std::make_pair(
					static_cast<int64_t>(ptrOFeature->GetFID()),
					ptrOFeature->GetFieldAsDouble(heightFieldIdx)));
			} else {
				returnMap.insert(
					std::make_pair(static_cast<int64_t>(ptrOFeature->GetFID()),
						       0.0));
			}
		} else {
			std::cout << "GdalDataModel::getBuildingsAtPoint(): Can't find polygon "
				     "geometries in current feature";
		}
	}
...
void GdalDataModel::printDebugInfo() const
{
	...
	while ((ptrOFeature = std::unique_ptr<OGRFeature, GdalHelpers::FeatureDeleter>(
		...
		if (ptrOGeometry != NULL &&
		    ptrOGeometry->getGeometryType() == OGRwkbGeometryType::wkbPolygon) {
			double height = (heightFieldIdx != -1 ?
						 ptrOFeature->GetFieldAsDouble(heightFieldIdx) :
						 0.0);
			returnMap.insert(std::make_pair(static_cast<int64_t>(ptrOFeature->GetFID()),
							height));
			...
			for (int fieldIdx = 0; fieldIdx < ptrOFeature->GetFieldCount();
			     ++fieldIdx) {
				std::cout << "    FIELD_" << fieldIdx << ": "
					  << ptrOFeature->GetFieldDefnRef(fieldIdx)->GetNameRef()
					  << " = " << ptrOFeature->GetFieldAsDouble(fieldIdx)
					  << std::endl;
			}
#endif
...
#if 1
				std::cout << "    POINT " << ptIdx << " : " << pRing->getX(ptIdx)
					  << " " << pRing->getY(ptIdx) << " " << lon << " " << lat
					  << std::endl;
#endif
	...
	std::cout << "RLAN "
		  << " : " << rlanX << " " << rlanY << " " << rlanLon << " " << rlanLat
		  << std::endl;
	std::cout << "FS   "
		  << " : " << fsX << " " << fsY << " " << fsLon << " " << fsLat << std::endl;
	...
	GdalHelpers::GeomUniquePtr<OGRLineString> signalPath(
		GdalHelpers::createGeometry<OGRLineString>());
	...
	_ptrLayer->SetSpatialFilter(
		signalPath.get()); // Filter entire building database for only polygons which
				   // intersect with signal path into constrained layer
...
/******************************************************************************************/
void GdalDataModel::getExtents(double &minLon,
			       double &maxLon,
			       double &minLat,
			       double &maxLat,
