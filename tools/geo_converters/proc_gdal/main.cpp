...
int main(int argc, char **argv)
{
	...
	try {
		...
		if (dataSet->parameterTemplate.seed == 0) {
			...
			unsigned int seed = (unsigned int)((((unsigned int)tseed.tv_sec) *
								    1000000000 +
							    tseed.tv_nsec) &
							   0xFFFFFFFF);
	...
	std::cout << "Elapsed time = " << (t2 - t1) << " sec = " << elapsedTimeDay << " days "
		  << elapsedTimeHour << " hours " << elapsedTimeMin << " min " << elapsedTimeSec
		  << " sec." << std::endl;
...
void set_options(int argc, char **argv, std::string &templateFile)
{
	static const char *help_msg[] = {" -templ     --file    parameter template file",
					 " -h         --help    print this help message",
					 " ",
					 0};
	static const char *usage[] = {" [ -option value] [ -h ]", 0};
	char *name = *argv;
	const char **p = help_msg;
	const char **u = usage;
	while (--argc > 0) {
		argv++;
		if (strcmp(*argv, "-templ") == 0) {
			templateFile = std::string(*++argv);
			argc--;
		}
