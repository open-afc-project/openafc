...
/******************************************************************************************/
ImageClass::ImageClass(int lonN0Val,
		       int lonN1Val,
		       int latN0Val,
		       int latN1Val,
		       int samplesPerDegVal) :
	lonN0(lonN0Val),
	lonN1(lonN1Val),
	latN0(latN0Val),
	latN1(latN1Val),
	samplesPerDeg(samplesPerDegVal)
{
	...
	if (!scan) {
		throw std::runtime_error("ERROR: memory allocation failed for scan");
	}
	for (int lonIdx = 0; lonIdx < numLon; ++lonIdx) {
		scan[lonIdx] = (int *)malloc(numLat * sizeof(int));
		if (!scan[lonIdx]) {
			throw std::runtime_error("ERROR: memory allocation failed for scan column");
		}
...
/******************************************************************************************/
void ImageClass::processSegment(double lon0,
				double lat0,
				int nx0,
				int ny0,
				double lon1,
				...
				int ny1)
{
	/* nx0/ny0/nx1/ny1 derive from file-supplied coordinates; a crafted
	 * file can place them outside the numLon x numLat grid.  Bound every
	 * store (see the fixVector/vectorCvg clamp in data_set.cpp). */
	auto setScan = [this](int x, int y) {
		if ((x >= 0) && (x < numLon) && (y >= 0) && (y < numLat)) {
			scan[x][y] = 1;
		}
	};
	...
	while ((nx != nx1) || (ny != ny1)) {
		double epsx = (sx ? ((lonN0 + nx + (sx + 1) / 2) - xval * samplesPerDeg) /
					       (deltax * samplesPerDeg) :
				    1.0);
		double epsy = (sy ? ((latN0 + ny + (sy + 1) / 2) - yval * samplesPerDeg) /
					       (deltay * samplesPerDeg) :
				    1.0);
		if (epsx < epsy) {
			xval += epsx * deltax;
			yval += epsx * deltay;
		...
		for (latIdx = 1; latIdx < numLat - 1; ++latIdx) {
			if (scan[lonIdx][latIdx] == val) {
				for (int rotIdx = 0; rotIdx < 4; ++rotIdx) {
					...
					if ((lonIdx + ix >= 0) && (lonIdx + ix < numLon) &&
					    (latIdx + iy >= 0) && (latIdx + iy < numLat)) {
						if ((scan[lonIdx + ix][latIdx + iy] != val) &&
...
PolygonClass *ImageClass::createPolygon(int lonIdx, int latIdx, int polyVal)
{
	...
	while (checkIdx <= grouplist.size() - 1) {
		...
		for (int rotIdx = 0; rotIdx < 4; ++rotIdx) {
			ix = rotx[rotIdx];
			iy = roty[rotIdx];
			if ((lonIdx + ix >= 0) && (lonIdx + ix < numLon) && (latIdx + iy >= 0) &&
			    (latIdx + iy < numLat)) {
				if (scan[lonIdx + ix][latIdx + iy] == 1) {
					scan[lonIdx + ix][latIdx + iy] = polyVal;
					grouplist.push_back(
						std::make_tuple(lonIdx + ix, latIdx + iy));
				}
			}
		}
		checkIdx++;
