...
# Validate the enforcement flags at startup and refuse to run on values
# outside the accepted set, so a case/spelling variant or a set-but-empty
# value can never silently change what the nginx maps and the Python
# services (ratafc.py: 'true'/'1'/'yes' case-insensitive) enforce.
for FLAG_NAME in AFC_ENFORCE_HTTPS AFC_ENFORCE_MTLS; do
    eval "FLAG_VALUE=\${$FLAG_NAME:-}"
    case "$(echo "$FLAG_VALUE" | tr '[:upper:]' '[:lower:]')" in
        true|1|yes|false|0|no|off) ;;
        *)
            echo "CRITICAL: $FLAG_NAME='$FLAG_VALUE' is not in the accepted set (true/1/yes/false/0/no/off, case-insensitive); refusing to start."
            exit 1
            ;;
    esac
done
