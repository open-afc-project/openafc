...
# A CA bundle is at most a few hundred KB; the objstorage object is untrusted
# input buffered into memory at every acceptor start, so refuse oversized
# objects before allocation (an OOM of this process takes down the WAN nginx
# that shares the container's PID1 shell — see dispatcher/entrypoint.sh).
BUNDLE_MAX_BYTES = 1024 * 1024  # 1 MiB
...
# Replay protection for cmd_remove.  The last-accepted timestamp is
# persisted to a sidecar file (mirroring the bundle-install .ver floor in
# run_restart) so an acceptor restart within the freshness window does not
# reopen acceptance of an already-seen signed message.  The nonce set stays
# process-local; the persisted timestamp floor bounds it across restarts.
CMD_REMOVE_TS_FILE = '/etc/nginx/certs/cmd_remove.ts'


def _load_last_remove_ts() -> int:
    try:
        with open(CMD_REMOVE_TS_FILE, 'r') as _tf:
            return int(_tf.read().strip())
    except (OSError, ValueError):
        return -1


_last_remove_ts = _load_last_remove_ts()
...
def run_restart(cfg):
    ...
    with DataIf().open(cfg['OBJST_CERT_CLI_BUNDLE']) as hfile:
        if hfile.head():
            ...
            bundle_bytes = hfile.read(max_bytes=BUNDLE_MAX_BYTES)
            ...
            if not _token:
                ...
                with DataIf().open(_hmac_path) as _hf:
                    if _hf.head():
                        ...
                        try:
                            _sidecar = _hf.read(
                                max_bytes=BUNDLE_MAX_BYTES).decode().strip()
                        except (UnicodeDecodeError, OSError,
                                RuntimeError) as _exc:
                            app_log.error(
...
def _verify_remove(msg) -> bool:
    ...
    # Persist the floor so a restart within CMD_REMOVE_MAX_AGE_NS cannot
    # replay this message (same pattern as the bundle-install .ver sidecar).
    # A persist failure degrades to the prior window-bounded behaviour and
    # must not reject a legitimately signed removal, so it is logged only.
    try:
        with open(CMD_REMOVE_TS_FILE, 'w') as _tf:
            _tf.write(str(_ts))
    except OSError as _exc:
        app_log.error(
            "cmd_remove: failed to persist replay floor to %s: %s",
            CMD_REMOVE_TS_FILE, _exc)
    return True
