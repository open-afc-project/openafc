...
def _afc_req_auth_headers() -> dict:
    """ Authentication headers for precompute POSTs to the AFC inquiry URL.

    The Internal AFC route requires x-afc-internal-token plus the
    batch-caller x-afc-precompute-token; without them every precompute
    attempt is rejected 403 before processing. Token files are provisioned
    via AFC_INTERNAL_TOKEN_FILE / AFC_PRECOMPUTE_TOKEN_FILE (docker
    secrets).
    """
    headers = {}
    for env_var, header in (("AFC_INTERNAL_TOKEN_FILE",
                             "x-afc-internal-token"),
                            ("AFC_PRECOMPUTE_TOKEN_FILE",
                             "x-afc-precompute-token")):
        file_name = os.environ.get(env_var)
        if not file_name:
            continue
        try:
            ...
            LOGGER.error("rcache: token file '%s' (%s) is unreadable: %s - "
                         "precompute requests will be rejected by the AFC "
                         "inquiry endpoint", file_name, env_var, ex)
            continue
        if token:
    ...
    if rcache_service is None:
        rcache_service = \
            RcacheService(
                ...
                afc_req_headers=_afc_req_auth_headers(),
