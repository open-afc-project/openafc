...
|<small>**RCACHE_UPDATE_ON_SEND**</small>|<small>*Current value*: True<br>*Defined in*: worker/Dockerfile<br>*Used in images*: worker</small>|TRUE if worker sends update request to Rcache service (this speeds up AFC request processing, but prolongs worker existence). FALSE if msghnd/rat_server send update request to Rcache service. This setting consumed by worker, but indirectly affects msghnd and rat_server services operation|
