...
||<small>**RCACHE_API_KEY_FILE**</small>|<small>*Current value*: (not set — auth disabled)<br>*Defined in*: environment<br>*Used in images*: rcache</small>|Path to a file containing the bearer token required by Rcache mutating endpoints (`/update`, `/invalidate`, etc.). **Should be set in production** to prevent unauthenticated cache write-access. If the file is configured but unreadable the service **fails closed** (HTTP 503).|
