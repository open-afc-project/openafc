...
### 4.4 Overlay Environment & Monitoring Reference

When deploying an overlay (e.g., `local/` or `prod/`), configure the following variables in `<overlay>/.env` or `<overlay>/docker-compose.yaml` depending on desired profile features:

| Variable | Recommended / Default | Purpose |
|---|---|---|
| `SOCKET_PROXY_LOGS` | `1` (when monitoring) | Enables `/containers/{id}/logs` access in `socket-proxy` for Alloy container log tailing into Loki. Default in `docker-compose.yaml` is `0` for security hardening (AR-16). |
| `SOCKET_PROXY_INSPECT` | `1` (when monitoring) | Enables `/containers/{id}/json` inspect access in `socket-proxy` for Alloy container label discovery. Default is `0`. |
| `DOCKER_DATA_ROOT` | `/var/lib/docker` (or host path) | Host Docker daemon data root path (check: `docker info --format '{{.DockerRootDir}}'`). Used by `cAdvisor` to mount the `overlay2/layerdb` sub-tree for container ID resolution. |
| `GRAFANA_PROXY_SUBNET` | `192.168.128.0/29` (local), `192.168.129.0/29` (prod) | Unique subnet for the internal `grafana-proxy` Docker network per overlay. Prevents Docker subnet collision when running multiple overlays. |
| `GRAFANA_PROXY_DISPATCHER_IP` | `192.168.128.2` (local), `192.168.129.2` (prod) | Dispatcher container IP on `grafana-proxy` network, passed to Grafana as `GRAFANA_AUTH_PROXY_WHITELIST`. |
| `GRAFANA_PROXY_GRAFANA_IP` | `192.168.128.3` (local), `192.168.129.3` (prod) | Grafana container IP on `grafana-proxy` network. |
| `ULS_FORCE_UPDATE` | `true` (dev only) | Local development setting for `uls_downloader` to bypass hash-manifest requirement. **Must NOT be set in production.** |
| `AFC_WORKER_CELERY_WORKERS` | `rat_1 rat_2` (prod), `rat_1` (local) | Worker instances for parallel WebUI vs AP request processing. |
| `RCACHE_PRECOMPUTE_QUOTA` | `1` (local dev), unset (prod) | Limits precompute queue depth in local dev so test requests are not queued behind background precomputations. |
...
> **`docker.sock` access & Loki Log Aggregation**: The monitoring stack uses a
> `socket-proxy` service that exposes only read-only container and event
> endpoints over an internal network (`monitor-net`).  Direct host-socket mounts are not used.
...
>
> **Loki Container Log Collection**: To enable Alloy container log aggregation into Loki,
> set `SOCKET_PROXY_LOGS=1` and `SOCKET_PROXY_INSPECT=1` in your overlay `.env` or
> overlay `docker-compose.yaml`. By default in `docker-compose.yaml`, these fall back to `0`
> (`LOGS: ${SOCKET_PROXY_LOGS:-0}`) to enforce principle-of-least-privilege security
> defaults (AR-16). Without setting `SOCKET_PROXY_LOGS=1` and `SOCKET_PROXY_INSPECT=1`, `socket-proxy`
> will reject Alloy log-tailing requests with `403 Forbidden` and Loki will show zero log streams.
